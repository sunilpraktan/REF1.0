...
{
    public class ADM_R02_VM : WorkspaceViewModel<STD_LIST_BE>
    {
        ...
        public static readonly DependencyProperty AutoSuggestTextViewModelProperty = DependencyProperty.Register("AutoSuggestTextViewModel", typeof(AutoSuggestTextViewModel<dynamic>), typeof(ADM_R02_VM));
        ...
        private AutoSuggestTextViewModel<dynamic> _AS_PAYER { get; set; }
        public AutoSuggestTextViewModel<dynamic> AS_PAYER
        {
            get { return _AS_PAYER; }
            set
            {
                if (_AS_PAYER != value)
                {
                    _AS_PAYER = value; RaisePropertyChanged("AS_PAYER");
                }
            }
        }
        ...
        #endregion
        public ADM_R02_VM(string ts_code) : base()
        {
        ...
        private void LoadInitialData()
        {
            try
            {
                ...
                MC = repository_MC.GetDataWithReturnDomainObject<STD_MIS_MC_BE>(MC, Request, "ADM_R02_BL", "ADM", " ", 0, "");
                ...
                List<STD_PARTY> PARTY_LIST_PAYER = MC.PARTY_LIST.Where(x => x.party_code != null).ToList();
                SuggestedValue = new ValueConverter(x => x == null ? "" : ((STD_PARTY)x).party_code ?? "");
                TheFilter = (o, prefix) => (((STD_PARTY)o).party_code ?? "").ToLower().Contains(prefix.ToString().ToLower()) || (((STD_PARTY)o).party_name ?? "").ToLower().Contains(prefix.ToString().ToLower());
                AS_PAYER = new AutoSuggestTextViewModel<dynamic>(PARTY_LIST_PAYER, TheFilter, SuggestedValue, "party_code", true);
                AS_PAYER.AutoSuggestVM.IsEmptyValueAllowed = true; AS_PAYER.AutoSuggestVM.IsFreeTextAllowed = false;
            ...
            try
            {
                ...
                else
                {
                    if (REQ_PARA_OBJ != null)
                    {
                        ...
                        //// POC: RDLC shift to web service is tested successfully on 10-07-2023. Club all report display methods in single generic method.
                        //string filePath = @"D:\Reflection ERP 2022\ReflectionWebServices\ReflectionServiceHost\Reports\REPORTS_STD\MIS\ADM_M046.rdlc";
                        //byte[] fileStream = repository_MC.DownloadFile(MIS_OBJ.obj_info, "RDLC", filePath, AppSessionState.client, REQ_PARA_OBJ.comp_code);
                        //ReportManager.DisplayReport(objDataSource, objDataSourceName, MIS_OBJ.obj_path + MIS_OBJ.obj_file, getParametersList(), MIS_OBJ.obj_name, fileStream)
