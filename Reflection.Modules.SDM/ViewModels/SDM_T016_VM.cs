...
{
    public class SDM_T016_VM : WorkspaceViewModel<TSK_T001_C>
    {
        ...
        public static readonly DependencyProperty AutoSuggestTextViewModelProperty = DependencyProperty.Register("AutoSuggestTextViewModel", typeof(AutoSuggestTextViewModel<dynamic>), typeof(SDM_T016_VM));
        ...
        private IEnumerable _STATUS_COLLECTION;
        public IEnumerable STATUS_COLLECTION
        {
            get { return _STATUS_COLLECTION; }
            set
            {
                _STATUS_COLLECTION = value;

                RaisePropertyChanged("STATUS_COLLECTION");
            }
        }
        ...
        private IEnumerable _ACTIVITY_TYPE_LIST { get; set; }
        public IEnumerable ACTIVITY_TYPE_LIST
        {
            get { return _ACTIVITY_TYPE_LIST; }
            set
            {
                if (_ACTIVITY_TYPE_LIST != value)
                {
                    _ACTIVITY_TYPE_LIST = value; RaisePropertyChanged("ACTIVITY_TYPE_LIST");
                }
            }
        }
        ...
        private IEnumerable _SOURCE_COLLECTION;
        public IEnumerable SOURCE_COLLECTION
        {
            get { return _SOURCE_COLLECTION; }
            set
            {
                _SOURCE_COLLECTION = value;

                RaisePropertyChanged("SOURCE_COLLECTION");
            }
        }

        private IEnumerable _ACTIVITY_COLLECTION;
        public IEnumerable ACTIVITY_COLLECTION
        {
            get { return _ACTIVITY_COLLECTION; }
            set
            {
                _ACTIVITY_COLLECTION = value;

                RaisePropertyChanged("ACTIVITY_COLLECTION");
            }
        }
        ...
        private void CollectionChangedNotifyForMasterValue(object sender, NotifyCollectionChangedEventArgs e)
        {
            try
            {
                ...
                if (e.Action == NotifyCollectionChangedAction.Add)
                {
                    foreach (TSK_T001_C item in e.NewItems)
                    {
                        if (string.IsNullOrWhiteSpace(item.doc_no))
                        {
                            item.Click = true;
                            ...
                            //item.doc_type = "LM"; //if (item.act_action == "Log" && item.action_type == "Meeting")
                            //item.doc_cat = "LM";
                            item.doc_type = "SM"; // if (item.act_action == "Schedule" && item.action_type == "Meeting")
                            item.doc_cat = "SM";
                            //item.doc_type = "LC"; // if (item.act_action == "Log" && item.action_type == "Call")
                            //item.doc_cat = "LC";
                            //item.doc_type = "SC"; // if (item.act_action == "Schedule" && item.action_type == "Call")
                            //item.doc_cat = "SC";
        ...
        #region Constructor
        public SDM_T016_VM(string ts_code, string doc_cat) : base()
        {
            ...
            BulkActivityCollection.CollectionChanged += new System.Collections.Specialized.NotifyCollectionChangedEventHandler(CollectionChangedNotifyForMasterValue);
        ...
        public SDM_T016_VM(string ts_code, string doc_cat, string doc_no) : base()
        {
            this.doc_cat_vm = doc_cat;
            this.ts_code_vm = ts_code;
            MasterEntity = new TSK_T001_C();
        ...
        private void LoadInitialData()
        {
            try
            {
                string Request = "LoadInitialData" + "!@" + AppSessionState.client + "!@" + AppSessionState.OBJ_COMPANY.comp_code + "!@" + AppSessionState.OBJ_LOCATION.location_id + "!@" + this.doc_cat_vm + "!@" + this.doc_cat_vm + "!@" + AppSessionState.EmpId + "!@" + AppSessionState.UserID;
                ...
                ACTIVITY_TYPE_LIST = MC.ACTIVITY_TYPE_LIST;
                ...
                var locationMaster = (from o in LocationList where o.comp_code == AppSessionState.OBJ_COMPANY.comp_code select o).ToList();
                ...
                STATUS_COLLECTION = MC.STATUS_LIST;

                var source = (from o in MC.PARAMETERS_VALUES_LIST where o.para_code == "P001" select o);
                var activity = (from o in MC.PARAMETERS_VALUES_LIST where o.para_code == "P002" select o);
            ...
            try
            {
                if (RequestList != null && RequestList.Count > 0)
                {
                    foreach (var item in RequestList)
                    {
                        ...
                        item.user_source2 = AppSessionState.UserSource2;
                        item.userid = AppSessionState.UserID;
                        item.session_id = AppSessionState.session_id;
                        item.ts_code = ts_code_vm;
        ...
        private void OpenDocumentViewer(object InputValue)
        {
            ...
            { 
                ...
                TSK_T001_C OBJ_ACT = (TSK_T001_C)InputValue;

                if (!string.IsNullOrEmpty(OBJ_ACT.doc_no))
                {
                    string Request = "LOAD_ATTACHMENTS" + "!@" + AppSessionState.client + "!@" + AppSessionState.OBJ_COMPANY.comp_code + "!@" + MasterEntity.location_Id + "!@" + OBJ_ACT.doc_cat + "!@" + OBJ_ACT.doc_type + "!@" + OBJ_ACT.doc_no + "!@" + OBJ_ACT.owner;
                    MCTemp = repository_MC.GetDataWithReturnDomainObject<MultipleContext_TSK_T001_C_Bulk>(MCTemp2, Request, "CRM_ActivityBulk", "CRM", "LoadData", 0, "");

                    Messenger.Default.Send<DocumentViewerPayload>(new DocumentViewerPayload() { DocumentNumber = OBJ_ACT.doc_no.Replace("/", "--"), DocumentList = MCTemp.ATTACHMENT_LIST, client = AppSessionState.client, comp_code = OBJ_ACT.comp_code });
                }
        ...
        private void InsertParty(object InputValue, bool NewRow, bool AllowDuplicate, bool AllowModify)
        {
            try
            {
                ...
                if (POPUPEntityObject != null) //&& MasterEntity.PartyId != POPUPEntityObject.PartyId) //Application:New Record Only.Condition: Only enter in the code block if EntityObject is Not null and not assigning same value again if event fire twice.
                {
                    ...
                    Request = "LoadPartyDetail" + "!@" + ItemEntityObject.PartyId + "!@" + AppSessionState.client + "!@" + (ItemEntityObject.comp_code ?? AppSessionState.OBJ_COMPANY.comp_code);
        ...
        private void InsertSales(object InputValue, bool NewRow, bool AllowDuplicate, bool AllowModify)
        {
            try
            {
                ...
                if (POPUPEntityObject != null)
                {
                    ...
                    ItemEntityObject.contact_person = POPUPEntityObject.buyer_name;
                    ItemEntityObject.person_number = POPUPEntityObject.cp_no;
                    ItemEntityObject.EmailId = POPUPEntityObject.cp_mail;
                }
        ...
        private void LoadData(object InputValue)
        {
            try
            {
                ...
                {
                    string Request = "LoadData" + "!@" + AppSessionState.OBJ_COMPANY.comp_code + "!@" + MasterEntity.location_Id + "!@" + AppSessionState.EmpId + "!@" + Convert.ToDateTime(SearchFromDate).ToString("MM/dd/yyyy") + "!@" + Convert.ToDateTime(SearchToDate).ToString("MM/dd/yyyy") + "!@" + MasterEntity.owner;
        ...
        private void FilterFlipGrid()
        {
            try
            {
                if (BulkActivityCollection != null && BulkActivityCollection.Count > 0)
                {

                    if (BulkActivityCollection != null)
                    {
                        DataGridViewFilter = CollectionViewSource.GetDefaultView(BulkActivityCollection);
                        DataGridViewFilter.Filter = adv => (((TSK_T001_C)adv).PartyId ?? "").ToString().ToLower().Contains((FLTR_FLIPGRID ?? "").ToString().ToLower()) ||
                            (((TSK_T001_C)adv).party_name ?? "").ToString().ToLower().Contains((FLTR_FLIPGRID ?? "").ToString().ToLower()) ||
                            (((TSK_T001_C)adv).sono ?? "").ToString().ToLower().Contains((FLTR_FLIPGRID ?? "").ToString().ToLower()) ||
                            (((TSK_T001_C)adv).project_name ?? "").ToString().ToLower().Contains((FLTR_FLIPGRID ?? "").ToString().ToLower()) ||
                            (((TSK_T001_C)adv).contact_person ?? "").ToString().ToLower().Contains((FLTR_FLIPGRID ?? "").ToString().ToLower()) ||
                            (((TSK_T001_C)adv).person_number ?? "").ToString().ToLower().Contains((FLTR_FLIPGRID ?? "").ToString().ToLower()) ||
                            (((TSK_T001_C)adv).EmailId ?? "").ToString().ToLower().Contains((FLTR_FLIPGRID ?? "").ToString().ToLower()) ||
                            (((TSK_T001_C)adv).owner ?? "").ToString().ToLower().Contains((FLTR_FLIPGRID ?? "").ToString().ToLower()) ||
                            (((TSK_T001_C)adv).EmpNameDisplay ?? "").ToString().ToLower().Contains((FLTR_FLIPGRID ?? "").ToString().ToLower()) ||
                            (((TSK_T001_C)adv).t_status ?? "").ToString().ToLower().Contains((FLTR_FLIPGRID ?? "").ToString().ToLower()) ||
                            (((TSK_T001_C)adv).act_desc ?? "").ToString().ToLower().Contains((FLTR_FLIPGRID ?? "").ToString().ToLower()) ||
                            (((TSK_T001_C)adv).note ?? "").ToString().ToLower().Contains((FLTR_FLIPGRID ?? "").ToString().ToLower());
        ...
        private string _FLTR_FLIPGRID;
        public string FLTR_FLIPGRID
        {
            get { return _FLTR_FLIPGRID; }
            set
            {
                _FLTR_FLIPGRID = value;
                RaisePropertyChanged("FLTR_FLIPGRID");
                FilterFlipGrid();
            }
        }
        ...
        //    if (BulkActivityCollection != null)
        //    {
        //        BulkActivityCollection.Refresh();
        //    }
        //}
        //public bool FLTR_FLIPGRID(object obj)
        //{
