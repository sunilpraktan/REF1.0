...
{
    public class SDM_M0017_VM : WorkspaceViewModel<ADM_M0061>
    {
        ...
        private string _FLTR_STR_ITEM_GRID;
        public string FLTR_STR_ITEM_GRID
        {
            get { return _FLTR_STR_ITEM_GRID; }
            set
            {
                if (_FLTR_STR_ITEM_GRID != value)
                {
                    _FLTR_STR_ITEM_GRID = value;
                    RaisePropertyChanged("FLTR_STR_ITEM_GRID");
        ...
        public IEnumerable PLANT_LIST
        {
            ...
            set
            {
                if (_PLANT_LIST != value)
                {
                    _PLANT_LIST = value; RaisePropertyChanged(nameof(PLANT_LIST));
                }
            }
        }
        ...
        private ADM_M0061_A _SETTING_OBJ;
        public ADM_M0061_A SETTING_OBJ
        {
            get
            {
                return _SETTING_OBJ;
            ...
            set
            {
                if (_SETTING_OBJ != value)
                {
                    _SETTING_OBJ = value;
                    RaisePropertyChanged(nameof(SETTING_OBJ));
                }
            }
        }
        ...
        //public static readonly DependencyProperty AutoSuggestTextViewModelProperty = DependencyProperty.Register("AutoSuggestTextViewModel", typeof(AutoSuggestTextViewModel<dynamic>), typeof(SDM_M0017_VM));
        ...
        public RelayCommand<object> cmdRunCosting { get; private set; }
        #endregion

        #region . Constructor .
        public SDM_M0017_VM(string doc_cat, string ts_code)
            : base()
        {
        ...
        public SDM_M0017_VM(string doc_cat, string ts_code, string doc_no)
            : base()
        {
            ts_code_vm = ts_code;
            doc_cat_vm = doc_cat;
        ...
        {
            string Request = "LOAD_BACKFLIP" + "!@" + AppSessionState.client + "!@" + (REQ_PARA_OBJ.comp_code ?? AppSessionState.OBJ_COMPANY.comp_code) + "!@" + (REQ_PARA_OBJ.location_id ?? "") + "!@" + doc_cat_vm + "!@" + (REQ_PARA_OBJ.doc_type ?? doc_cat_vm) + "!@" + (REQ_PARA_OBJ.active_code ?? "").ToString() + "!@" + Convert.ToDateTime(REQ_PARA_OBJ.from_date).ToString("MM/dd/yyyy") + "!@" + Convert.ToDateTime(REQ_PARA_OBJ.to_date).ToString("MM/dd/yyyy") + "!@" + AppSessionState.UserID + "!@" + REQ_PARA_OBJ.party_code + "!@" + REQ_PARA_OBJ.t_status;
        ...
        private void RunCosting(object InputValue)
        {
            try
            {
                ADM_M0061_A ENTITY_OBJ = null;
                if (InputValue.GetType() != typeof(string) && InputValue != null)
                {
                    if (((IEnumerable)InputValue).Cast<ADM_M0061_A>().Count() > 0)
                    {
                        ENTITY_OBJ = ((IEnumerable)InputValue).Cast<ADM_M0061_A>().ToList()[0];
                    }
                }
                if (ENTITY_OBJ != null)
                {
                    string Request = "RUN_COSTING!@" + ENTITY_OBJ.client + "!@" + ENTITY_OBJ.comp_code + "!@!@" + doc_cat_vm + "!@" + doc_cat_vm + "!@" + ENTITY_OBJ.item_code + "!@" + (ENTITY_OBJ.sku ?? "") + "!@" + ENTITY_OBJ.curr_code + "!@" + Convert.ToDateTime(MasterEntity.price_date).ToString("MM/dd/yyyy") + "!@" + (ENTITY_OBJ.party_code ?? "");
                    MC_TEMP = REPOSITORY_OBJ_TEMP.GetDataWithReturnDomainObject<ADM_M0061>(MC_TEMP, Request, "ADM_M0061_BL", "ADM", "", 0, "");
                    ...
                    if(MC_TEMP.ITEM_ENTITY_LIST != null)
                    {
                        ...
                        {
                            ENTITY_OBJ = MC_TEMP.ITEM_ENTITY_LIST[0];
                            ITEM_ENTITY_OBJ.billing_doc_cat = ENTITY_OBJ.billing_doc_cat;
                            ITEM_ENTITY_OBJ.billing_doc = ENTITY_OBJ.billing_doc;
                            ITEM_ENTITY_OBJ.billing_item_row_id = ENTITY_OBJ.billing_item_row_id;
                            ITEM_ENTITY_OBJ.ref_doc_cat = ENTITY_OBJ.ref_doc_cat;
                            ITEM_ENTITY_OBJ.ref_item_row_id = ENTITY_OBJ.ref_item_row_id;
                            ITEM_ENTITY_OBJ.purchase_price = ENTITY_OBJ.purchase_price;
                            ITEM_ENTITY_OBJ.exch_rate_pur = ENTITY_OBJ.exch_rate_pur;
                            ITEM_ENTITY_OBJ.curr_code = ENTITY_OBJ.curr_code;
                            ITEM_ENTITY_OBJ.profit_ratio = ENTITY_OBJ.profit_ratio;
                            ITEM_ENTITY_OBJ.freight1 = ENTITY_OBJ.freight1;
                            ITEM_ENTITY_OBJ.freight2 = ENTITY_OBJ.freight2;
                            ITEM_ENTITY_OBJ.freight3 = ENTITY_OBJ.freight3;
                            ITEM_ENTITY_OBJ.profit_ratio = ENTITY_OBJ.profit_ratio;
                            ITEM_ENTITY_OBJ.profit_value = ENTITY_OBJ.profit_value;
                            ITEM_ENTITY_OBJ.curr_code_purchase = ENTITY_OBJ.curr_code_purchase;
                            ITEM_ENTITY_OBJ.landed_cost = ENTITY_OBJ.landed_cost;
                            ITEM_ENTITY_OBJ.profit_value = ENTITY_OBJ.profit_value;
                            ITEM_ENTITY_OBJ.unit_price = ENTITY_OBJ.unit_price;
                            ITEM_ENTITY_OBJ.party_code = ENTITY_OBJ.party_code;
                            ITEM_ENTITY_OBJ.party_name = ENTITY_OBJ.party_name;
                        }
                    }
                }
            }
        ...
        void ModelChangeNotification_Item(object sender, EventArgs e)
        {
            try
            {
                ...
                if (sender.ToString()    == "freight1"     || sender.ToString() == "freight2"   || sender.ToString() == "freight3" 
                    || sender.ToString() == "duties"       || sender.ToString() == "other_cost" || sender.ToString() == "profit_ratio" 
                    || sender.ToString() == "profit_value" || sender.ToString() == "price_var"  || sender.ToString() == "price_var_value" 
                    || sender.ToString() == "price_tol"    || sender.ToString() == "purchase_price" || sender.ToString() == "base_price")
                {
                    if (sender.ToString() == "base_price") // NOTE: remove one price because base and purchase price is same, but make sure first before remove.
                    {
                        ITEM_ENTITY_OBJ.purchase_price = ITEM_ENTITY_OBJ.base_price; // NOT: base price shuld be the price [paid to the manufacture = purchase price
                    }
        ...
        void Computation()
        {
            ...
            {
                ITEM_ENTITY_OBJ.landed_cost = (ITEM_ENTITY_OBJ.purchase_price ?? 0) + (ITEM_ENTITY_OBJ.freight1 ?? 0) + (ITEM_ENTITY_OBJ.freight2 ?? 0) + (ITEM_ENTITY_OBJ.freight3 ?? 0) + (ITEM_ENTITY_OBJ.duties ?? 0) + (ITEM_ENTITY_OBJ.other_cost ?? 0);
                ITEM_ENTITY_OBJ.profit_value = ((ITEM_ENTITY_OBJ.landed_cost ?? 0) * (ITEM_ENTITY_OBJ.profit_ratio ?? 0)) / 100;
                ITEM_ENTITY_OBJ.unit_price = (ITEM_ENTITY_OBJ.landed_cost ?? 0) + (ITEM_ENTITY_OBJ.profit_value ?? 0);
            }
        }
        ...
        private void CommandInitialisation()
        {
            ...
            cmdRunCosting = new RelayCommand<object>(items => { if (items == null) { return; } RunCosting(items); });
        }
        private void LoadInitialData()
        {
            try
            {
                ...
                PLANT_LIST = AppSessionState.LOCATION_LIST.Where(x => x.comp_code == AppSessionState.OBJ_COMPANY.comp_code).ToList();
                ...
                #endregion
                if (MC.CATLOG_SETTING_LIST != null)
                {
                    if (MC.CATLOG_SETTING_LIST.Count > 0)
                    {
                        SETTING_OBJ = MC.CATLOG_SETTING_LIST[0];
                    }
                }
            ...
            try
            {
                ...
                else if (ParameterObject != null)
                {
                    if (((IEnumerable)ParameterObject).Cast<STD_LIST_BE>().ToList().Count > 0)
                    {
                        ...
                        Request = "LOAD_DOC_BY_DOC_NO" + "!@" + AppSessionState.client + "!@" + (REQ_PARA_OBJ.comp_code ?? AppSessionState.OBJ_COMPANY.comp_code) + "!@" + (REQ_PARA_OBJ.location_id ?? "") + "!@" + doc_cat_vm + "!@" + (REQ_PARA_OBJ.doc_type ?? doc_cat_vm) + "!@" + ParameterEntityObject.doc_no;
        ...
        private bool Validation()
        {
            ...
            if (string.IsNullOrWhiteSpace(MasterEntity.obj_type))//when form is blank and we save the record
            {
                sms.ButtonSetup = DialogButton.Ok; sms.Caption = "Message"; sms.Text = String.Format("Insert Catlog Type........"); sms.ShowMessage();
                return false;
            }
        ...
        private void FilterScheduleDataGrid()
        {
            try
            {
                if (ItemsEntity != null && ItemsEntity.Count > 0
                       && ItemsEntity != null && ItemsEntity.Count > 0) //&&  !string.IsNullOrEmpty(FLTR_STR_ITEM_GRID)
                {
                    DataGridViewFilter = CollectionViewSource.GetDefaultView(ItemsEntity);
                    DataGridViewFilter.Filter = adv => (((ADM_M0061_A)adv).item_code ?? "").ToLower().Contains((FLTR_STR_ITEM_GRID ?? "").ToLower()) || (((ADM_M0061_A)adv).item_name ?? "").ToLower().Contains((FLTR_STR_ITEM_GRID ?? "").ToLower());
