...
{
    public class SDM_T003_VM : WorkspaceViewModel<SEL_T001>
    {
        #region AutoSuggest TextBox Region
        public static readonly DependencyProperty AutoSuggestTextViewModelProperty = DependencyProperty.Register("AutoSuggestTextViewModel", typeof(AutoSuggestTextViewModel<dynamic>), typeof(SDM_T003_VM));
        ...
        #region Constructor
        public SDM_T003_VM(string doc_cat, string ts_code) : base()
        {
        ...
        public SDM_T003_VM(string doc_cat, string ts_code, string doc_no) : base()
        {
            this.ts_code_vm = ts_code;
        ...
        private async void InsertSoldToParty(object InputValue, bool OverrideValue)
        {
            try
            {
                ...
                if (POPUPEntityObject != null) //&& MasterEntity.PartyId != POPUPEntityObject.PartyId) //Application:New Record Only.Condition: Only enter in the code block if EntityObject is Not null and not assigning same value again if event fire twice.
                {
                    ...
                    else if (isNewRecord == true && ItemsEntity.Count > 0 && MasterEntity.PartyId != POPUPEntityObject.PartyId)
                    {
                        ...
                        if (showMessageService.ShowMessage() == DialogResult.Ok)
                        {
                            ...
                            MCTemp = await repository_MCTemp.GetDataWithReturnDomainObjectASynchronus<MultipleContext_SEL_T001>(MCTemp, RequestParameterData, "SEL_T001_BL", "SDM", "", 0, "");
                    ...
                    else if (MasterEntity.PartyId != POPUPEntityObject.PartyId || MasterEntity.party_name != POPUPEntityObject.PartyNm)
                    {
                        ...
                        RequestParameterData = "LoadSoldToPartyDetails" + "!@" + AppSessionState.client + "!@" + MasterEntity.comp_code + "!@" + MasterEntity.location_Id + "!@" + doc_cat_vm + "!@" + MasterEntity.doc_type + "!@!@" + AppSessionState.UserID + "!@" + AppSessionState.EmpId + "!@" + ts_code_vm + "!@" + AppSessionState.so_code + "!@" + (AppSessionState.sg_code ?? "") + "!@" + MasterEntity.PartyId;
                        // RequestParameterData = "LoadSoldToPartyDetails" + "!@" + AppSessionState.OBJ_COMPANY.comp_code + "!@" + AppSessionState.OBJ_LOCATION.location_id + "!@" + MasterEntity.doc_type + "!@" + MasterEntity.doc_cat + "!@" + MasterEntity.PartyId + "!@" + AppSessionState.client;
                        //MCTemp = repository_MCTemp.GetDataWithReturnDomainObject<MultipleContext_SEL_T001>(MCTemp, RequestParameterData, "SEL_T001_BL", "SDM", "", 0, "");
                        MCTemp = await repository_MCTemp.GetDataWithReturnDomainObjectASynchronus<MultipleContext_SEL_T001>(MCTemp, RequestParameterData, "SEL_T001_BL", "SDM", "", 0, "");
                ...
                var msg = new NotificationMessage("SDM_T003_VM");
            ...
            try
            {
                ...
                if (POPUPEntityObject != null) //&& MasterEntity.PartyId != POPUPEntityObject.PartyId) //Application:New Record Only.Condition: Only enter in the code block if EntityObject is Not null and not assigning same value again if event fire twice.
                {
                    ...
                    if (MasterEntity.PartyId != MasterEntity.ship_to_party)
                    {
                        ...
                        // RequestParameterData = "LoadShipToPartyDetails" + "!@" + AppSessionState.OBJ_COMPANY.comp_code + "!@" + AppSessionState.OBJ_LOCATION.location_id + "!@" + MasterEntity.doc_type + "!@" + MasterEntity.doc_cat + "!@" + MasterEntity.ship_to_party + "!@" + AppSessionState.client;
                        MCTemp = repository_MCTemp.GetDataWithReturnDomainObject<MultipleContext_SEL_T001>(MCTemp, RequestParameterData, "SEL_T001_BL", "SDM", "", 0, "");
                ...
                if (POPUPEntityObject != null) // Only enter in the code block if ENtity Not null.
                {
                    ...
                    else if (dgSelectedIndexItem >= 0 && ItemsEntity.Count > dgSelectedIndexItem) //Update Row: If Old or Blank row get added, then Edit Rule will applicable in this Loop.
                    {
                        if (ItemsEntity[dgSelectedIndexItem].id == 0 && ((IndexOfExistValue == -1) || (IndexOfExistValue != -1 && AllowDuplicate == true))) // It will allowe to add unique value or Duplicate value if AllowDuplicate Status == True
                        {
                            ...
                            ItemsEntity[dgSelectedIndexItem].t_status = "01";//"Draft";
        ...
        private void LoadBackFlipData(object InputValue)
        {
            try
            {
                ...
                string Request = "LoadBackFlipData" + "!@" + AppSessionState.client + "!@" + MasterEntity.comp_code + "!@" + (RequestPara.location_Id ?? AppSessionState.OBJ_LOCATION.location_id) + "!@" + doc_cat_vm + "!@" + (RequestPara.doc_type ?? doc_cat_vm) + "!@!@" + AppSessionState.UserID + "!@" + (RequestPara.EmpId ?? AppSessionState.EmpId) + "!@" + ts_code_vm + "!@" + AppSessionState.so_code + "!@" + (AppSessionState.sg_code ?? "") + "!@" + (Utilities.NullIf(RequestPara.PartyId) ?? "") + "!@!@" + (RequestPara.active.ToString() ?? "") + "!@" + (Utilities.NullIf(RequestPara.t_status) ?? "") + "!@" + Convert.ToDateTime(RequestPara.FromDate).ToString("MM/dd/yyyy") + "!@" + Convert.ToDateTime(RequestPara.ToDate).ToString("MM/dd/yyyy");
        ...
        private void LoadDocumentByReferenceDocumentNumber(object ParameterObject, string ParameterReference)
        {
            try
            {
                ...
                if (ParameterObject.GetType() == typeof(string) && ParameterObject != null) // This Block of code read parameter . First for string and Entity Object in else part.
                {
                    ...
                    {
                        try
                        { Request = "LoadDocumentWithReferenceDocumentNumberQT" + "!@" + AppSessionState.client + "!@" + AppSessionState.OBJ_COMPANY.comp_code + "!@!@!@!@" + ParametersStringValue; }
                        catch (Exception ex) { }
                    }
                }
                MCTemp = repository_MCTemp.GetDataWithReturnDomainObject<MultipleContext_SEL_T001>(MCTemp, Request, "SEL_T001_BL", "SDM", "LoadDocumentWithReferenceDocumentNumberQT", 0, "");
                if (MCTemp.ItemsEntity != null)
                {
                    ItemsEntity.Clear();
                ...
                if (ParameterReference == "ReferenceDocument")
                {
                    ...
                    MasterEntity.t_status = "01";//"Draft";
                    MasterEntity.shipped = false;
                    MasterEntity.version = "v1.0";
                    ScheduleEntity.t_status = "01"; //"Draft";
        ...
        protected override void OnPrintAction(InquiryActionResult<SEL_T001> result)
        {
            ...
            try
            {
                ...
                MCTemp = repository_MC.GetDataWithReturnDomainObject<MultipleContext_SEL_T001>(MC, Request, "SEL_T001_BL", "SDM", "LoadAll", 0, "");
                ...
                string ReportName = "";
                var SystemDocumentObject = (from o in MC.DOC_TYPE_LIST where o.doc_cat == MasterEntity.doc_cat select o).ToList();
                if (PrePrinted == true)
                {
        ...
        private void LoadInitialData()
        {
            ...
            try
            {
                ...
                ASSoldToParty = new AutoSuggestTextViewModel<dynamic>(MC.PartyMaster, TheFilter, SuggestedValue, "party_name", true);
                ASSoldToParty.AutoSuggestVM.IsEmptyValueAllowed = true;
                ASSoldToParty.AutoSuggestVM.IsFreeTextAllowed = true;

                SuggestedValue = new ValueConverter(x => x == null ? "" : ((ADM_M028_P)x).PartyNm);
                TheFilter = (o, prefix) => ((ADM_M028_P)o).PartyNm.ToString().ToLower().Contains(prefix.ToLower()) || ((ADM_M028_P)o).PartyId.ToString().ToLower().Contains(prefix.ToLower());
                ASShipToParty = new AutoSuggestTextViewModel<dynamic>(MC.PartyMaster, TheFilter, SuggestedValue, "PartyNm", true);
                ...
                var LocFilterList = (from o in LocationList where o.comp_code == AppSessionState.OBJ_COMPANY.comp_code select o).ToList();
                SuggestedValue = new ValueConverter(x => x == null ? "" : ((ADM_M003)x).location_Id);
                TheFilter = (o, prefix) => (((ADM_M003)o).location_Id ?? "").ToString().ToLower().Contains(prefix.ToLower()) || (((ADM_M003)o).LoctnNm ?? "").ToString().ToLower().Contains(prefix.ToLower());
                ...
                Doc_TypeCollectionBf = new CollectionViewSource { Source = MC.DOC_TYPE_LIST }.View;
                Doc_TypeCollectionBf.Filter = new Predicate<object>(Filter_DocTypeBf);
                StringListDocumentTypesBf = MC.DOC_TYPE_LIST.Select(x => x.doc_type).ToList();
