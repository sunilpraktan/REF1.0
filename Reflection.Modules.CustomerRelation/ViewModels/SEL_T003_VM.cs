...
{
    public class SEL_T003_VM : WorkspaceViewModel<SEL_T003>
    {
        #region AutoSuggest TextBox Region
        public static readonly DependencyProperty AutoSuggestTextViewModelProperty = DependencyProperty.Register("AutoSuggestTextViewModel", typeof(AutoSuggestTextViewModel<dynamic>), typeof(SEL_T003_VM));
        ...
        #region Constructor 
        public SEL_T003_VM(string doc_cat, string ts_code) : base()
        {
        ...
        public SEL_T003_VM(string doc_cat, string ts_code, string doc_no) : base()
        {
            this.ts_code_vm = ts_code;
            this.doc_cat_vm = doc_cat;
        ...
        private void LoadInitialData()
        {
            try
            {
                ...
                //string Request = "LoadInitialData" + "!@" + AppSessionState.comp_code + "!@" + AppSessionState.location_Id + "!@" + MasterEntity.doc_cat + "!@" + MasterEntity.doc_cat + "!@" + "PI,CI,SI" + "!@" + AppSessionState.so_code + "!@" + AppSessionState.sg_code + "!@" + AppSessionState.EmpId + "!@" + AppSessionState.client + "!@" + AppSessionState.UserID + "!@" + this.ts_code_vm;
        ...
        private void OpenDocumentViewer(object InputValue)
        {
            try
            {
                ...
                if (EntityObjectParameter.id != 0)
                {
                    ...
                    {
                        Messenger.Default.Send<DocumentViewerPayload>(new DocumentViewerPayload() { DocumentNumber = MasterEntity.bill_doc.Replace("/", "--"), Row_ID = EntityObjectParameter.id.ToString(), DocumentList = MCAttachments.Attachments, client = AppSessionState.client, comp_code = (MasterEntity.comp_code ?? AppSessionState.comp_code) });
                    }
                }
            }
            catch (Exception ex)
        ...
        private void PrintExportReport(object input)
        {
            ...
            try
            {
                ...
                else if (input.ToString() == "Custom Invoice")
                {
                    ...
                    MCTemp = repository_MC.GetDataWithReturnDomainObject<MultipleContext_SEL_T003>(MC, Request, "SalesInvoice", "CRM", "LoadAll", 0, Request);
                }
            ...
            try
            {
                if (!string.IsNullOrWhiteSpace(MasterEntity.bill_doc))
                {
                    if (MasterEntity.t_status == "017")
                    {
                        ...
                        {
                            if (MCTemp.MasterEntity[0].t_status == "002")
                            {
            ...
            try
            {
                ...
                if (ParameterObject != null)
                {
                        ...
                        if (ParametersStringValue.Length > 0)
                        {
                            try
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        Request = "LoadDocumentFromSalesOrderDeliverySchedule" + "!@" + AppSessionState.client + "!@" + MasterEntity.comp_code + "!@" + MasterEntity.location_Id + "!@" + this.doc_cat_vm + "!@" + this.doc_cat_vm + "!@" + DocumentList + "!@" + AppSessionState.UserID + "!@!@!@" + MasterEntity.ref_doc_cat;
                    ...
                    else if (ParameterObject != null)
                    {
                        ...
                        {
                            Request = "LoadDocumentWithDocumentNumber" + "!@" + AppSessionState.client + "!@" + AppSessionState.comp_code + "!@" + MasterEntity.location_Id + "!@" + this.doc_cat_vm + "!@" + MasterEntity.doc_type + "!@" + ParameterObject.ToString();
                        ...
                        else if (((IEnumerable)ParameterObject).Cast<SEL_T003Flip>().ToList().Count > 0)
                        {
                            ...
                            Request = "LoadDocumentWithDocumentNumber" + "!@" + AppSessionState.client + "!@" + AppSessionState.comp_code + "!@" + MasterEntity.location_Id + "!@" + MasterEntity.doc_cat + "!@" + MasterEntity.doc_type + "!@" + ParameterEntityObject.bill_doc + "!@!@!@!@!@!@" + ParameterEntityObject.PartyId;
        ...
        private void Export_To_JSON_FILE()
        {
            try
            {
                ...
                string Request = "JSON_EXPORT" + "!@" + AppSessionState.client + "!@" + AppSessionState.comp_code + "!@" + AppSessionState.location_Id + "!@" + MasterEntity.doc_cat + "!@" + MasterEntity.doc_type + "!@" + MasterEntity.bill_doc + "!@" + AppSessionState.UserID + "!@" + AppSessionState.EmpId + "!@" + this.ts_code_vm + "!@" + AppSessionState.so_code + "!@" + AppSessionState.sg_code;
                MCTemp = repository_MCTemp.GetDataWithReturnDomainObject<MultipleContext_SEL_T003>(MCTemp, Request, "SEL_T003_BL", "FICO", "LoadAll", 0, Request);
                ...
                System.IO.File.WriteAllText(@"C:\eWayBills\" + MasterEntity.bill_doc.Replace("/", "") + ".json", jsonObject);
                    ...
                    if (ItemsEntity != null && ItemsEntity.Count > 0 && dgSelectedIndexItem >= 0 && dgSelectedIndexItem < ItemsEntity.Count) //Condition satisfy only if Items collection is not empty.
                    {
                        ...
                        if (ItemsEntity[dgSelectedIndexItem].tax_id != null && ItemsEntity[dgSelectedIndexItem].active == true) // Remove previously assign Taxes from TotalDocumentTaxes to apply new for perticuler item row.//Condition satisfy only if Selected Item not null  and Active status is true.
                        {
                            if (!String.IsNullOrEmpty(ItemsEntity[dgSelectedIndexItem].tax_id.Trim()) && ItemsEntity[dgSelectedIndexItem].active != false) //Condition satisfy only if Selected Item having Tax assigned.
                            {
                                ...
                                foreach (var tax in copy)
                                {
                                    if (tax.ItemCode == ItemsEntity[dgSelectedIndexItem].ItemCode && (tax.sku?.ToString() ?? "") == (ItemsEntity[dgSelectedIndexItem].sku?.ToString() ?? "") && tax.item_row_id == ItemsEntity[dgSelectedIndexItem].id)
                                    {
                                        TotalDocumentTaxes.Remove(tax);
                                    }
                                }
                    ...
                    if (ItemsEntity != null && ItemsEntity.Count > 0 && RowIndex >= 0 && RowIndex < ItemsEntity.Count) //Condition satisfy only if Items collection is not empty.
                    {
                        ...
                        if (ItemsEntity[RowIndex].tax_id != null && ItemsEntity[RowIndex].active == true) // Remove previously assign Taxes from TotalDocumentTaxes to apply new for perticuler item row.//Condition satisfy only if Selected Item not null  and Active status is true.
                        {
                            if (!String.IsNullOrEmpty(ItemsEntity[RowIndex].tax_id.Trim()) && ItemsEntity[RowIndex].active != false) //Condition satisfy only if Selected Item having Tax assigned.
                            {
                                ...
                                foreach (var tax in copy)
                                {
                                    if (tax.ItemCode == ItemsEntity[RowIndex].ItemCode && (tax.sku?.ToString() ?? "") == (ItemsEntity[RowIndex].sku?.ToString() ?? "") && tax.item_row_id == ItemsEntity[dgSelectedIndexItem].id)
                                    {
                                        TotalDocumentTaxes.Remove(tax);
                                    }
                                }
            ...
            try
            {
                ...
                foreach (NotificationData VarData in objNotifyData)
                {
                    List<KeyValuePair<string, string>> kvpList = new List<KeyValuePair<string, string>>()
                    {
                        ...
                        new KeyValuePair<string, string>("[CUST]","M/s: " +  MasterEntity.sold_to_party_name),
                        ...
                        new KeyValuePair<string, string>("[PREF]", (MasterEntity.cust_ref ?? "").ToString() + " Dated: " + (MasterEntity.ref_date?.ToShortDateString())), //MasterEntity.cust_ref_date.HasValue ? MasterEntity.cust_ref_date.Value.ToString() : string.Empty;
            ...
            try
            {
                if (MasterEntity.bill_doc != null && MasterEntity.bill_doc != "")
                {
                    if (MasterEntity.t_status != "017")
                    {
                        ...
                        //string Request = "ValidateInvoice" + "!@" + AppSessionState.comp_code + "!@" + AppSessionState.location_Id + "!@" + AppSessionState.client + "!@" + MasterEntity.bill_doc + "!@" + AppSessionState.UserID + "!@" + this.ts_code_vm;
                        MCTemp = repository_MC.GetDataWithReturnDomainObject<MultipleContext_SEL_T003>(MCTemp, Request, "SalesInvoice", "CRM", "LoadAll", 0, "");
        ...
        protected override void OnRefreshCommand(InquiryActionResult<SEL_T003> result)
        {
            ...
            //string Request = "Refresh" + "!@" + AppSessionState.comp_code + "!@" + AppSessionState.location_Id.ToString() + "!@" + MasterEntity.doc_type + "!@" + MasterEntity.doc_cat + "!@" + "PI,CI,SI" + "!@" + AppSessionState.so_code + "!@" + AppSessionState.sg_code + "!@" + AppSessionState.EmpId + "!@" + AppSessionState.client + "!@" + AppSessionState.UserID + "!@" + this.ts_code_vm;
            ...
            var refdoctempa1 = (from o in MC.Sales_Invoice_Reference where o.doc_cat == "SO" || o.doc_cat == "DN" || o.doc_cat == "OP" || o.doc_cat == "PI" || o.doc_cat == "CI" || o.doc_cat == "DC" select o).ToList();
        ...
        protected override void OnPrintAction(InquiryActionResult<SEL_T003> result)
        {
            try
            {
                ...
                var SystemDocumentObject = (from o in MC.doc_typeList where o.doc_cat == MasterEntity.doc_cat && o.doc_type == MasterEntity.doc_type select o).ToList();
        ...
        private void MailDocuments(object InputValue)
        {
            try
            {
                ...
                {
                    string Request = "SI_Report" + "!@" + AppSessionState.client + "!@" + MasterEntity.comp_code + "!@" + MasterEntity.location_Id + "!@" + this.doc_cat_vm + "!@" + MasterEntity.doc_type + MasterEntity.bill_doc;
                    //string Request = "SI_Report" + "!@" + MasterEntity.bill_doc + "!@" + AppSessionState.client + "!@" + MasterEntity.comp_code + "!@" + MasterEntity.location_Id + "!@" + MasterEntity.doc_cat + "!@" + MasterEntity.doc_type + "!@" + AppSessionState.UserID + "!@" + this.ts_code_vm;
