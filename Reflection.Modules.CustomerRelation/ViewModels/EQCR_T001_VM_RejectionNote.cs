...
{
    public class EQCR_T001_VM_RejectionNote : WorkspaceViewModel<EQCR_T001_A>
    {
        bool blNew = true;
        ...
        public EQCR_T001_VM_RejectionNote(string ts_code)
            : base()
        {
        ...
        protected override void OnPrintAction(InquiryActionResult<EQCR_T001_A> result)
        {
             try
            {
                if (SelectedEQCR_T001.id > 0)
                {
                    ...
                    MC_EQCR_T001_A = repositoryM.GetDataWithReturnDomainObject<MultipleContext_EQCR>(MC_EQCR_T001_A, "EQCR_T001_A_Data", "QCR", "QMS", "RN_Report", SelectedEQCR_T001.id, "");                    
                    ...
                    ReportManager.DisplayReport(objDS, obj_DS, "\\QMS\\QCR.rdlc", "QCR");
                }
            }
            catch
            {
