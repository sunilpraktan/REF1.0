...
{
    public class SalesOrderEssemVM1 : WorkspaceViewModel<SEL_T001>
    {
        #region AutoSuggest TextBox Region
        public static readonly DependencyProperty AutoSuggestTextViewModelProperty = DependencyProperty.Register("AutoSuggestTextViewModel", typeof(AutoSuggestTextViewModel<dynamic>), typeof(SalesOrderEssemVM1));
        ...
        private AutoSuggestTextViewModel<dynamic> _ASUOM_WT { get; set; }
        public AutoSuggestTextViewModel<dynamic> ASUOM_WT
        {
            get { return _ASUOM_WT; }
            set
            {
                if (_ASUOM_WT != value)
                {
                    _ASUOM_WT = value; RaisePropertyChanged("ASUOM_WT");
                }
            }
        }
        ...
        private AutoSuggestTextViewModel<dynamic> _ASPacking { get; set; }
        public AutoSuggestTextViewModel<dynamic> ASPacking
        {
            get { return _ASPacking; }
            set
            {
                if (_ASPacking != value)
                {
                    _ASPacking = value; RaisePropertyChanged("ASPacking");
                }
            }
        }
        ...
        private void SetAutoTextSource(DataGridCellInfo dgCellInfo)
        {
            if (dgCellInfo != null)
            {
                ...
                if (column != null)
                {
                    ...
                    { ASDefault1 = ASTR_MODE; }
                    else if (SourceName == "pkgunit")
                    { ASDefault = ASPacking; }
        ...
        public int dgSelectedIndexItem
        {
            ...
            set
            {
                if (_dgSelectedIndexItem != value)
                {
                    ...
                    FilterScheduleShipToAdd();
                    if (TotalDocumentTaxesItem.Count > 0 && ItemsEntity.Count > dgSelectedIndexItem && dgSelectedIndexItem != -1)
                    {
        ...
        public GalaSoft.MvvmLight.Command.RelayCommand cmdLoadInfoReport { get; private set; }
        ...
        public RelayCommand<object> cmdPlant2 { get; private set; }
        public RelayCommand<object> cmdWt_unit { get; private set; }
        ...
        /// </summary>
        /// <param name="NA"></param>
        public SalesOrderEssemVM1(string doc_cat, string ts_code) : base()
        {
        ...
        public SalesOrderEssemVM1(string doc_cat, string ts_code, string doc_no) : base()
        {
            CursorControl.SetBusyState();
            this.doc_cat_vm = doc_cat;
            this.ts_code_vm = ts_code;
        ...
        private void FilterScheduleShipToAdd()
        {
            try
            {
                if (ItemScheduleEntity != null && ItemScheduleEntity.Count > 0 && ItemsEntity != null && ItemsEntity.Count > 0 && dgSelectedIndexItem >= 0 && MC.PartysShipToAddresses != null
                    && ItemEntityObject != null)
                {
                    ...
                    {
                        var tempAdd = (from o in MC.PartysShipToAddresses where o.PartyId == ItemEntityObject.ship_to_Party select o);
        ...
        private void MailDocuments(object InputValue)
        {
            ...
            try
            {
                ...
                {
                    //string Request = "SO_Report" + "!@" + MasterEntity.sono + "!@" + AppSessionState.client + "!@" + MasterEntity.comp_code + "!@" + MasterEntity.location_Id + "!@" + MasterEntity.doc_cat + "!@" + MasterEntity.doc_type + "!@" + MasterEntity.EmpId + "!@" + AppSessionState.UserID + "!@" + MasterEntity.ts_code; 
        ...
        private void InsertWt_unit(object InputValue)
        {
            try
            {
                string Request = "";
        ...
        private void PrintProformaInvoiceReport(object InputValue)
        {
            try
            {
                ...
                objDataSourceName[0] = "dsSDMaster";
                objDataSourceName[1] = "dsSDItem";
                objDataSourceName[2] = "dsSDTax";
                objDataSourceName[3] = "dsCompany";
                objDataSourceName[4] = "dsLocation";
                objDataSourceName[5] = "dsSDSchedule";
                objDataSourceName[6] = "dsSDTC";


                ReportManager ReportManager = new ReportManager();
                ReportManager.DisplayReport(objDataSource, objDataSourceName, "\\CRM\\SalesOrderPI.rdlc", getParametersList(), "SalesOrder2");
        ...
        private void LoadInfoReport()
        {
            try
            {
                ...
                string ReportName = "SalesInfoReport.rdlc";
                ...
                var SalesUnitPriceList = SalesUnitPriceCollection.ToList();
                var SalesQFRList = SalesQFRCollection.ToList();
                var SalesDispatchList = SalesDispatchCollection.ToList();
                var SalesProjDispatchList = SalesProjDispatchCollection.ToList();

                objDataSource[0] = SalesUnitPriceList;
                objDataSource[1] = SalesQFRList;
                objDataSource[2] = SalesDispatchList;
                objDataSource[3] = SalesProjDispatchList;

                List<ADM_M002> TempCmpList = (List<ADM_M002>)AppSessionState.ADM_M002_List;
                var CmpResult = TempCmpList.Where(Cmp => Cmp.comp_code == MasterEntity.comp_code).ToList();
                objDataSource[4] = CmpResult;

                List<ADM_M003> TempList = (List<ADM_M003>)AppSessionState.ADM_M003_List;
                var Result = TempList.Where(loc => loc.location_Id == MasterEntity.location_Id).ToList();
                objDataSource[5] = Result;



                objDataSourceName[0] = "dsSalesUnitPriceCollection";
                objDataSourceName[1] = "dsSalesQFRCollection";
                objDataSourceName[2] = "dsSalesDispatchCollection";
                objDataSourceName[3] = "dsSalesProjDispatchCollection";
                objDataSourceName[4] = "dsCompany";
                objDataSourceName[5] = "dsLocation";
        ...
        private void Insertplant2(object InputValue)
        {
            try
            {
                string Request = "";
                ...
                if (POPUPEntityObject != null) //&& MasterEntity.PartyId != POPUPEntityObject.PartyId) //Application:New Record Only.Condition: Only enter in the code block if EntityObject is Not null and not assigning same value again if event fire twice.
                {
                    if (String.IsNullOrEmpty(MasterEntity.sono) != true || String.IsNullOrWhiteSpace(MasterEntity.sono) != true) //Condition: Only enter in the code block if ENtity Not null. Means It is in Edit Mode.
                    {
                        ...
                        else
                        {
                            ...
                            MasterEntity.ship_mark = POPUPEntityObject.abbr;
                    ...
                    else
                    {
                        ...
                        RequestParameterData = "LoadSoldToPartyDetails" + "!@" + AppSessionState.client + "!@" + MasterEntity.comp_code + "!@" + MasterEntity.location_Id + "!@" + doc_cat_vm + "!@" + MasterEntity.doc_type + "!@!@" + AppSessionState.UserID + "!@" + AppSessionState.EmpId + "!@" + ts_code_vm + "!@" + AppSessionState.so_code + "!@" + (AppSessionState.sg_code ?? "") + "!@" + MasterEntity.PartyId;
                        //RequestParameterData = "LoadSoldToPartyDetails" + "!@" + AppSessionState.comp_code + "!@" + AppSessionState.location_Id + "!@" + MasterEntity.doc_type + "!@" + MasterEntity.doc_cat + "!@" + MasterEntity.PartyId + "!@" + AppSessionState.client + "!@" + MasterEntity.EmpId + "!@" + AppSessionState.UserID + "!@" + MasterEntity.ts_code; ;
                        MCTempp = repository_MCTemp.GetDataWithReturnDomainObject<MultipleContext_SEL_T001>(MCTempp, RequestParameterData, "SalesOrderMaster", "CRM", "", 0, "");
                    ...
                    if (string.IsNullOrEmpty(MasterEntity.buss_place))
                    {
                        tempTax = SelectedTaxList.Where(T => T.base_code_id == 1 || T.base_code_id == 2 || T.base_code_id == 3 || T.base_code_id == null).ToList();
                        TaxDictoneryParent = tempTax.ToDictionary(X => X.id.ToString(), X => (object)X.description);
                    }
                ...
                if (POPUPEntityObject != null) //&& MasterEntity.PartyId != POPUPEntityObject.PartyId) //Application:New Record Only.Condition: Only enter in the code block if EntityObject is Not null and not assigning same value again if event fire twice.
                {
                    ...
                    if (MasterEntity.PartyId != MasterEntity.ship_to_party)
                    {
                        ...
                        if (MC.PartysShipToAddresses.Count == 1)
                        {
                            ...
                            MasterEntity.final_dest = MC.PartysShipToAddresses[0].CntryName;
                        }
                        else if (MC.PartysShipToAddresses.Count > 1)
                        {
                            ...
                            else if (DeliveryAddress.Count == 1)
                            {
                                ...
                                MasterEntity.final_dest = DeliveryAddress[0].CntryName;
                            }
                            else if (DeliveryAddress.Count > 1)
                            {
                                MasterEntity.del_address = 0;
                    ...
                    else if (MasterEntity.PartyId == POPUPEntityObject.PartyId)
                    {
                        ...
                        if (DeliveryAddress.Count == 0)
                        {
                            ...
                            MasterEntity.final_dest = MasterEntity.final_dest;
        ...
        private void InsertdgTransporter(object InputValue, bool NewRow, bool AllowDuplicate, bool AllowModify)
        {
            try
            {
                ...
                if (POPUPEntityObject != null && ItemEntityObject != null)
                {
                    if (dgSelectedIndexItemSchedule >= 0 && ItemScheduleEntity.Count > dgSelectedIndexItemSchedule && ItemEntityObject != null) //Update Row: If Old or Blank row get added, then Edit Rule will applicable in this Loop.
                    {
                        List<SEL_T002_A> temp = ItemScheduleEntity.ToList();
        ...
        private void InsertInk(object InputValue) // Para1
        {
            try
            {
                ...
                if (POPUPEntityObject != null && ItemEntityObject != null)
                {
                    ItemEntityObject.para1 = POPUPEntityObject.ink;
                }
            }
            catch (Exception ex)
            {
        ...
        private void InsertBallType(object InputValue) // Para5
        {
            try
            {
                ...
                if (POPUPEntityObject != null)
                {
                    ItemEntityObject.para5 = POPUPEntityObject.ball_type;
                }
            }
            catch (Exception ex)
            {
        ...
        private void InsertILD(object InputValue) // Para6
        {
            try
            {
                ...
                if (POPUPEntityObject != null && ItemEntityObject != null)
                {
                    ItemEntityObject.para5 = POPUPEntityObject.ild;
                }
            }
            catch (Exception ex)
            {
        ...
        private void InsertWireType(object InputValue) // Para7
        {
            try
            {
                ...
                if (POPUPEntityObject != null && ItemEntityObject != null)
                {
                    ItemEntityObject.para7 = POPUPEntityObject.wire_type;
                }
            }
            catch (Exception ex)
            {
        ...
        private void InsertBallDia(object InputValue) // Para9
        {
            try
            {
                ...
                if (POPUPEntityObject != null && ItemEntityObject != null)
                {
                    ItemEntityObject.para9 = POPUPEntityObject.Ball_dia.ToString();
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                try
                {
                    if (InputValue.GetType() == typeof(string) && InputValue != null)
                    {
                        ...
                        {
                            try
                            { POPUPEntityObject = MC.PartysContactInfo.Where(x => x.PersonName.ToString().Equals(Request, StringComparison.OrdinalIgnoreCase) == true).ToList()[0]; }
                            catch (Exception ex) { }
                        }
                    }
                    else if (InputValue != null)
            ...
            try
            {
                ...
                if (POPUPEntityObject != null) // Only enter in the code block if ENtity Not null.
                {
                    ...
                    if (NewRow == true && (AllowDuplicate == true || IndexOfExistValue == -1) && ItemsEntity.Count == dgSelectedIndexItem)
                    {
                        ...
                        if (dgSelectedIndexItem >= 0 && dgSelectedIndexItem <= ItemsEntity.Count && ItemEntityObject != null)
                        {
                    ...
                    else if (dgSelectedIndexItem >= 0 && ItemsEntity.Count > dgSelectedIndexItem && ItemEntityObject != null) //Update Row: If Old or Blank row get added, then Edit Rule will applicable in this Loop.
                    {
                        if (ItemEntityObject.id == 0 && ((IndexOfExistValue == -1) || (IndexOfExistValue != -1 && AllowDuplicate == true))) // It will allowe to add unique value or Duplicate value if AllowDuplicate Status == True
                        {
                            ...
                            ItemEntityObject.para6 = POPUPEntityObject.tipshape;
                            ItemEntityObject.para3 = "A";
                            ...
                            ItemEntityObject.para18 = POPUPEntityObject.wire_type;
                            ItemEntityObject.para19 = POPUPEntityObject.wire_size.ToString();
                            ItemEntityObject.para20 = POPUPEntityObject.ball_dia.ToString();
                            ItemEntityObject.para2 = POPUPEntityObject.ball_type;
                            ItemEntityObject.para8 = Convert.ToDecimal(POPUPEntityObject.total_len);
        ...
        private void InsertPacking(object InputValue, bool NewRow, bool AllowDuplicate, bool AllowModify)
        {
            try
            {
                string Request = "";
                ...
                if (InputValue.GetType() == typeof(string) && InputValue != null)
                {
                    ...
                    {
                        try
                        { POPUPEntityObject = MC.PackingList.Where(x => x.id.ToString().Equals(Request, StringComparison.OrdinalIgnoreCase) == true).ToList()[0]; }
                        catch (Exception ex) { }
                    }
                }
                else if (InputValue != null)
                ...
                if (POPUPEntityObject != null && ItemEntityObject != null) // Only enter in the code block if ENtity Not null.
                {
                    var InputValueIfExists = ItemsEntity.Where(X => X.id.ToString() == POPUPEntityObject.id.ToString()).FirstOrDefault(); // Prefer Primary Key for this instruction.
                    int IndexOfExistValue = ItemsEntity.IndexOf(ItemsEntity.Where(X => X.id.ToString() == POPUPEntityObject.id.ToString()).FirstOrDefault()); // Prefer Primary/Unique Key of Row for this instruction.
                    ...
                    if (dgSelectedIndexItem >= 0 && ItemsEntity.Count > dgSelectedIndexItem) //Update Row: If Old or Blank row get added, then Edit Rule will applicable in this Loop.
                    {
                        if (ItemEntityObject.id == 0 && ((IndexOfExistValue == -1) || (IndexOfExistValue != -1 && AllowDuplicate == true))) // It will allowe to add unique value or Duplicate value if AllowDuplicate Status == True, id=0 indicates it will add or edit for New record only.
                        {
                            ...
                            ItemEntityObject.pkgid = POPUPEntityObject.id;
                            ItemEntityObject.pkgunit = POPUPEntityObject.pkgunit;

                        }
                        else if (ItemEntityObject.pkgid != POPUPEntityObject.id)
                        {
            ...
            try
            {
                ...
                if (ParameterObject != null)
                {
                    if (((IEnumerable)ParameterObject).Cast<SEL_T001_Flip>().ToList().Count > 0)
                    {
                        ...
                        //RequestParameterData = "LoadSoldToPartyDetails" + "!@" + AppSessionState.comp_code + "!@" + AppSessionState.location_Id + "!@" + MasterEntity.doc_type + "!@" + MasterEntity.doc_cat + "!@" + ParameterEntityObject.PartyId + "!@" + AppSessionState.client + "!@" + AppSessionState.UserID + "!@" + MasterEntity.EmpId + "!@" + MasterEntity.ts_code; 
                        MCTemp = repository_MCTemp.GetDataWithReturnDomainObject<MultipleContext_SEL_T001>(MCTemp, RequestParameterData, "SalesOrderMaster", "CRM", "", 0, "");
        ...
        private void SetPopupSuggestionDataAfterLoad()
        {
            try
            {
                ...
                if (ASBuyer != null)
                {
                    ...
                    ASDeliveryAdddress.AutoSuggestVM.Suggestion = MC.PartysShipToAddresses.Find(x => x.SrNo.ToString() == MasterEntity.del_address.ToString());
                }
                ...
                var plantList = (from o in PlantList where o.location_Id == AppSessionState.location_Id select o).ToList();
                ASPlant.AutoSuggestVM.Suggestion = plantList.Find(x => x.location_Id == MasterEntity.location_Id);
                ...
                ASt_status.AutoSuggestVM.Suggestion = MC.STATUS_LIST.Find(x => x.t_display == MasterEntity.t_display);
                ...
                //ASFltrt_status.AutoSuggestVM.Suggestion = MC.STATUS_LIST.Find(x => x.t_display == MasterEntity.fltr_t_display);
        ...
        protected override void OnPrintAction(InquiryActionResult<SEL_T001> result)
        {
            ...
            try
            {
                ...
                objDataSource[5] = MCTemp.ScheduleDetailsEntity;
                objDataSource[6] = MCTemp.TermsAndCondition;
                objDataSource[7] = MCTemp.Attachment;
                ...
                if (e.Action == NotifyCollectionChangedAction.Add)
                {
                    try
                    {
                        foreach (SEL_T001_A item in e.NewItems)
                        {
                            ...
                            if (ItemsEntity.Count > dgSelectedIndexItem && dgSelectedIndexItem >= 0 && ItemEntityObject != null)
                            {
                                ItemEntityObject.order_to_plant = MasterEntity.location_Id;
                            }
                            item.symbol = MasterEntity.symbol;
                            item.client = AppSessionState.client;
                            item.PropertyChanged += EntityViewModelPropertyChanged;
                            // FilterItemSalesData(item);
                        }
                        if (ItemsEntity.Count > dgSelectedIndexItem && dgSelectedIndexItem >= 0)
                        {
                            this.ErrorExist = false; /*ItemEntityObject.HasErrors;*/
            ...
            try
            {
                if (EntityChangeEnable == true)
                {
                    ...
                    if (sender.ToString() == "comp_code")
                    {
                        ...
                        PlantList = ((List<ADM_M003>)AppSessionState.ADM_M003_List).Where(item => item.comp_code == MasterEntity.comp_code).ToList();
        ...
        private void LoadInitialData()
        {
            ...
            try
            {
                ...
                //string Request = "LoadInitialData" + "!@" + MasterEntity.comp_code + "!@" + MasterEntity.location_Id.ToString() + "!@" + MasterEntity.doc_type + "!@" + MasterEntity.doc_cat + "!@" + MasterEntity.so_code + "!@" + MasterEntity.sg_code + "!@" + AppSessionState.EmpId + "!@" + MasterEntity.client + "!@" + AppSessionState.UserID + "!@" + MasterEntity.ts_code;
                ...
                TheFilter = (o, prefix) => (((ADM_M024_P)o).EmpId ?? "").ToString().ToLower().Contains(prefix.ToLower()) || (((ADM_M024_P)o).EmpName ?? "").ToString().ToLower().Contains(prefix.ToLower());
                ASSalesPersonView = new AutoSuggestTextViewModel<dynamic>(MC.Sellers, TheFilter, SuggestedValue, "fltr_SalesPersonNM", true);
                ...
                TheFilter = (o, prefix) => (((ZADM_M017_P)o).pkgunit ?? "").ToString().ToLower().Contains(prefix.ToLower());
                ASPacking = new AutoSuggestTextViewModel<dynamic>(MC.PackingList, TheFilter, SuggestedValue, "pkgunit", "pkgunit", true);
                ASPacking.AutoSuggestVM.IsEmptyValueAllowed = true;
                ...
                ASt_status.AutoSuggestVM.IsEmptyValueAllowed = false;
                ...
                SuggestedValue = new ValueConverter(x => x == null ? "" : ((ADM_M038_B_P)x).unit_code);
                TheFilter = (o, prefix) => (((ADM_M038_B_P)o).unit_code ?? "").ToString().ToLower().Contains(prefix.ToLower()) || (((ADM_M038_B_P)o).unit_name ?? "").ToString().ToLower().Contains(prefix.ToLower());
                ASUOM_WT = new AutoSuggestTextViewModel<dynamic>(MC.UOM, TheFilter, SuggestedValue, "unit_code", "unit_code", true);
                ASUOM_WT.AutoSuggestVM.IsEmptyValueAllowed = true;
                ASUOM_WT.AutoSuggestVM.IsFreeTextAllowed = false;
                #endregion
        ...
        private void CommandInitialization()
        {
            ...
            try
            {
                ...
                cmdPacking = new RelayCommand<object>(cmdPara => { if (cmdPara == null) { return; } InsertPacking(cmdPara, false, true, true); });
                ...
                cmdLoadInfoReport = new GalaSoft.MvvmLight.Command.RelayCommand(() => { LoadInfoReport(); });
                ...
                cmdPlant2 = new RelayCommand<object>(items => { if (items == null) { return; } Insertplant2(items); });
                cmdWt_unit = new RelayCommand<object>(cmdPara => { if (cmdPara == null) { return; } InsertWt_unit(cmdPara); });
                cmdPrintReport = new RelayCommand<object>(cmdPara => { if (cmdPara == null) { return; } PrintProformaInvoiceReport(cmdPara); });
                #endregion
            }
            catch (Exception ex)
            {
        ...
        private void OpenDocumentViewer(object InputValue)
        {
            ...
            try
            {
                ...
                string Request = "GetAllFiles" + "!@" + AppSessionState.client + "!@" + (EntityObjectParameter.comp_code ?? AppSessionState.comp_code) + "!@" + (EntityObjectParameter.location_Id ?? AppSessionState.location_Id) + "!@!@!@" + EntityObjectParameter.sono + "!@" + EntityObjectParameter.id.ToString();
                //string Request = "GetAllFiles" + "!@" + EntityObjectParameter.ItemCode + "!@" + AppSessionState.client + "!@" + MasterEntity.comp_code + "!@" + MasterEntity.location_Id + "!@" + MasterEntity.doc_cat + "!@" + MasterEntity.doc_type + "!@" + MasterEntity.EmpId + "!@" + AppSessionState.UserID + "!@" + MasterEntity.ts_code; 
        ...
        private void DefaultValues()
        {
            try
            {
                ...
                MasterEntity.para2 = "From Fresh Production";

                MasterEntity.qty_tol = 5;
                MasterEntity.amt_tol = 5;
            ...
            try
            {
                if (ItemsEntity.Count > 0 && dgSelectedIndexItem >= 0 && dgSelectedIndexItem < ItemsEntity.Count && ItemEntityObject != null)
                {
                    if (ItemEntityObject.qty_price > 0 && ItemEntityObject.price_qty > 0 && string.IsNullOrWhiteSpace(ItemEntityObject.price_qty_uom) == false)
                    {
                        ...
                        ItemEntityObject.price_qty = ItemEntityObject.unit_price;// NOTE: Change for Essem because price cannot edit due to this code block. price_qty column not available for user to edit. remove this line when price_qty column available on screen.
            ...
            try
            {
                ...
                foreach (var o in ItemsEntity)
                {
                    ...
                    if (o.id == 0)
                    {
                        ...
                        {
                            if (o.ItemCode == p.ItemCode && (o.sku?.ToString() ?? "") == (p.sku?.ToString() ?? "") && o.line_id == p.line_id)
                            {
                                flag++;
                            }
                        }
                    ...
                    if (o.ItemCode != null && o.ItemCode != "" && o.Description != null)
                    {
                        ...
                        if ((o.para1 == null || o.para1 == "") && o.StockUnt == true)
                        {
                            IShowMessageViewService showMessageService = this.GetViewService<IShowMessageViewService>();
                            showMessageService.ButtonSetup = DialogButton.Ok;
                            showMessageService.Caption = "Message";
                        ...
                        if ((o.para5 == null || o.para5 == "") && o.StockUnt == true)
                        {
                            IShowMessageViewService showMessageService = this.GetViewService<IShowMessageViewService>();
                            showMessageService.ButtonSetup = DialogButton.Ok;
                            showMessageService.Caption = "Message";
                        ...
                        if ((o.para3 == null || o.para3 == "") && o.StockUnt == true)
                        {
                            IShowMessageViewService showMessageService = this.GetViewService<IShowMessageViewService>();
                            showMessageService.ButtonSetup = DialogButton.Ok;
                            showMessageService.Caption = "Message";
