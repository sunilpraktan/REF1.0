...
{
    class EPR_T003_LocalCarton_VM2 : WorkspaceViewModel<EPR_T003_A>
    {
        ...
        public RelayCommand<IList> cmdMachineOperator { get; private set; }
        //BackSide
        ...
        public EPR_T003_LocalCarton_VM2(string ts_code) : base()
        {
        ...
        public EPR_T003_LocalCarton_VM2(string ts_code,string doc_no) : base()
        {
            this.ts_code_vm = ts_code;
        ...
        private void LoadInitialData()
        {
            try
            {
                ...
                MC = repository_MC.GetDataWithReturnDomainObject<MultipleContext_EPR_T003>(MC, Request, "LocalExportCarton2", "Production", "LoadAll", 0, MasterEntity.doc_type);
                ...
                cmdMachineOperator = new RelayCommand<IList>(items => { if (items == null) { return; } InsertMachineOperator(items); });
                            ...
                            if (POPUPEntityObject != null)
                            {
                                if (POPUPEntityObject.carton_cons_stat != true)
                                {
                                    if (LabelGeneratedEntity.Count == 0)
                                    {
                                        ...
                                        CartonsPacked += 1;
                                        CartonsToBePacked = Convert.ToDecimal(MasterEntity.tot_no_bags) - CartonsPacked;

                                        QtyPacked += Convert.ToDecimal(POPUPEntityObject.label_qty);
        ...
        //    }
        //}     
        private void InsertMachineOperator(IList InputValue)
        {
            try
            {
                IList list = InputValue as IList;
                List<ADM_M024_P> GetSelectedItem = list.Cast<ADM_M024_P>().ToList();

                if (GetSelectedItem.Count > 0)
                {
                    MasterEntity.m_operator = GetSelectedItem[0].EmpId;
                    MasterEntity.OperatorNm = GetSelectedItem[0].EmpName;
                }
            }
            catch (Exception ex)
            {
        ...
        private void LoadLocalExportCartons()
        {
            try
            {
                ...
                + "!@" + Label_used + "!@" + Convert.ToDateTime(BackFlipEntity.FromDate).ToString("MM/dd/yyyy") + "!@" + Convert.ToDateTime(BackFlipEntity.ToDate).ToString("MM/dd/yyyy");

                MCTemp = repository_MC.GetDataWithReturnDomainObject<MultipleContext_EPR_T003>(MCTemp, Request, "LocalExportCarton2", "Production", "LoadAll", 0, BackFlipEntity.doc_type);
            ...
            try
            {
                ...
                if (ParameterObject != null)
                {
                    if (((IEnumerable)ParameterObject).Cast<EPR_T003_A>().ToList().Count > 0)
                    {
                        ...
                        CartonsPacked = Convert.ToDecimal(MasterEntity.tot_no_bags);
        ...
        protected override void OnSaveAction(InquiryActionResult<EPR_T003_A> result)
        {
            try
            {
                if (Validation() == true && isNewRecord == true)
                {
                    ...
                    MasterEntity = repository.SaveWithReturnDomainObject<SEL_T001>(MasterEntity, "LocalExportCarton2", "Production");
                ...
                else if (Validation() == true && isNewRecord == false)
                {
                    ...
                    MasterEntity = repository.UpdateWithReturnDomainObject<EPR_T003_A>(MasterEntity, "LocalExportCarton2", "Production");
        ...
        protected override void OnCreateAction(InquiryActionResult<EPR_T003_A> result)
        {
            try
            {
                ...
                MasterEntity.gross_wt = MC.SettingsList[0].gross_wt;
                var msg = new NotificationMessage("EPR_T003_LocalCarton_VM2");
                Messenger.Default.Send<NotificationMessage>(msg);

                QtyPacked = 0;
                QtyToBePacked = Convert.ToDecimal(MasterEntity.tot_qty);
                CartonsPacked = 0;
                CartonsToBePacked = Convert.ToDecimal(MasterEntity.tot_no_bags);
            }
            catch (Exception ex)
            {
                IShowMessageViewService showMessageService = this.GetViewService<IShowMessageViewService>();
        ...
        protected override void OnPrintAction(InquiryActionResult<EPR_T003_A> result)
        {
            if (MCTemp.MasterEntityList != null && MCTemp.MasterEntityList.Count > 0 && MC.SettingsList.Count > 0)
            {
                if (PrintOption == "All")
                {
                    ...
                    objDataSource[0] = MCTemp.MasterEntityList;
                ...
                else if (PrintOption == "Selected")
                {
                    ...
                    if (count > 0)
                    {
                        ...
                        //List<RptLabelGenList> temp = new List<RptLabelGenList>();
                        //temp = MCTemp.RptLabelGenList.Where(x => x.check == true).ToList();

                        objDataSource[0] = MCTemp.MasterEntityList.Where(x => x.check == true);
                        objDataSourceName[0] = "dsRptLabelGen";
