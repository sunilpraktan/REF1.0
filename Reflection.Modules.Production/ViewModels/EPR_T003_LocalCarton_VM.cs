...
{
    class EPR_T003_LocalCarton_VM : WorkspaceViewModel<EPR_T003_A>
    {
        ...
        private ICollectionView _CustModel;
        public ICollectionView CustModel
        {
            get { return _CustModel; }
            set
            {
                _CustModel = value;
                RaisePropertyChanged("CustModel");
            }
        }
        ...
        #endregion
        #region Constructor
        public EPR_T003_LocalCarton_VM(string ts_code) : base()
        {
        ...
        public EPR_T003_LocalCarton_VM(string ts_code, string doc_no) : base()
        {
            this.ts_code_vm = ts_code;
        ...
        private void LoadInitialData()
        {
            try
            {
                ...
                string Request = "LoadInitialData" + "!@" + AppSessionState.comp_code + "!@" + AppSessionState.location_Id + "!@" + "LC" + "!@" + MasterEntity.doc_cat;
                ...
                //MC = repositoryM.GetData<MultipleContext_EPR_T003>(MC, Request, "LC", "Reflection.BusinessLogic.EPR_T003BL");

                #region Command Initialization
                cmdCheckAllRows = new RelayCommand<object>(items => { if (items == null) { return; } CheckAllRows(items); });
                cmdCartonType = new RelayCommand<object>(items => { if (items == null) { return; } GetDocumentType(items); });
                cmdCustProductName = new RelayCommand<object>(items => { if (items == null) { return; } InsertCustProductName(items); });
                cmdCustModel = new RelayCommand<object>(items => { if (items == null) { return; } InsertCustModel(items); });
                ...
                CustModel = new CollectionViewSource { Source = MC.CustProdNameList }.View;
                CustModel.Filter = new Predicate<object>(Filter_CustModel);
        ...
        private void CheckAllRows(object InputValue) // can be remove this function, we hahe include this code in Function of OK Button below the popup window.
        {
            try
            {
                if (MCTemp.MasterEntityList != null)
                {
        ...
        private void GetDocumentType(object InputValue)
        {
            try
            {
                ...
                if (InputValue != null)
                {
                    if (InputValue.ToString() == "Local")
                    {
                        MasterEntity.doc_type = "LC";
                    }
                    else if (InputValue.ToString() == "Export")
                    {
                        MasterEntity.doc_type = "EC";
                    }
                }
        ...
        private void InsertCustModel(object InputValue)
        {
            try
            {
                ...
                if (POPUPEntityObject != null)
                {
                    MasterEntity.cpn_model_no = POPUPEntityObject.CustomerProductName;
                }
            }
            catch (Exception ex)
            {
                ...
                if (MasterEntity.no_of_carton > 0 && MasterEntity.no_of_carton > SmallCartonEntity.Count && MC.SmallCartonList != null)
                {
                    if (barcode.Length > Scan_Length)
                    {
                        ...
                        if (InputValueIfExists != null)
                        {
                            ...
                            if (POPUPEntityObject != null)
                            {
                                if (POPUPEntityObject.carton_used_Flg == false)
                                {
                                    if (SmallCartonEntity.Count == 0)
                                    {
                                        ...
                                        MasterEntity.cpn_model_no = POPUPEntityObject.cust_prod_code;
                                        MasterEntity.qty_per_bag = POPUPEntityObject.qty_per_bag;
                                        MasterEntity.carton_pack_dt = POPUPEntityObject.carton_pack_dt;
                                        ...
                                        SmallCartonEntity.Add(new EPR_T003_B()
                                        {
                                            ...
                                            qty = Convert.ToDecimal(POPUPEntityObject.tot_qty),    // Convert.ToDecimal(POPUPEntityObject.qty_per_bag * MasterEntity.bags_per_carton),
                                       ...
                                       MasterEntity.grade == POPUPEntityObject.grade) ||
                                       (SmallCartonEntity.Count > 0 && MasterEntity.ItemCode == POPUPEntityObject.ItemCode && MasterEntity.grade == POPUPEntityObject.grade && MasterEntity.ink_id == POPUPEntityObject.ink_id && MasterEntity.grade != "A"))
                                    {
                            ...
                            var msg = new NotificationMessage("EPR_T003_LocalCarton_VM");
                        ...
                        //    showMessageService.ButtonSetup = DialogButton.Ok;
                        //    showMessageService.Caption = "Message";
                        //    showMessageService.Text = String.Format("Invalid Barcode/Batch.\n Please Make Sure That Batch You Are Scanning is Unused and Active.", this.Title);
                        //    showMessageService.ShowMessage();
                        //    if (showMessageService.ShowMessage() == DialogResult.Ok || showMessageService.ShowMessage() == DialogResult.Cancel)
                        //    {
                        //        barcode = "";
        ...
        void ModelUpdated_Calculation(object sender, EventArgs e)
        {
            try
            {
                ...
                else if (sender.ToString() == "tot_no_bags")
                {
                    if (MasterEntity.tot_no_bags != null && MasterEntity.tot_no_bags > 0)
                    {
                        MasterEntity.tot_qty = MasterEntity.tot_no_bags * MasterEntity.qty_per_bag;
                    }
        ...
        private void LoadLocalExportCartons()
        {
            try
            {
                ...
                BackFlipEntity.carton_type = "Local";
                BackFlipEntity.doc_type = "LC";
                ...
                + "!@" + Label_used + "!@" + Convert.ToDateTime(BackFlipEntity.FromDate).ToString("MM/dd/yyyy") + "!@" + Convert.ToDateTime(BackFlipEntity.ToDate).ToString("MM/dd/yyyy") + "!@!@" + "PRODUCTION";
                ...
                if (MCTemp.MasterEntityList.Count > 0)
                {
                    ...
                    FlipDataGridCollection.Filter = new Predicate<object>(Filter_LocalCarton);
                }
        ...
        {   //validation Pending Delete
            ...
            if (MasterEntity.tot_no_bags == null || MasterEntity.tot_no_bags <= 0)
            {
                ...
                showMessageService.ButtonSetup = DialogButton.Ok;
                showMessageService.Caption = "Message";
                showMessageService.Text = String.Format(" Total No of bags Must Be Greater Than 0", this.Title);
                showMessageService.ShowMessage();
                return false;
            }
            ...
            if (MasterEntity.tot_qty == null || MasterEntity.tot_qty <= 0)
            {
                ...
                showMessageService.ButtonSetup = DialogButton.Ok;
                showMessageService.Caption = "Message";
                showMessageService.Text = String.Format(" Total Quantity Must Be Greater Than 0", this.Title);
                showMessageService.ShowMessage();
                return false;
            }
        ...
        private void RemoveRefDoc()
        {
            try
            {
                ...
                MC.LabelGeneratedList.RemoveAll(X => BatchList.Contains(X.batch_no));
            }
            catch (Exception ex)
            {
                IShowMessageViewService showMessageService = this.GetViewService<IShowMessageViewService>();
                ...
                if (MCTemp.MasterEntityList != null && MCTemp.MasterEntityList.Count > 0 && MC.SettingsList.Count > 0)
                {
                    ...
                    else if (PrintOption == "Selected")
                    {
                        ...
                        if (count > 0)
                        {
                            ...
                            List<RptLocalExportCarton> temp = new List<RptLocalExportCarton>();
                            temp = MCTemp.RptLocalExportCartonList.Where(x => x.check == true).ToList();

                            objDataSource[0] = temp;// MCTemp.RptLocalExportCartonList.Where(x => x.check == true);
                            objDataSourceName[0] = "dsRptLocalExportCarton";
        ...
        public string FilterString
        {
            ...
            set
            {
                ...
                FilterCollection_LocalCarton();
            }
        }


        private void FilterCollection_LocalCarton()
        {
        ...
        public bool Filter_LocalCarton(object obj)
        {
            ...
            if (data != null)
            {
                if (!string.IsNullOrEmpty(_FilterString))
                {
                    return (data.doc_no != null && data.doc_no.ToString().ToLower().Contains(_FilterString.ToLower())) ||
                        ...
                        (data.carton_pack_dt != null && data.carton_pack_dt.ToString().ToLower().Contains(_FilterString.ToLower())) ||
                        (data.ItemCode != null && data.ItemCode.ToString().ToLower().Contains(_FilterString.ToLower())) ||
                        (data.ItemName != null && data.ItemName.ToString().ToLower().Contains(_FilterString.ToLower())) ||
                        (data.grade != null && data.grade.ToString().ToLower().Contains(_FilterString.ToLower())) ||
                        ...
                        (data.unit_code != null && data.unit_code.ToString().ToLower().Contains(_FilterString.ToLower())) ||
                        (data.PackingUnit != null && data.PackingUnit.ToString().ToLower().Contains(_FilterString.ToLower())) ||
                        (data.tot_no_bags != null && data.tot_no_bags.ToString().ToLower().Contains(_FilterString.ToLower())) ||
                        (data.no_of_carton != null && data.no_of_carton.ToString().ToLower().Contains(_FilterString.ToLower()));
        ...
        private string _filterString_CustModel;
        public string FilterString_CustModel
        {
            get { return _filterString_CustModel; }
            set
            {
                _filterString_CustModel = value;
                RaisePropertyChanged("FilterString_CustModel");
                FilterCollection_CustModel();
            }
        }
        private void FilterCollection_CustModel()
        {
            if (_CustModel != null)
            {
                _CustModel.Refresh();
        ...
        public bool Filter_CustModel(object obj)
        {
            var data = obj as ZADM_M020_P;
            if (data != null)
            {
                if (!string.IsNullOrEmpty(_filterString_CustModel))
                {
                    return (data.id != null && data.id.ToString().ToLower().Contains(_filterString_CustModel.ToLower())) ||
                        (data.ProductName != null && data.ProductName.ToString().ToLower().Contains(_filterString_CustModel.ToLower())) ||
                         (data.CustomerProductName != null && data.CustomerProductName.ToString().ToLower().Contains(_filterString_CustModel.ToLower()));
