...
{
    public class ENG_T001VM : WorkspaceViewModel<ENG_T001>
    {
        ...
        #region Constructor
        public ENG_T001VM(string ts_code) : base()
        {
        ...
        public ENG_T001VM(string ts_code, string doc_no) : base()
        {
            this.ts_code_vm = ts_code;
        ...
        private void LoadInitialData()
        {
            try
            {
                ...
                SuggestedValue = new ValueConverter(x => x == null ? "" : ((ADM_M022_POPUP)x).item_code);
                TheFilter = (o, prefix) => (((ADM_M022_POPUP)o).item_code ?? "").ToString().ToLower().Contains(prefix.ToLower()) || (((ADM_M022_POPUP)o).item_name ?? "").ToString().ToLower().Contains(prefix.ToLower());
                ASItem = new AutoSuggestTextViewModel<dynamic>(MC.ItemDetails, TheFilter, SuggestedValue, "item_code", true);
                ...
                SuggestedValue = new ValueConverter(x => x == null ? "" : ((ADM_M022_POPUP)x).item_code);
                TheFilter = (o, prefix) => (((ADM_M022_POPUP)o).item_code ?? "").ToString().ToLower().Contains(prefix.ToLower()) || (((ADM_M022_POPUP)o).item_name ?? "").ToString().ToLower().Contains(prefix.ToLower());
                ASGridItem = new AutoSuggestTextViewModel<dynamic>(MC.ItemDetailsForGrid, TheFilter, SuggestedValue, "item_code", "item_code", true);
                ...
                SuggestedValue = new ValueConverter(x => x == null ? "" : ((ADM_M022_POPUP)x).item_code);
                TheFilter = (o, prefix) => ((ADM_M022_POPUP)o).item_code.ToLower().Contains(prefix) || ((ADM_M022_POPUP)o).item_name.ToLower().Contains(prefix);
                ASAltItem = new AutoSuggestTextViewModel<dynamic>(MC.ItemDetailsForGrid, TheFilter, SuggestedValue, "item_code", "item_code", true);
                ...
                ItemCollection.Filter = new Predicate<object>(Filter_item_code);
                StringListItem = MC.ItemDetails.Select(x => x.item_code.ToString()).ToList();
                ...
                StringListAlternateItem = MC.ItemDetailsForGrid.Select(x => x.item_code.ToString()).ToList();
        ...
        private void InsertItem(object InputValue)
        {
            ...
            try
            {
                if (InputValue.GetType() == typeof(string) && InputValue != null)
                {
                    ...
                    {
                        try
                        { POPUPEntityObject = MC.ItemDetails.Where(x => x.item_code.Equals(Request, StringComparison.OrdinalIgnoreCase) == true).ToList()[0]; }
                        catch (Exception ex)
                        {
                        }
                    }
        ...
        private void InsertItemForGrid(object InputValue, bool NewRow, bool AllowDuplicate, bool AllowModify)
        {
            ...
            try
            {
                if (InputValue.GetType() == typeof(string) && InputValue != null)
                {
                    ...
                    {
                        try
                        { POPUPEntityObject = MC.ItemDetailsForGrid.Where(x => x.item_code.Equals(Request, StringComparison.OrdinalIgnoreCase) == true).ToList()[0]; }
                        catch (Exception ex)
                        {
                        }
                    }
            ...
            if (POPUPEntityObject != null)
            {
                var InputValueIfExists = ItemsEntity.Where(x => x.item_code == POPUPEntityObject.item_code).FirstOrDefault();
                int IndexOfExistValue = ItemsEntity.IndexOf(ItemsEntity.Where(X => X.item_code == POPUPEntityObject.item_code).FirstOrDefault());
                //Insert
                if (NewRow == true && (AllowDuplicate == true || IndexOfExistValue == -1) && ItemsEntity.Count == dgSelectedIndexItem)
                {
                    ItemsEntity.Add(new ENG_T001_A()
                    {
                        ...
                        sub_cat = POPUPEntityObject.SubCatCode,
                        ind_sku = POPUPEntityObject.StockUnt,
                        active = "1",
                ...
                else if (dgSelectedIndexItem >= 0 && ItemsEntity.Count > dgSelectedIndexItem)
                {
                    ...
                    {
                        ItemsEntity[dgSelectedIndexItem].item_code = POPUPEntityObject.item_code;
                        ItemsEntity[dgSelectedIndexItem].item_name = POPUPEntityObject.item_name;
                        ItemsEntity[dgSelectedIndexItem].unit_code = POPUPEntityObject.unit_code;
                        ItemsEntity[dgSelectedIndexItem].sub_cat = POPUPEntityObject.SubCatCode;
                        ItemsEntity[dgSelectedIndexItem].ind_sku = POPUPEntityObject.StockUnt;
                        ...
                        ItemsEntity[dgSelectedIndexItem].active = "1";
                    ...
                    else if (ItemsEntity[dgSelectedIndexItem].item_code != POPUPEntityObject.item_code)
                    {
                        ItemsEntity[dgSelectedIndexItem].item_code = "";
                        ItemsEntity[dgSelectedIndexItem].item_name = "";
                    }
                }
        ...
        private void InsertAlternateItem(object InputValue, bool NewRow, bool AllowDuplicate, bool AllowModify)
        {
            ...
            if (POPUPEntityObject != null)
            {
                ...
                if (NewRow == true && (AllowDuplicate == true || IndexOfExistValue == -1) && ItemsEntity.Count == dgSelectedIndexItem)
                {
                    ...
                    {
                        id = 0,
                        item_code_alt = POPUPEntityObject.ItemCode,
                        item_name_alt = POPUPEntityObject.ItemName,
                    });
                }
                //update
                else if (dgSelectedIndexItem >= 0 && ItemsEntity.Count > dgSelectedIndexItem)
                {
                    if (ItemsEntity[dgSelectedIndexItem].id == 0 && ((IndexOfExistValue == -1) || (IndexOfExistValue != -1 && AllowDuplicate == true)))
                    {
                        ItemsEntity[dgSelectedIndexItem].item_code_alt = POPUPEntityObject.ItemCode;
                        ItemsEntity[dgSelectedIndexItem].item_name_alt = POPUPEntityObject.ItemName;


                    }
                    else if (ItemsEntity[dgSelectedIndexItem].item_code != POPUPEntityObject.ItemCode)
                    {
                        ItemsEntity[dgSelectedIndexItem].item_code_alt = POPUPEntityObject.ItemCode;
                        ItemsEntity[dgSelectedIndexItem].item_name_alt = POPUPEntityObject.ItemName;
            ...
            foreach (var o in ItemsEntity)
            {
                ...
                if (o.item_code != null && o.item_code != "")
                {
                    if (o.qty == null || o.qty == 0)
                    {
                        ...
                        showMessageService.ButtonSetup = DialogButton.Ok;
                        showMessageService.Caption = "Message";
                        showMessageService.Text = String.Format("Quantity cannot be null or 0 for item {0} and Parameter {1}", o.item_code, o.sku_desc);
                        showMessageService.ShowMessage();
                        return false;
                    }
                    ...
                    if (o.unit_code == null || o.unit_code == "")
                    {
                        ...
                        showMessageService.ButtonSetup = DialogButton.Ok;
                        showMessageService.Caption = "Message";
                        showMessageService.Text = String.Format("Please Enter Valid Unit Code for the item {0} and Parameter {1}", o.item_code, o.sku_desc);
                        showMessageService.ShowMessage();
                        return false;
                    }
                ...
                //                    showMessageService.Text = String.Format("All Parameters of item {0} are not selected..!!! \n If you can see All Parameter Values Selected Please Select the Same Values Again ", o.item_code);
                //                    showMessageService.ShowMessage();
                //                    return false;
                //                }
                //            }
                ...
                //            showMessageService.ButtonSetup = DialogButton.Ok;
                //            showMessageService.Caption = "Parameter Validation";
                //            showMessageService.Text = String.Format("All Parameters of item {0} are not selected\n If you can see All Parameter Value Selected Please Select the Same Values Again ", o.item_code);
                //            showMessageService.ShowMessage();
                //            return false;
                //        }
                //    }
        ...
        private void GetSelectedParaValue(IList parameter)
        {
            try
            {
                ...
                if (dgSelectedIndexItem != -1 && SelectedParaValueList.Count > 0 && ItemsEntity[dgSelectedIndexItem].ind_sku == true)
                {
            ...
            try
            {
                ...
                if (dgSelectedIndexItem != -1 && ItemsEntity.Count > 0 && ItemsEntity.Count > dgSelectedIndexItem)
                {
                    ...
                    if (ItemsEntity.Count > 0)
                    {
                        ...
                        if (ItemsEntity[dgSelectedIndexItem].id == 0 && ItemsEntity[dgSelectedIndexItem].ind_sku == true)
                        {
                            parameter = true;
                        }
                        else
        ...
        private void SetBusinessEntitiesAfterLoad(string ParameterOption1, string ParameterOption2)
        {
            ...
            if (MasterEntity.X != null && isNewRecord == true && ParameterOption1 == "Save")
            {
        ...
        private string _filterStringitem_code;
        public string filterStringitem_code
        {
            get { return _filterStringitem_code; }
            set
            {
                _filterStringitem_code = value;
                RaisePropertyChanged("filterStringitem_code");
                Filter_item_codeCollection();
            }
        }
        private void Filter_item_codeCollection()
        {
        ...
        public bool Filter_item_code(object obj)
        {
            var data = obj as ADM_M022_POPUP;
            if (data != null)
            {
                if (!string.IsNullOrEmpty(filterStringitem_code))
                {
                    return (data.item_code != null && data.item_code.ToString().ToLower().Contains(_filterStringitem_code.ToLower())) ||
                           (data.item_name != null && data.item_name.ToString().ToLower().Contains(_filterStringitem_code.ToLower()));
        ...
        public bool FilterAlternateItem(object obj)
        {
            ...
            if (data != null)
            {
                ...
                {
                    return (data.item_code != null && data.item_code.ToString().ToLower().Contains(_FilterStringAlternateItem.ToLower())) ||
                           (data.item_name != null && data.item_name.ToString().ToLower().Contains(_FilterStringAlternateItem.ToLower()));
