...
{
    public class EPR_T003LocalExportPackingVM : WorkspaceViewModel<EPR_T003_A>
    {
        ...
        private decimal _CartonsPacked;
        public decimal CartonsPacked
        {
            get
            {
                return _CartonsPacked;
            }
            set
            {
                if (_CartonsPacked != value)
                {
                    _CartonsPacked = value;
                    RaisePropertyChanged("CartonsPacked");
                }
            }
        }
        ...
        private decimal _CartonsToBePacked;
        public decimal CartonsToBePacked
        {
            get
            {
                return _CartonsToBePacked;
            }
            set
            {
                if (_CartonsToBePacked != value)
                {
                    _CartonsToBePacked = value;
                    RaisePropertyChanged("CartonsToBePacked");
                }
            }
        }
        ...
        public RelayCommand cmdPrintReport6 { get; private set; }
        ...
        public EPR_T003LocalExportPackingVM(string ts_code) : base()
        {
        ...
        public EPR_T003LocalExportPackingVM(string ts_code, string doc_no) : base()
        {
            this.ts_code_vm = ts_code;
        ...
        private void LoadInitialData()
        {
            try
            {
                ...
                cmdPrintReport6 = new RelayCommand(() => { LoadPrintReport6(); });
        ...
        private void InsertAllDataFromScannedBarcode(object InputValue)
        {
            try
            {
                ...
                if (MasterEntity.carton_type != null && MasterEntity.carton_type != "" && MC.SmallCartonList != null)
                {
                    if (MasterEntity.no_of_carton > 0 && MasterEntity.no_of_carton > SmallCartonEntity.Count)
                    {
                                ...
                                if (POPUPEntityObject != null)
                                {
                                    if (POPUPEntityObject.carton_used_Flg == false)
                                    {
                                        if (SmallCartonEntity.Count == 0)
                                        {
                                            ...
                                            CartonsPacked += 1;
                                            CartonsToBePacked = Convert.ToDecimal(MasterEntity.no_of_carton) - CartonsPacked;

                                            QtyPacked += Convert.ToDecimal(POPUPEntityObject.tot_qty);
                                            QtyToBePacked = Convert.ToDecimal(MasterEntity.tot_qty - QtyPacked);
                                           ...
                                           MasterEntity.grade == POPUPEntityObject.grade && MasterEntity.cust_prod_code == POPUPEntityObject.cust_prod_code) ||
                                           (SmallCartonEntity.Count > 0 && MasterEntity.ItemCode == POPUPEntityObject.ItemCode && MasterEntity.grade == POPUPEntityObject.grade && MasterEntity.ink_id == POPUPEntityObject.ink_id && MasterEntity.cust_prod_code == POPUPEntityObject.cust_prod_code && MasterEntity.grade != "A"))
                                        {
        ...
        private void LoadLocalExportCartons()
        {
            try
            {
                ...
                if (MCTemp.MasterEntityList.Count > 0)
                {
                    ...
                    FlipDataGridCollection.Filter = new Predicate<object>(Filter_ExportCarton);
            ...
            try
            {
                if (MCTemp.MasterEntityList != null && MCTemp.MasterEntityList.Count > 0 && MC.SettingsList.Count > 0)
                {
                    if (PrintOption == "All")
                    {
                        ...
                        ReportManager ReportManager = new ReportManager();
                        ReportManager.DisplayReport(objDataSource, objDataSourceName, "\\Label\\" + MC.SettingsList[0].report_name6, MC.SettingsList[0].report_name6);
                    }
                    else if (PrintOption == "Selected")
                    {
                        int count = MCTemp.MasterEntityList.Count(x => x.check == true);
            ...
            try
            {
                ...
                if (ParameterObject != null)
                {
                    if (((IEnumerable)ParameterObject).Cast<EPR_T003_A>().ToList().Count > 0)
                    {
                        ...
                        CartonsPacked = Convert.ToDecimal(MasterEntity.no_of_carton);
                        QtyPacked = Convert.ToDecimal(MasterEntity.tot_qty);
        ...
        protected override void OnCreateAction(InquiryActionResult<EPR_T003_A> result)
        {
            try
            {
                ...
                var msg = new NotificationMessage("EPR_T003LocalExportPackingVM");
                Messenger.Default.Send<NotificationMessage>(msg);

                QtyPacked = 0;
                QtyToBePacked = Convert.ToDecimal(MasterEntity.tot_qty);
                CartonsPacked = 0;
                CartonsToBePacked = Convert.ToDecimal(MasterEntity.no_of_carton);
            }
            catch (Exception ex)
            {
                IShowMessageViewService showMessageService = this.GetViewService<IShowMessageViewService>();
        ...
        public string FilterString
        {
            ...
            set
            {
                ...
                FilterCollection_ExportCarton();
            }
        }


        private void FilterCollection_ExportCarton()
        {
        ...
        public bool Filter_ExportCarton(object obj)
        {
