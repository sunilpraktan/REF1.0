    ...
    public static class Taskbar
    {
        ...
        public static void Init()
        {
            try
            {
                if (TaskbarManager.IsPlatformSupported)
                {
                    ...
                    ((INotifyCollectionChanged)TaskManagerView.Root.TaskData.AllFolders).CollectionChanged += (sender, args) => RefreshTaskbarTasks(jumpList);
                }
            }
            catch (COMException)
            {
        ...
        private static void RefreshTaskbarTasks(JumpList jumpList)
        {
            try
            {
                ...
                foreach (BaseFolder f in TaskManagerView.Root.TaskData.AllFolders)
                {
                    jumpList.AddUserTasks(
