...
{
    public class PMS_T002_VM : WorkspaceViewModel<PMS_T001>, ITS_VIEW_MODEL
    {
        ...
        private AutoSuggestTextViewModel<dynamic> _AS_STANDARD { get; set; }
        public AutoSuggestTextViewModel<dynamic> AS_STANDARD
        {
            get { return _AS_STANDARD; }
            set
            {
                if (_AS_STANDARD != value)
                {
                    _AS_STANDARD = value; RaisePropertyChanged("AS_STANDARD");
                }
            }
        }

        private AutoSuggestTextViewModel<dynamic> _AS_SUB_STANDARD { get; set; }
        public AutoSuggestTextViewModel<dynamic> AS_SUB_STANDARD
        {
            get { return _AS_SUB_STANDARD; }
            set
            {
                if (_AS_SUB_STANDARD != value)
                {
                    _AS_SUB_STANDARD = value; RaisePropertyChanged("AS_SUB_STANDARD");
                }
            }
        }
        ...
        //public STD_LIST_BE doc_info_para { get => throw new NotImplementedException(); set => throw new NotImplementedException(); }
        ...
        void ModelUpdated_Master(object sender, EventArgs e)
        {
            try
            {
                if (EntityChangeEnable == true)
                {
                    ...
                    {
                        if(MasterEntity.date_start.HasValue && MasterEntity.date_end.HasValue)
                        {
                            MasterEntity.duration = Convert.ToDecimal((MasterEntity.date_end - MasterEntity.date_start).Value.TotalDays);
                        }
                    }
                }
            }
        ...
        public RelayCommand<object> cmdPrintDocument { get; private set; }
        ...
        private void DefaultValues()
        {
            ...
            MasterEntity.date_end = DateTime.Now;
        ...
        #region Constructor
        public PMS_T002_VM(string ts_code, string doc_cat) : base()
        {
        ...
        private void CommandInitialization()
        {
            ...
            try
            {
                ...
                cmdPrintDocument = new RelayCommand<object>(cmdPara => { if (cmdPara == null) { return; } PrintDocument(cmdPara); });
        ...
        private void LoadInitialData()
        {
            ...
            {
                isNewRecord = true;
                string Request = "LOAD_INI" + "!@" + AppSessionState.client + "!@" + AppSessionState.OBJ_COMPANY.comp_code + "!@" + (REQ_PARA_OBJ.location_id ?? AppSessionState.OBJ_LOCATION.location_id) + "!@" + doc_cat_vm + "!@" + doc_cat_vm + "!@" + (MasterEntity.project_id ?? "") + "!@" + AppSessionState.UserID + "!@" + AppSessionState.EmpId + "!@" + ts_code_vm + "!@" + REQ_PARA_OBJ.org_code + "!@" + (REQ_PARA_OBJ.group_code ?? "");
                ...
                AS_STANDARD = new AutoSuggestTextViewModel<dynamic>(MC.STANDARD_LIST, TheFilter, SuggestedValue, "value_code", true);
                AS_STANDARD.AutoSuggestVM.IsEmptyValueAllowed = true;

                SuggestedValue = new ValueConverter(x => x == null ? "" : ((STD_LIST_BE)x).value_code);
                TheFilter = (o, prefix) => (((STD_LIST_BE)o).value_code ?? "").ToString().ToLower().Contains(prefix.ToLower()) || (((STD_LIST_BE)o).value_name ?? "").ToString().ToLower().Contains(prefix.ToLower());
                AS_SUB_STANDARD = new AutoSuggestTextViewModel<dynamic>(MC.VALUE_LIST, TheFilter, SuggestedValue, "value_code", true);
                AS_SUB_STANDARD.AutoSuggestVM.IsEmptyValueAllowed = true;
        ...
        private void DocumentAttachment(object InputValue)
        {
            try
            {
                ...
                {            
                    Messenger.Default.Send<DocumentViewerPayload>(new DocumentViewerPayload() { DocumentNumber = MasterEntity.project_id.Replace("/", "--"), DocumentList = MC_TEMP.ATTACHMENT_LIST, client = AppSessionState.client, comp_code = MasterEntity.comp_code });
                }
            }
            catch (Exception ex)
            {
        ...
        private void PrintDocument(object InputValue)
        {
            CursorControl.SetBusyState();
            EntityChangeEnable = false;
            try
            {
                string Request = "LOAD_DOC_BY_DOC_NO2" + "!@" + AppSessionState.client + "!@" + MasterEntity.comp_code + "!@" + (MasterEntity.location_id ?? "") + "!@" + doc_cat_vm + "!@" + doc_cat_vm + "!@" + MasterEntity.project_id + "!@" + AppSessionState.UserID + "!@" + MasterEntity.emp_id + "!@" + ts_code_vm;
                MC_TEMP = REPO_MC.GetDataWithReturnDomainObject<MC_PMS_BE>(MC_TEMP, Request, "PMS_T001_BL", "PMS", "LOAD_DOC_BY_DOC_NO", 0, "");

                object[] dsObject = new object[8];
                string[] dsName = new string[8];

                //List<ADM_M002> TempCmpList = (List<ADM_M002>)AppSessionState.ADM_M002_List;
                //var CompanyData = MC.COMPANY_LIST.Where(x => x.comp_code == MasterEntity.comp_code).ToList();
                //var CompanyData = TempCmpList.Where(x => x.comp_code == MasterEntity.comp_code).ToList();
                var CompanyData = AppSessionState.COMPANY_LIST.Where(x => x.comp_code == MasterEntity.comp_code).ToList();
                var LocationData = AppSessionState.LOCATION_LIST.Where(x => x.location_id == MasterEntity.location_id).ToList();

                dsObject[0] = CompanyData;
                dsObject[1] = LocationData;
                dsObject[2] = MC_TEMP.PROJECT_LIST;
                dsObject[3] = MC_TEMP.ELEMENT_LIST;
                dsObject[4] = MC_TEMP.MILESTONE_LIST;
                dsObject[5] = MC_TEMP.TASK_LIST;
                dsObject[6] = MC_TEMP.ACTIVITY_LIST;
                dsObject[7] = MC_TEMP.REF_DOC_LIST;

                dsName[0] = "dsCompany";
                dsName[1] = "dsLocation";
                dsName[2] = "dsMaster";
                dsName[3] = "dsElement";
                dsName[4] = "dsMilestones";
                dsName[5] = "dsTask";
                dsName[6] = "dsActivity";
                dsName[7] = "dsReference";


                ReportManager ReportManager = new ReportManager();

                var ReportStringList = (from o in MC.DOC_TYPE_LIST where o.doc_type == MasterEntity.doc_type select o).ToList();
                string ReportName = ReportStringList[0].report_name.Split(',')[0];
                string ReportDisplayName = MasterEntity.project_name;
                ReportManager.DisplayReport(dsObject, dsName, "\\REPORTS_STD\\TXN\\" + ReportName, ReportDisplayName);
                EntityChangeEnable = true;
            }
            catch (Exception ex)
            {
        ...
        private void Save(object InputValue)
        {
            ...
            {
                CursorControl.SetBusyState();
                string Request = ""; // request for cloning of project before creating new document number we prepare request.

                if (Validation() == true)
                {
                    if(string.IsNullOrWhiteSpace(MasterEntity.project_id))
                    {
                        isNewRecord = true;
                    }
                    else
                    ...
                    MasterEntity.ts_code = ts_code_vm;
                    MasterEntity.userid = AppSessionState.UserID;
                    if (MasterEntity.ind_copy == true)
                    {
                        isNewRecord = true;
                        MasterEntity.ref_doc_cat = MasterEntity.doc_cat;
                        MasterEntity.ref_doc_no = MasterEntity.project_id;
                    }
                    if (isNewRecord == true)
                    {
                        MasterEntity = REPO.SaveWithReturnDomainObject<PMS_T001>(MasterEntity, "PMS_T001_BL", "PMS");
                    ...
                    {
                        MasterEntity = REPO.UpdateWithReturnDomainObject<PMS_T001>(MasterEntity, "PMS_T001_BL", "PMS");
                    }

                    
                    isNewRecord = false;
                    
                    // Logic sifted to PMS_T001_INS sp.
                    //if (MasterEntity.ind_copy == true) // Once copy created then create further all WBS element, Task, Activity etc.
                    //{
                    //    EntityChangeEnable = false;
                    //    //Request = "COPY_WBS_STRUCTURE" + "!@" + AppSessionState.client + "!@" + MasterEntity.comp_code + "!@" + (MasterEntity.location_id ?? "") + "!@" + doc_cat_vm + "!@" + doc_cat_vm + "!@" + MasterEntity.project_id + "!@" + AppSessionState.UserID + "!@" + AppSessionState.EmpId + "!@" + ts_code_vm + "!@" + MasterEntity.project_no.ToString() + "!@" + MasterEntity.doc_date.ToString();
                    //    MC_TEMP = REPO_MC.GetDataWithReturnDomainObject<MC_PMS_BE>(MC_TEMP, Request, "PMS_T001_BL", "PMS", "LoadAll", 0, "");
                    //    EntityChangeEnable = true;
                    //    MasterEntity.ind_copy = false;
                    //}
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (InputValue.GetType() == typeof(string) && InputValue != null)
                    {
                        ...
                        {
                            try
                            { POPUPEntityObject = MC.TYPE_LIST.Where(x => x.curr_code.Equals(Request, StringComparison.OrdinalIgnoreCase) == true).ToList()[0]; }
                            catch (Exception ex) { }
                        }
                    }
                    else if (InputValue != null)
        ...
        private void LoadDocumentByDocumentNumber(STD_LIST_BE ParameterObject)
        {
            try
            {
                ...
                MC_TEMP = REPO_MC.GetDataWithReturnDomainObject<MC_PMS_BE>(MC_TEMP, Request, "PMS_T001_BL", "PMS", "LoadAll", 0, "");
                if (MC_TEMP.PROJECT_LIST != null)
                {
                    if (MC_TEMP.PROJECT_LIST.Count > 0)
                    {
        ...
        private bool Validation()
        {
            try
            {
                if (MasterEntity.project_name == null)
                {
                ...
                {
                    sms.ButtonSetup = DialogButton.Ok; sms.Caption = "Required"; sms.Text = String.Format("Please input Unit ........"); sms.ShowMessage();
                    return false;
                }
