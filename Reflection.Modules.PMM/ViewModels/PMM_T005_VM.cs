...
{
    public class PMM_T005_VM : WorkspaceViewModel<PMM_T005>
    {
        ...
        WebServiceRepository<PMM_T005> REPO = new WebServiceRepository<PMM_T005>();
        WebServiceRepository<MC_PMM_BE> REPO_MC = new WebServiceRepository<MC_PMM_BE>();
        ObjectSerializationService obj = new ObjectSerializationService();
        WebServiceRepository<MC_PPC_BE> REPO_MC_MIS = new WebServiceRepository<MC_PPC_BE>();
        IShowMessageViewService sms;

        private MC_PMM_BE _MC = new MC_PMM_BE();
        public MC_PMM_BE MC
        {
            get { return _MC; }
            set { if (_MC != value) { _MC = value; RaisePropertyChanged("MC"); } }
        }
        private MC_PMM_BE _MC_TEMP = new MC_PMM_BE();
        ...
        private PMM_T005 _MasterEntity;
        public PMM_T005 MasterEntity
        {
            get { return _MasterEntity; }
            set { if (_MasterEntity != value) { _MasterEntity = value; RaisePropertyChanged("MasterEntity"); } }
        }
        private STD_LIST_BE _ReferenceEntity;
        public STD_LIST_BE ReferenceEntity
        {
            get { return _ReferenceEntity; }
            set { if (_ReferenceEntity != value) { _ReferenceEntity = value; RaisePropertyChanged("ReferenceEntity"); } }
        }
        ...
        public static readonly DependencyProperty AutoSuggestTextViewModelProperty = DependencyProperty.Register("AutoSuggestTextViewModel", typeof(AutoSuggestTextViewModel<dynamic>), typeof(PMM_T005_VM));
        ...
        public RelayCommand<PMM_T005> cmdIndent { get; private set; }
        public RelayCommand<PMM_T005> cmdTrace { get; private set; }
        public RelayCommand<PMM_T005> cmdLogReport { get; private set; }
        //public RelayCommand<object> cmdInsertCompany { get; private set; }

        #endregion

        #region Constructor
        public PMM_T005_VM(string ts_code, string doc_cat) : base()
        {
            this.doc_cat_vm = doc_cat;
            this.ts_code_vm = ts_code;
            PMM_T005.ModelEntityUpdated += new EventHandler(Model_MasterEntityChange);
            MC = new MC_PMM_BE();
            MC_TEMP = new MC_PMM_BE();
            REQUEST_PARA = new STD_REQ_PARA_BE();
            MasterEntity = new PMM_T005();
            ReferenceEntity = new STD_LIST_BE();
            sms = this.GetViewService<IShowMessageViewService>();
            InitializeCommands();
        }
        public PMM_T005_VM(string ts_code, string doc_cat, string doc_no) : base()
        {
            this.doc_cat_vm = doc_cat;
            this.ts_code_vm = ts_code;
        ...
        private void LoadInitialData(string company, string location)
        {
            try
            {
                ...
                MC = REPO_MC.GetDataWithReturnDomainObject<MC_PMM_BE>(MC, Request, "PMM_T005_BL", "PMM", " ", 0, "");
        ...
        private void LoadDocumentByDocumentNumber(object InputValue)
        {
            try
            {
                ...
                if (POPUPEntityObject != null)
                {
                    ...
                    MC_TEMP = REPO_MC.GetDataWithReturnDomainObject<MC_PMM_BE>(MC_TEMP, Request, "PMM_T005_BL", "PMM", " ", 0, "");

                    if (MC_TEMP.MEASUREMENT_LIST.Count > 0)
                    {
                        MasterEntity = MC_TEMP.MEASUREMENT_LIST[0];
                    }
                }
        ...
        private void InitializeCommands()
        {
            ...
            cmdIndent = new RelayCommand<PMM_T005>(items => { if (items == null) { return; } CreateIndent(items); });
            cmdTrace = new RelayCommand<PMM_T005>(items => { if (items == null) { return; } TraceReport(items); });
            cmdLogReport = new RelayCommand<PMM_T005>(items => { if (items == null) { return; } LogReport(items); });
        ...
        private void DefaultValues()
        {
            try
            {
                ...
                MasterEntity.record_date = DateTime.UtcNow;
                MasterEntity.record_time = DateTime.UtcNow.TimeOfDay.ToString();
        ...
        private bool Validation()
        {
            ...
            if (string.IsNullOrWhiteSpace(MasterEntity.char_name))
            {
                IShowMessageViewService sms = this.GetViewService<IShowMessageViewService>();
        ...
        private void LoadBackFlipData(object InputValue)
        {
            try
            {
                ...
                MC_TEMP = REPO_MC.GetDataWithReturnDomainObject<MC_PMM_BE>(MC_TEMP, Request, "PMM_T005_BL", "PMM", "LoadAll", 0, "");
        ...
        //    }
        //}
        private void CreateIndent(PMM_T005 ParameterObject)
        {
            try
            {
                if (!string.IsNullOrWhiteSpace(MasterEntity.doc_no))
                {
                    STD_LIST_BE OBJ_PARA = new STD_LIST_BE();
                    OBJ_PARA.ref_doc_cat = "MP";
                    OBJ_PARA.ref_doc_no = MasterEntity.doc_no;
                    OBJ_PARA.equip_no = MasterEntity.obj_no;
                    OBJ_PARA.equip_name = MasterEntity.obj_name;
                    OBJ_PARA.read_value = MasterEntity.read_value;
                    OBJ_PARA.unit_code = MasterEntity.unit_code;
                    OBJ_PARA.post_date = MasterEntity.record_date;

                    CursorControl.SetBusyState();
                    AppSessionState.ViewTitle = "Consumption Indent";
                    AppSessionState.TransactionCode = "MS11";
                    ...
                    Type type = assembly.GetType("Reflection.Modules.MM.Views.MM_T001");
                    if (type != null)
                    {
                        dynamic instance = Activator.CreateInstance(type, AppSessionState.TransactionCode, null,OBJ_PARA);
                ...
                {
                    CursorControl.SetBusyState();
                    AppSessionState.ViewTitle = "Indent Request";
                    AppSessionState.TransactionCode = "MS11";
        ...
        private void TraceReport(PMM_T005 ParameterObject)
        {
            try
            {
                if (!string.IsNullOrWhiteSpace(MasterEntity.doc_no))
                {
                    CursorControl.SetBusyState();
                    MC_PPC_BE MC_TEMP_MIS = new MC_PPC_BE();

                    string RequestParameter = "TRACE_REPORT" + "!@" + AppSessionState.client + "!@" + MasterEntity.comp_code + "!@" + (MasterEntity.location_id ?? "") + "!@" + MasterEntity.doc_cat + "!@" + MasterEntity.doc_type + "!@" + MasterEntity.obj_no + "!@" + MasterEntity.doc_no;
                    MC_TEMP_MIS = REPO_MC_MIS.GetDataWithReturnDomainObject<MC_PPC_BE>(MC_TEMP_MIS, RequestParameter, "EPR_T001_BL", "PPC", " ", 0, "");

                    REQUEST_PARA.obj_code = "RP0002"; //MIS_OBJ.obj_code
                    REQUEST_PARA.obj_name = "Material History for  " + (MasterEntity.obj_no ?? ""); //MIS_OBJ.obj_name
                    REQUEST_PARA.comp_name = MC.COMPANY_LIST.Where(x => x.comp_code == MasterEntity.comp_code).ToList()[0].comp_name;
                    object[] objDataSource = new object[1];
                    string[] objDataSourceName = new string[1];

                    objDataSource[0] = MC_TEMP_MIS.STD_MIS_LIST;
        ...
        private void LogReport(PMM_T005 ParameterObject)
        {
            try
            {
                if (!string.IsNullOrWhiteSpace(MasterEntity.doc_no))
                {
                    CursorControl.SetBusyState();
                    MC_PPC_BE MC_TEMP_MIS = new MC_PPC_BE();
                    string RequestParameter = "LOG_REPORT" + "!@" + AppSessionState.client + "!@" + MasterEntity.comp_code + "!@" + MasterEntity.location_id + "!@" + MasterEntity.doc_cat + "!@" + MasterEntity.doc_type + "!@" + MasterEntity.obj_no + "!@" + MasterEntity.doc_no;
                    MC_TEMP_MIS = REPO_MC_MIS.GetDataWithReturnDomainObject<MC_PPC_BE>(MC_TEMP_MIS, RequestParameter, "EPR_T001_BL", "PPC", " ", 0, "");

                    REQUEST_PARA.obj_code = "RP0003"; //MIS_OBJ.obj_code
                    REQUEST_PARA.obj_name = "Log Book for  " + (MasterEntity.obj_name ?? ""); //MIS_OBJ.obj_name
                    REQUEST_PARA.comp_name = MC.COMPANY_LIST.Where(x => x.comp_code == MasterEntity.comp_code).ToList()[0].comp_name;
                    object[] objDataSource = new object[1];
                    string[] objDataSourceName = new string[1];
        ...
        private Dictionary<string, string> getParametersList()
        {
            ...
            try
            {
                result.Add("from_date", Convert.ToString(REQUEST_PARA.from_date));
                result.Add("to_date", Convert.ToString(REQUEST_PARA.to_date));
                result.Add("rpt_title", Convert.ToString(REQUEST_PARA.obj_name));
                result.Add("comp_code", Convert.ToString(REQUEST_PARA.comp_code));
                result.Add("comp_name", Convert.ToString(REQUEST_PARA.comp_name));
                result.Add("para1", Convert.ToString(REQUEST_PARA.para4));
            }
            catch (Exception ex)
            {
                IShowMessageViewService sms = this.GetViewService<IShowMessageViewService>();
        ...
        private void ExecuteReference(object InputValue)
        {
            try
            {
                ...
                if (POPUPEntityObject != null)
                {
                    ...
                    MasterEntity.previous_value = POPUPEntityObject.read_value;
                }
            }
            catch (Exception ex)
            {
        ...
        void Model_MasterEntityChange(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (sender.ToString() == "read_value")
                    {
                        if(MasterEntity.read_value.HasValue && MasterEntity.previous_value.HasValue)
                        {
                            MasterEntity.read_diff = MasterEntity.read_value - MasterEntity.previous_value;
                            MasterEntity.read_counter = MasterEntity.previous_value;
                        }
                        else
                        {
                            MasterEntity.read_diff = 0;
                            MasterEntity.read_counter = MasterEntity.read_value;
                        }
                    }
                }
            }
        ...
        protected override void OnCreateAction(InquiryActionResult<PMM_T005> result)
        {
        ...
        protected override void OnDiscardAction(InquiryActionResult<PMM_T005> result)
        {
        ...
        protected override void OnFevoriteAction(InquiryActionResult<PMM_T005> result)
        {

        }
        protected override void OnFlipAction(InquiryActionResult<PMM_T005> result)
        {

        }
        protected override void OnHelpAction(InquiryActionResult<PMM_T005> result)
        {

        }
        protected override void OnRefreshCommand(InquiryActionResult<PMM_T005> result)
        {
            throw new NotImplementedException();
        }

        protected override void OnLedgerViewCommand(InquiryActionResult<PMM_T005> result)
        {
            throw new NotImplementedException();
        }

        protected override void OnValidateCommand(InquiryActionResult<PMM_T005> result)
        {
            throw new NotImplementedException();
        }

        protected override void OnTraceCommand(InquiryActionResult<PMM_T005> result)
        {
            throw new NotImplementedException();
        }

        protected override void OnMailCommand(InquiryActionResult<PMM_T005> result)
        {
            throw new NotImplementedException();
        }
        protected override void OnPrintAction(InquiryActionResult<PMM_T005> result)
        {

        }
        protected override void OnRemoveAction(InquiryActionResult<PMM_T005> result)
        {

        }
        protected override void OnSaveAction(InquiryActionResult<PMM_T005> result)
        {
            try
            {
                if (Validation() == true)
                {
                    ...
                    if (isNewRecord == true)
                    {
                        MasterEntity = REPO.SaveWithReturnDomainObject<PMM_T005>(MasterEntity, "PMM_T005_BL", "PMM");
                        isNewRecord = false;
                    ...
                    {
                        MasterEntity = REPO.UpdateWithReturnDomainObject<PMM_T005>(MasterEntity, "PMM_T005_BL", "PMM");
                        isNewRecord = false;
                        IShowMessageViewService sms = this.GetViewService<IShowMessageViewService>(); sms.ButtonSetup = DialogButton.Ok; sms.Caption = "Message"; sms.Text = String.Format("Record Addred Successfully!", this.Title); sms.ShowMessage();
                    }
                }
