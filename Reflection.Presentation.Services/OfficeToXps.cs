...
namespace Reflection.Presentation.Services
{
    public class OfficeToXps
    {
        ...
        private static List<string> outlookExtensions = new List<string>
        {
            ".msg"
        };
        ...
        private static bool IsValidFilePath(string sourceFilePath)
        {
        ...
        private static bool IsConvertableFilePath(string sourceFilePath)
        {
            var ext = Path.GetExtension(sourceFilePath).ToLower();

            return wordExtensions.Contains(ext) ||
                   excelExtensions.Contains(ext) ||
                   outlookExtensions.Contains(ext) ||
                   powerpointExtensions.Contains(ext);
        }
        ...
        private static OfficeToXpsConversionResult ConvertFromOutlook(string sourceFilePath, ref string resultFilePath)
        {
            // Need to find a solution for Outlook
            return new OfficeToXpsConversionResult();
        }
