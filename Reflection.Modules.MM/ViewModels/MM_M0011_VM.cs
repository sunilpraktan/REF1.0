...
{
    public class MM_M0011_VM : WorkspaceViewModel<STD_BE_A>
    {
        ...
        //public static readonly DependencyProperty AutoSuggestTextViewModelProperty = DependencyProperty.Register("AutoSuggestTextViewModel", typeof(AutoSuggestTextViewModel<dynamic>), typeof(MM_M0011_VM));
        ...
        public RelayCommand<object> cmdInsertGroup { get; private set; }
        #endregion

        #region Constructor
        public MM_M0011_VM(string ts_code) : base()
        {
            ...
            cmdInsertGroup = new RelayCommand<object>(items => { if (items == null) { return; } InsertGroup(items); });
            ItemsEntity.CollectionChanged += new System.Collections.Specialized.NotifyCollectionChangedEventHandler(CollectionChangedNotifyForItem);
            LoadInitialData();
        }
        ...
        private void LoadInitialData()
        {
            try
            {
                ...
                MC = repository_MC.GetDataWithReturnDomainObject<MC_GEN_BE>(MC, Request, "MM_M0032_BL", "MM", "LOAD_INI", 0, "");
                ...
                TheFilter = (o, prefix) => (((STD_LIST_BE)o).group_code ?? "").ToString().ToLower().Contains(prefix) ||
                                           (((STD_LIST_BE)o).group_name ?? "").ToString().ToLower().Contains(prefix);
                AS_GROUP = new AutoSuggestTextViewModel<dynamic>(MC.GROUP_LIST, TheFilter, SuggestedValue, "group_code", "group_code", true);
                AS_GROUP.AutoSuggestVM.IsEmptyValueAllowed = true; AS_GROUP.AutoSuggestVM.IsFreeTextAllowed = false;
        ...
        private bool Validation()
        {
            foreach (var o in ItemsEntity)
            {
                ...
                if (o.selected == true)
                {
                    ...
                    {
                        if (o.comp_code == p.comp_code && o.ctry_code == p.ctry_code && o.group_code == p.group_code)
                        {
                            flag++;
                        }
                    }
                    ...
                    {
                        sms.ButtonSetup = DialogButton.Ok; sms.Caption = "Message"; sms.Text = String.Format("Select company for the HSN {0} and Product Group {1} and Country {1}", o.hsn_code, o.group_code, o.ctry_code); sms.ShowMessage();
                        return false;
                    }
                    if (string.IsNullOrWhiteSpace(o.ctry_code))
                    {
                        sms.ButtonSetup = DialogButton.Ok; sms.Caption = "Message"; sms.Text = String.Format("Select Country for the HSN {0} and Product Group {1} and Company {1}", o.hsn_code, o.group_code, o.comp_code);
                        sms.ShowMessage(); return false;
                    }
                    if (string.IsNullOrWhiteSpace(o.group_code))
                    {
                        sms.ButtonSetup = DialogButton.Ok; sms.Caption = "Message"; sms.Text = String.Format("Select Group for the HSN {0} and Country {1} and Company {1}", o.hsn_code, o.ctry_code, o.comp_code);
                        sms.ShowMessage(); return false;
                    }
                    if (string.IsNullOrWhiteSpace(o.hsn_code))
                    {
                        sms.ButtonSetup = DialogButton.Ok; sms.Caption = "Message"; sms.Text = String.Format("Select HSN Code for the Country {0} and Product Group {1} and Company {1}", o.ctry_code, o.group_code, o.comp_code);
                        sms.ShowMessage(); return false;
                    }
                }
            }
            ...
            try
            {
                ...
                try
                {
                    if (InputValue.GetType() == typeof(string) && InputValue != null)
                    {
                        ...
                        {
                            try
                            { POPUPEntityObject = MC.GROUP_LIST.Where(x => x.group_code.Equals(Request, StringComparison.OrdinalIgnoreCase) == true).ToList()[0]; }
                            catch (Exception ex) { }
                        }
                    }
                ...
                {
                    ItemEntityObject.group_code = POPUPEntityObject.group_code;
                    ItemEntityObject.group_name = POPUPEntityObject.group_name;
                }
            }
            catch (Exception ex)
            {
        ...
        protected override void OnSaveAction(InquiryActionResult<STD_BE_A> result)
        {
            try
            {
                ...
                if (Validation() == true)
                {
                    string strReturn = repository.Save<List<STD_BE_A>>(RequestList, "MM_M0032_BL", "MM");
