...
{
    public class MM_T005_VM : WorkspaceViewModel<MM_T001>
    {
        ...
        public static readonly DependencyProperty AutoSuggestTextViewModelProperty = DependencyProperty.Register("AutoSuggestTextViewModel", typeof(AutoSuggestTextViewModel<dynamic>), typeof(MM_T005_VM));
        ...
        private IEnumerable _ASSET_COLLECTION;
        public IEnumerable ASSET_COLLECTION
        {
            get { return _ASSET_COLLECTION; }
            set
            {
                _ASSET_COLLECTION = value;

                RaisePropertyChanged("_ASSET_COLLECTION");
            }
        }
        ...
        public RelayCommand<object> cmdAssetScanned { get; private set; } // user can scann aset and then it will auto set item and asset details.
        ...
        public RelayCommand<object> cmdInsertBatchItem { get; private set; }
        ...
        #region . Constructor .
        public MM_T005_VM(string doc_cat, string ts_code)
            : base()
        {
        ...
        public MM_T005_VM(string doc_cat, string ts_code, string doc_no)
            : base()
        {
            ts_code_vm = ts_code;
            doc_cat_vm = doc_cat;
        ...
        public MM_T005_VM(string ts_code, STD_LIST_BE STD_OBJ)
            : base()
        {
            ts_code_vm = ts_code;
            doc_cat_vm = STD_OBJ.doc_cat;
        ...
        private void SelectionChangedItem(object InputValue)
        {
            try
            {
                if (InputValue != null)
                {
                    ...
                    {
                        //if (ITEM_ENTITY_OBJ.line_id != ((IEnumerable)InputValue).Cast<MM_T001_A>().ToList()[0].line_id) // NOTE: Commented on 01-06-2023 because it was not calling when only one item in issue, and also exact meaning of this condition not recalling
                        {
                            ...
                            if (ITEM_ENTITY_OBJ != null)
                            {
                                if (!string.IsNullOrWhiteSpace(ITEM_ENTITY_OBJ.ItemCode))
                                {
                                    List<STD_ITEM> BATCH_LIST_OBJ;
                                    if (MC.BATCH_LIST.Where(item => item.item_code == ITEM_ENTITY_OBJ.ItemCode).Any() == false)
                                    {
                                        string Request = "Load_Batch_Data" + "!@" + AppSessionState.client + "!@" + (ITEM_ENTITY_OBJ.comp_code ?? MasterEntity.comp_code) + "!@" + (ITEM_ENTITY_OBJ.location_Id ?? MasterEntity.location_Id) + "!@" + this.doc_cat_vm + "!@" + this.doc_cat_vm + "!@" + ITEM_ENTITY_OBJ.ItemCode + "!@" + ITEM_ENTITY_OBJ.unit_code + "!@" + ITEM_ENTITY_OBJ.store_code + "!@" + (ITEM_ENTITY_OBJ.sku ?? "");
                                        MC_TEMP = REPOSITORY_OBJ_TEMP.GetDataWithReturnDomainObject<MC_MM_T001>(MC_TEMP, Request, "MM_T001_BL", "MM", "LoadAll", 0, "");

                                        foreach (var item in MC_TEMP.BATCH_LIST) // this is to keep on adding batch no if not exists
                                        {
                                        ...
                                        //MC.BATCH_LIST = MC_TEMP.BATCH_LIST; // This is for every time selection changed, new record get loaded. following logic commented because other user can use same batch if use from cache. so its better to refresh every time selection changes.
                                        //foreach (var item in MC_TEMP.BATCH_LIST) // this is to keep on adding batch no if not exists
                                        //{
                                        //    if (!MC.BATCH_LIST.Any(x => x.batch_no == item.batch_no))
                                        //    {
                                        ...
                                        BATCH_LIST_OBJ = MC.BATCH_LIST.Where(x => x.item_code == ITEM_ENTITY_OBJ.ItemCode && (x.sku ?? "") == (ITEM_ENTITY_OBJ.sku ?? "") && x.store_code == ITEM_ENTITY_OBJ.store_code).ToList(); // No nned to filter if for every single item we fetch batch data.
                                        SuggestedValue = new ValueConverter(x => x == null ? "" : ((STD_ITEM)x).batch_no);
                                        TheFilter = (o, prefix) => (((STD_ITEM)o).batch_no ?? "").ToString().ToLower().Contains(prefix.ToLower()) || (((STD_ITEM)o).barcode ?? "").ToString().ToLower().Contains(prefix.ToLower());
                                        AS_BATCH = new AutoSuggestTextViewModel<dynamic>(BATCH_LIST_OBJ, TheFilter, SuggestedValue, "batch_no", "batch_no", true); //C.BATCH_LIST
                                        AS_BATCH.AutoSuggestVM.IsEmptyValueAllowed = true; AS_BATCH.AutoSuggestVM.IsFreeTextAllowed = true;
                                    }
                                    else if(MC.BATCH_LIST.Where(item => item.item_code == ITEM_ENTITY_OBJ.ItemCode).Any() == true)
                                    {
                                        //NOTE: Also load Batch Data on change of SKU also.
                                        BATCH_LIST_OBJ = MC.BATCH_LIST.Where(x => x.item_code == ITEM_ENTITY_OBJ.ItemCode && (x.sku ?? "") == (ITEM_ENTITY_OBJ.sku ?? "") && x.store_code == ITEM_ENTITY_OBJ.store_code).ToList(); // No need to filter if for every single item we fetch batch data.
                                        SuggestedValue = new ValueConverter(x => x == null ? "" : ((STD_ITEM)x).batch_no);
                                        TheFilter = (o, prefix) => (((STD_ITEM)o).batch_no ?? "").ToString().ToLower().Contains(prefix.ToLower()) || (((STD_ITEM)o).barcode ?? "").ToString().ToLower().Contains(prefix.ToLower());
                                        AS_BATCH = new AutoSuggestTextViewModel<dynamic>(BATCH_LIST_OBJ, TheFilter, SuggestedValue, "batch_no", "batch_no", true); //C.BATCH_LIST
                                        AS_BATCH.AutoSuggestVM.IsEmptyValueAllowed = true; AS_BATCH.AutoSuggestVM.IsFreeTextAllowed = true;
        ...
        private void SelectionChangedBatch(object InputValue)
        {
            try
            {
                if (InputValue != null)
                {
                    if (((IEnumerable)InputValue).Cast<MM_T001_B>().ToList().Count > 0)
                    {
                        BATCH_ENTITY_OBJ = ((IEnumerable)InputValue).Cast<MM_T001_B>().ToList()[0];
                    }
                }
            }
            catch (Exception ex)
                ...
                if (POPUP_ENTITY_OBJ != null && ITEM_ENTITY_OBJ != null && !string.IsNullOrWhiteSpace(ITEM_ENTITY_OBJ.ItemCode))
                {
                    ...
                    if(BatchEntity != null)
                    {
                        if(BatchEntity.Count > 0)
                        {
                            try
                            {
                                BATCH_ENTITY_OBJ = BatchEntity.Where(X => X.batch_no == POPUP_ENTITY_OBJ.batch_no && X.ItemCode == ITEM_ENTITY_OBJ.ItemCode).Single();
                            }
                            catch(Exception ex)
                            {
                                BATCH_ENTITY_OBJ = new MM_T001_B();
                                BatchEntity.Add(BATCH_ENTITY_OBJ);
                            }
                        }
                    }

                    BATCH_ENTITY_OBJ.batch_no = POPUP_ENTITY_OBJ.batch_no;
                    ...
                    BATCH_ENTITY_OBJ.comp_code = POPUP_ENTITY_OBJ.comp_code;
                    BATCH_ENTITY_OBJ.location_Id = POPUP_ENTITY_OBJ.location_id;
                    BATCH_ENTITY_OBJ.userid = AppSessionState.UserID;
                    BATCH_ENTITY_OBJ.add_by = AppSessionState.UserID;
                    BATCH_ENTITY_OBJ.sku = POPUP_ENTITY_OBJ.sku;
                    BATCH_ENTITY_OBJ.t_status = ITEM_ENTITY_OBJ.t_status;
                    BATCH_ENTITY_OBJ.active = true;
                    BATCH_ENTITY_OBJ.client = AppSessionState.client;
                    BATCH_ENTITY_OBJ.ref_batch_row_id = ITEM_ENTITY_OBJ.id;
                }
            }
            catch (Exception ex)
            { sms.ButtonSetup = DialogButton.Ok; sms.Caption = "Message"; sms.Text = String.Format(ex.Message, this.Title); sms.ShowMessage(); }
        ...
        private void InsertBatchItem(object InputValue)// NOTE: remove this once single function work for all grid
        {
            try
            {
                string Request = "";
        ...
        private void LoadBackFlipData(object para)
        {
            ...
            MC_TEMP = REPOSITORY_OBJ_TEMP.GetDataWithReturnDomainObject<MM_T001>(MC_TEMP, Request, "MM_T001_BL", "MM", "", 0, "");
        ...
        private void InsertMovementType(object InputValue)
        {
            try
            {
                ...
                if (POPUP_ENTITY_OBJ != null)
                {
                    ...
                    // Add all in REF_DOC_LIST with doc_cat value
        ...
        private void InsertOrder(object InputValue) // NOTE: depricated
        {
            try
            {
                string Request = "";
        ...
        private void InsertWorkCenter(object InputValue)
        {
            try
            {
                string Request = "";
        ...
        private void InsertStoreCode(object InputValue)
        {
            try
            {
                ...
                if (POPUP_ENTITY_OBJ != null) // Only enter in the code block if ENtity Not null.
                {
                    ...
                    SuggestedValue = new ValueConverter(x => x == null ? "" : ((STD_ITEM)x).batch_no);
                    TheFilter = (o, prefix) => (((STD_ITEM)o).batch_no ?? "").ToString().ToLower().Contains(prefix.ToLower()) || (((STD_ITEM)o).batch_no ?? "").ToString().ToLower().Contains(prefix.ToLower());
        ...
        private void InsertMaterial(object InputValue, string InputSource) // NOTE: do not allow to add Material without MasterEntity fields like comp_code,Location etc.
        {
            try
            {
                ...
                if (InputValue.GetType() == typeof(string) && InputValue != null)
                {
                    ...
                    {
                        try
                        { POPUP_ENTITY_OBJ = MC.STD_ITEM_LIST.Where(x => x.item_code.Equals(Request, StringComparison.OrdinalIgnoreCase) == true || (x.barcode ?? "").Equals(Request, StringComparison.OrdinalIgnoreCase) == true).ToList()[0]; }
                        catch (Exception ex) { }
                    }
                }
                else if (InputValue != null)
                ...
                if (POPUP_ENTITY_OBJ != null && ITEM_ENTITY_OBJ != null) // Only enter in the code block if ENtity Not null.
                {
                    ...
                    if (InputSource == "Scan") // if item added by scan event then new row need to add first.
                    {
                        ...
                        //OBJ_TEMP = ITEM_ENTITY_OBJ;
                        ITEM_ENTITY_OBJ = new MM_T001_A();
                        //ITEM_ENTITY_OBJ = OBJ_TEMP;
                    }
                    ...
                    ITEM_ENTITY_OBJ.qty = 1; // POPUP_ENTITY_OBJ.qty;
                    ...
                    if (InputSource == "Scan")
                    {
                        ItemsEntity.Add(ITEM_ENTITY_OBJ);
                    }
                }
            }
            catch (Exception ex)
        ...
        private void InsertMaterialAndAsset(object InputValue, string InputSource) // NOTE: do not allow to add Material without MasterEntity fields like comp_code,Location etc.
        {
            try
            {
                string Request = "";
                STD_ITEM POPUP_ENTITY_OBJ = null;
                STD_LIST_BE ASSET_ENTITY_OBJ = null;
                #region Command Parameter Read Section
                if (InputValue.GetType() == typeof(string) && InputValue != null)
                {
                    Request = InputValue.ToString();
                    if (Request.Length > 0)
                    {
                        try
                        { 
                            if(MC.EQUIPMENT_LIST != null) // First search asset and then get item code form asset
                            {
                                if(MC.EQUIPMENT_LIST.Count > 0)
                                {
                                    ASSET_ENTITY_OBJ = MC.EQUIPMENT_LIST.Where(x => x.equip_no.Equals(Request, StringComparison.OrdinalIgnoreCase) == true).ToList()[0];
                                }
                            }
                            
                            POPUP_ENTITY_OBJ = MC.STD_ITEM_LIST.Where(x => x.item_code.Equals(ASSET_ENTITY_OBJ.item_code, StringComparison.OrdinalIgnoreCase) == true).ToList()[0];

                            if(ASSET_ENTITY_OBJ?.t_status != "12" && MasterEntity.doc_cat == "MI")
                            {
                                sms.ButtonSetup = DialogButton.Ok; sms.Caption = "Message"; sms.Text = String.Format("Asset cannot issue if not in unristricted stock!", this.Title); sms.ShowMessage();
                                return;
                            }
                        }
                        catch (Exception ex) { }
                ...
                if (POPUP_ENTITY_OBJ != null && ITEM_ENTITY_OBJ != null) // Only enter in the code block if ENtity Not null.
                {
                    ...
                    ITEM_ENTITY_OBJ.ItemCode = POPUP_ENTITY_OBJ.item_code;
                    ITEM_ENTITY_OBJ.description = ASSET_ENTITY_OBJ.equip_name ?? POPUP_ENTITY_OBJ.equip_name;
                    ITEM_ENTITY_OBJ.unit_code = POPUP_ENTITY_OBJ.unit_code;
                    ITEM_ENTITY_OBJ.StockUnt = Convert.ToBoolean(POPUP_ENTITY_OBJ.ind_sku);
                    ITEM_ENTITY_OBJ.active = true;
                    ITEM_ENTITY_OBJ.SubCatCode = POPUP_ENTITY_OBJ.item_subcat;
                    ...
                    if (MC.STORE_LIST != null)
                    {
                        ...
                        {
                            ITEM_ENTITY_OBJ.store_code = MC.STORE_LIST[0].store_code;
                        }
                    }
                    ITEM_ENTITY_OBJ.qty = 1; // POPUP_ENTITY_OBJ.qty;
                    ITEM_ENTITY_OBJ.order_doc_no = MasterEntity.order_doc_no;
                    ...
                    ITEM_ENTITY_OBJ.t_status = MasterEntity.t_status;
                    ITEM_ENTITY_OBJ.t_display = MasterEntity.t_display;
                    ITEM_ENTITY_OBJ.equip_no = ASSET_ENTITY_OBJ.equip_no;
                    ITEM_ENTITY_OBJ.equip_name = ASSET_ENTITY_OBJ.equip_name;
                    ITEM_ENTITY_OBJ.asset_no = ASSET_ENTITY_OBJ.equip_no;
                    //if (!string.IsNullOrWhiteSpace(ASSET_ENTITY_OBJ.serial_no))
                    //{
                    //    ITEM_ENTITY_OBJ.description = $"{ITEM_ENTITY_OBJ.description} {" Sr.No: "} {ASSET_ENTITY_OBJ.serial_no}";
                    //}
                    if (InputSource == "Scan")
                    {
                        ItemsEntity.Add(ITEM_ENTITY_OBJ);
        ...
        private void BarcodeScanned(object InputValue)
        {
            try
            {
                ...
                if (barcode.Length >= 10) // condition change from Fix 9 to from DB
                {
        ...
        private void AssetScanned(object InputValue)
        {
            try
            {
                //EPR_T003_A_P POPUPEntityObject = null;
                ...
                if (barcode.Length >= 7) // condition change from Fix 9 to from DB
                {
                    InsertMaterialAndAsset(InputValue, "Scan");
                    Barcode = "";
                }
            }
            catch (Exception ex)
            ...
            if (e.Action == NotifyCollectionChangedAction.Add) 
            {
                try
                {
                    foreach (MM_T001_A item in e.NewItems)
                    {
                        ...
                        //item.ref_doc_cat = MasterEntity.ref;
                        item.ref_doc_no = MasterEntity.ref_doc;

                        if (MasterEntity.doc_cat=="MR" || MasterEntity.doc_cat == "GR")
                        {
                            item.debcr_ind = "C";
                        }
                        else
        ...
        private void CollectionChangedNotifyForBatch(object sender, NotifyCollectionChangedEventArgs e)
        {
            if (e.Action == NotifyCollectionChangedAction.Add && BatchEntity.Count > 0) // Batch can only enable to add if items exists in Items Entity.
            {
                try
                {
                    ...
                    {
                        item.ItemCode = ITEM_ENTITY_OBJ.ItemCode;
                        item.sku = ITEM_ENTITY_OBJ.sku;
                        item.comp_code = MasterEntity.comp_code;
                        item.location_Id = MasterEntity.location_Id;
                        item.store_code = ITEM_ENTITY_OBJ.store_code;
                        item.id = 0;
                        item.sr_line_no = ITEM_ENTITY_OBJ.line_id;
                        item.item_line_id = ITEM_ENTITY_OBJ.id;
                        item.t_status = ITEM_ENTITY_OBJ.t_status;
                        item.client = AppSessionState.client;
                        item.active = true;

                        BATCH_ENTITY_OBJ = item;
                    }
                }
                catch (Exception ex)
                { sms.ButtonSetup = DialogButton.Ok; sms.Caption = "Message"; sms.Text = String.Format(ex.Message, this.Title); sms.ShowMessage(); }
        ...
        private void CommandInitialisation()
        {
            ...
            cmdInsertMaterial = new RelayCommand<object>(cmdPara => { if (cmdPara == null) { return; } InsertMaterial(cmdPara, "List"); });
            cmdInsertAsset = new RelayCommand<object>(cmdPara => { if (cmdPara == null) { return; } InsertMaterialAndAsset(cmdPara, "List"); });
            cmdInsertUnit = new RelayCommand<object>(cmdPara => { if (cmdPara == null) { return; } InsertUOM(cmdPara); });
            cmdInsertStoreCode = new RelayCommand<object>(cmdPara => { if (cmdPara == null) { return; } InsertStoreCode(cmdPara); });
            cmdInsertBatch = new RelayCommand<object>(cmdPara => { if (cmdPara == null) { return; } InsertBatch(cmdPara); });
            cmdInsertBatchItem = new RelayCommand<object>(cmdPara => { if (cmdPara == null) { return; } InsertBatchItem(cmdPara); });
            ...
            cmdInsertWorkCenter = new RelayCommand<object>(cmdPara => { if (cmdPara == null) { return; } InsertWorkCenter(cmdPara); });
            ...
            cmdAssetScanned = new RelayCommand<object>(items => { if (items == null) { return; } AssetScanned(items); });
        }
        private void LoadInitialData()
        {
            try
            {
                string Request = "LOAD_INI_" + doc_cat_vm + "!@" + AppSessionState.client + "!@" + AppSessionState.OBJ_COMPANY.comp_code + "!@" + AppSessionState.OBJ_LOCATION.location_id + "!@" + doc_cat_vm + "!@" + (MasterEntity.doc_type ?? doc_cat_vm) + "!@!@" + AppSessionState.UserID + "!@" + AppSessionState.EmpId + "!@" + ts_code_vm;
                MC = REPOSITORY_OBJ_TEMP.GetDataWithReturnDomainObject<MC_MM_T001>(MC, Request, "MM_T001_BL", "MM", Request, 0, "LOAD_INI");
                ...
                List<STD_LIST_BE> WORK_CENTER_LIST_OBJ = MC.WC_LIST.Where(x => x.comp_code == AppSessionState.OBJ_COMPANY.comp_code && x.location_id == AppSessionState.OBJ_LOCATION.location_id).ToList();
                SuggestedValue = new ValueConverter(x => x == null ? "" : ((STD_LIST_BE)x).wc_code);
                TheFilter = (o, prefix) => ((STD_LIST_BE)o).wc_code.ToLower().Contains(prefix.ToLower()) || ((STD_LIST_BE)o).wc_name.ToLower().Contains(prefix.ToLower());
                AS_WORK_CENTER = new AutoSuggestTextViewModel<dynamic>(WORK_CENTER_LIST_OBJ, TheFilter, SuggestedValue, "wc_code", "wc_code", true);
                AS_WORK_CENTER.AutoSuggestVM.IsEmptyValueAllowed = true; AS_WORK_CENTER.AutoSuggestVM.IsFreeTextAllowed = false;
                ...
                AS_EQUIPMENT = new AutoSuggestTextViewModel<dynamic>(MC.EQUIPMENT_LIST, TheFilter, SuggestedValue, "asset_no", "equip_no", true);
                AS_EQUIPMENT.AutoSuggestVM.IsEmptyValueAllowed = true; AS_EQUIPMENT.AutoSuggestVM.IsFreeTextAllowed = false;
                
                ASSET_COLLECTION = MC.EQUIPMENT_LIST;
        ...
        private void LoadDocumentByDocumentNumber(object ParameterObject, string ParameterReference)
        {
            try
            {
                ...
                {
                    if (ParameterObject.GetType() == typeof(STD_LIST_BE))
                    {
                        ParameterEntityObject = (STD_LIST_BE)ParameterObject;
                        Request = "LOAD_DOC_BY_DOC_NO" + "!@" + AppSessionState.client + "!@" + (ParameterEntityObject.comp_code ?? AppSessionState.OBJ_COMPANY.comp_code) + "!@" + (ParameterEntityObject.location_id ?? AppSessionState.OBJ_LOCATION.location_id) + "!@" + (ParameterEntityObject.doc_cat ?? doc_cat_vm) + "!@" + (ParameterEntityObject.doc_type ?? doc_cat_vm) + "!@" + ParameterEntityObject.doc_no;
                        MC_TEMP = REPOSITORY_OBJ_TEMP.GetDataWithReturnDomainObject<MC_MM_T001>(MC_TEMP, Request, "MM_T001_BL", "MM", Request, 0, "LOAD_DOC_BY_DOC_NO");
                    ...
                    {
                        string strDocNo = (string)ParameterObject;
                        Request = "LOAD_DOC_BY_DOC_NO" + "!@" + AppSessionState.client + "!@" + AppSessionState.OBJ_COMPANY.comp_code + "!@" + AppSessionState.OBJ_LOCATION.location_id + "!@" + doc_cat_vm + "!@" + doc_cat_vm + "!@" + strDocNo;
                        MC_TEMP = REPOSITORY_OBJ_TEMP.GetDataWithReturnDomainObject<MC_MM_T001>(MC_TEMP, Request, "MM_T001_BL", "MM", Request, 0, "LOAD_DOC_BY_DOC_NO");
                    ...
                    else if (((IEnumerable)ParameterObject).Cast<STD_LIST_BE>().ToList().Count > 0)
                    {
                        ...
                        Request = "LOAD_DOC_BY_DOC_NO" + "!@" + AppSessionState.client + "!@" + AppSessionState.OBJ_COMPANY.comp_code + "!@" + (REQ_PARA_OBJ.location_id ?? AppSessionState.OBJ_LOCATION.location_id) + "!@" + doc_cat_vm + "!@" + (REQ_PARA_OBJ.doc_type ?? doc_cat_vm) + "!@" + ParameterEntityObject.doc_no;
                        MC_TEMP = REPOSITORY_OBJ_TEMP.GetDataWithReturnDomainObject<MC_MM_T001>(MC_TEMP, Request, "MM_T001_BL", "MM", Request, 0, "LOAD_DOC_BY_DOC_NO");
        ...
        private void WindowEvetCall(object InputValue)
        {
            try
            {
                ...
                else if (STD_LIST_OBJ!=null)
                {
                    if (STD_LIST_OBJ.request_type == "INVOKE_DOC")
                    {
        ...
        private void DefaultValues()
        {
            ...
            DateTimeOffset dto = DateTimeOffset.Now; // or your source
            MasterEntity.post_date = DateTime.SpecifyKind(dto.LocalDateTime, DateTimeKind.Unspecified);

            //MasterEntity.post_date = DateTime.Now;
            if (MasterEntity.doc_cat == "MI")
            {
                MasterEntity.mov_tp = "120";
                MasterEntity.mov_name = "Goods Issue  without Referance";
            }
            else
            {
                MasterEntity.mov_tp = "115";
                MasterEntity.mov_name = "Goods receipt  without Referance";
            }
            ...
            if (MC.DOC_TYPE_LIST != null)
            {
                ...
                {
                    MasterEntity.mov_tp = (from o in MC.DOC_TYPE_LIST where o.doc_type == MasterEntity.doc_type select o.mov_tp_mi).FirstOrDefault();
                }
            }
                ...
                foreach (var o in ItemsEntity)
                {
                    if (o.ItemCode != null && o.ItemCode != "" && o.description != null)
                    {
                        ...
                        if (o.id == 0)
                        {
                            ...
                            {
                                if (o.ItemCode == p.ItemCode && o.sku == p.sku && o.line_id==p.line_id && o.machine_id == p.machine_id)
                                {
                                    flag++;
                                }
                            }
                        ...
                        if (!string.IsNullOrWhiteSpace(o.asset_no))
                        {
                            flag = 0;
                            foreach (var p in ItemsEntity)
                            {
                                if (o.asset_no == p.asset_no)
                                {
                                    flag++;
                                }
                            }
                            if (flag > 1)
                            {
                                ...
                                showMessageService.Text = String.Format("Cannot Save Duplicate Asset/Equipment {0} for Item {1}", o.asset_no, o.ItemCode);
            ...
            if (data != null)
            {
                if (!string.IsNullOrEmpty(FLTR_STR_REF_DOC))
                {
                    return (data.ref_doc_no != null && data.ref_doc_no.ToLower().Contains(FLTR_STR_REF_DOC.ToLower()) ||
                        ...
                        data.item_code != null && data.item_code.ToLower().Contains(FLTR_STR_REF_DOC.ToLower()) ||
                        data.item_name != null && data.item_name.ToLower().Contains(FLTR_STR_REF_DOC.ToLower()) ||
        ...
        private void RemoveReferenceDocuments()
        {
            try
            {
                if (string.IsNullOrWhiteSpace(MasterEntity.ref_doc) == false && MC.REF_DOC_LIST != null)
                {
                    ...
                    {
                        MC.REF_DOC_LIST.Remove(MC.REF_DOC_LIST.FirstOrDefault(s => s.ref_doc_no == MasterEntity.ref_doc));
                    }
                }
        ...
        protected override void OnSaveAction(InquiryActionResult<MM_T001> result)
        {
            try
            {
                ...
                if (Validation() == true)
                {
                    if (NewRecord == true)
                    {
                        MasterEntity = REPOSITORY_OBJ.SaveWithReturnDomainObject<MM_T001>(MasterEntity, "MM_T001_BL", "MM");
                    ...
                    {
                        MasterEntity = REPOSITORY_OBJ.UpdateWithReturnDomainObject<MM_T001>(MasterEntity, "MM_T001_BL", "MM");
                    }
        ...
        public string ConvertDataTableToHTML()
        {
            ...
            html += "<td width=5%> <p><strong><span style=color:#000080;> Remark </span></strong></p> </td>";
            html += "</tr>";
            ...
            foreach (var item in ItemsEntity)
            {
                ...
                html += "<td width=5%> <p><strong><span style=color:#000080;> " + item.note ?? MasterEntity.notes + "</span></strong></p> </td>";
                html += "</tr>";
            }
            html += "</table>";
            ...
            try
            {
                ...
                foreach (NotificationData VarData in objNotifyData)
                {
                    List<KeyValuePair<string, string>> kvpList = new List<KeyValuePair<string, string>>()
                    {
                        ...
                        new KeyValuePair<string, string>("[Comp]", AppSessionState.OBJ_COMPANY.abrv),
                        new KeyValuePair<string, string>("[TSTS]", MasterEntity.t_display),
                        new KeyValuePair<string, string>("[Attn]",VarData.EmpName),
                        new KeyValuePair<string, string>("[CUST]",MasterEntity.Req_Name),
                        new KeyValuePair<string, string>("[DOCDATE]", MasterEntity.post_date.ToString()),
                        new KeyValuePair<string, string>("[MODDT]", DateTime.Now.ToString()),
                        new KeyValuePair<string, string>("[PREF]", (MasterEntity.ref_doc ?? "").ToString() + "  " + (MasterEntity.notes ?? "")), //MasterEntity.cust_ref_date.HasValue ? MasterEntity.cust_ref_date.Value.ToString() : string.Empty;
        ...
        protected override void OnRemoveAction(InquiryActionResult<MM_T001> result)
        {
            ...
            if (sms.ShowMessage() == DialogResult.Ok)
            {
                ...
                string response = REPOSITORY_OBJ.Delete(MasterEntity.doc_no, "MM_T001_BL", "MM");
                MasterEntity = new MM_T001();
                ItemsEntity = new ObservableCollection<MM_T001_A>();
                BatchEntity = new ObservableCollection<MM_T001_B>();
                NewRecord = true;
