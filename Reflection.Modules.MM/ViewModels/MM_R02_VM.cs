...
{
    public class MM_R02_VM : WorkspaceViewModel<STD_LIST_BE>
    {
        ...
        public static readonly DependencyProperty AutoSuggestTextViewModelProperty = DependencyProperty.Register("AutoSuggestTextViewModel", typeof(AutoSuggestTextViewModel<dynamic>), typeof(MM_R02_VM));
        ...
        #endregion
        public MM_R02_VM(string ts_code) : base()
        {
        ...
        private void UpdateClosingStock(object obj)
        {
            try
            {
                ...
                MC_TEMP = repository_MC.GetDataWithReturnDomainObject<STD_MIS_MC_BE>(MC_TEMP, Request, "MM_R02", "MM", " ", 0, "");
        ...
        private void UpdateCurrentStock(object obj)
        {
            try
            {
                ...
                MC_TEMP = repository_MC.GetDataWithReturnDomainObject<STD_MC_BE>(MC_TEMP, Request, "MM_R02", "MM", " ", 0, "");
        ...
        private void LoadInitialData()
        {
            try
            {
                ...
                MC = repository_MC.GetDataWithReturnDomainObject<STD_MIS_MC_BE>(MC, Request, "MM_R02", "MM", " ", 0, "");
        ...
        private void DisplayReport(object tem)
        {
            try
            {
                ...
                //else if (string.IsNullOrWhiteSpace(REQ_PARA_OBJ.location_id))
                //{
                //    IShowMessageViewService sms = this.GetViewService<IShowMessageViewService>();
                //    sms.ButtonSetup = DialogButton.Ok; sms.Caption = "Message"; sms.Text = String.Format("Please Select Location"); sms.ShowMessage();
                //}
                ...
                else
                {
                    if (REQ_PARA_OBJ != null)
                    {
                        ...
                        if(MC_TEMP.STD_MIS_LIST != null)
                        {
                            ...
                            {
                                if (!string.IsNullOrWhiteSpace(MC_TEMP.STD_MIS_LIST[0].qr_code))
                                {
                                    QRCodeService QRGenerator = new QRCodeService();
                                    foreach (STD_MIS_BE item in MC_TEMP.STD_MIS_LIST)
                                    {
                                        if (!string.IsNullOrWhiteSpace(item.qr_code))
                                        {
                                            item.qr_image = QRGenerator.RenderQrCodeForLabel(item.qr_code, 15, "");
                                        }
                                    }
                                }
                            }
        ...
        private Dictionary<string, string> getParametersList()
        {
            ...
            try
            {
                ...
                result.Add("comp_name", Convert.ToString(REQ_PARA_OBJ.comp_name + "   Location : " + (REQ_PARA_OBJ.location_id ?? "")));
