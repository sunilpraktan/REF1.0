...
{
    public class MM_T002_VM : WorkspaceViewModel<MM_T001>
    {
        ...
        public static readonly DependencyProperty AutoSuggestTextViewModelProperty = DependencyProperty.Register("AutoSuggestTextViewModel", typeof(AutoSuggestTextViewModel<dynamic>), typeof(MM_T002_VM));
        ...
        #endregion
        #region . Constructor .
        public MM_T002_VM(string ts_code) : base()
        {
            MoveFlag = true;
        ...
        public MM_T002_VM(string doc_cat, string ts_code) : base()
        {
            MoveFlag = true;
            this.ts_code_vm = ts_code;
        ...
        public MM_T002_VM(string doc_cat, string ts_code, string doc_no) : base()
        {
            MoveFlag = true;
            this.ts_code_vm = ts_code;
        ...
        private void LoadInitialData(string doc_cat_info, string ReferenceDoc)
        {
            try
            {
                string Request = "LOAD_INI_GM" + "!@" + AppSessionState.client + "!@" + AppSessionState.OBJ_COMPANY.comp_code + "!@" + AppSessionState.OBJ_LOCATION.location_id + "!@" + this.doc_cat_vm + "!@" + this.doc_cat_vm + "!@" + AppSessionState.EmpId;
                ...
                TheFilter = (o, prefix) => (((MM_M0004)o).mov_tp ?? "").ToString().ToLower().Contains(prefix.ToLower()) || (((MM_M0004)o).mov_tp_name ?? "").ToString().ToLower().Contains(prefix.ToLower());
        ...
        private void ExecuteReferenceDocument(object InputValue)   // this method is little complicated and lengthy need to optimize : pending optimization
        {
            try
            {
                if (REQ_PARA_OBJ.doc_no != null || REQ_PARA_OBJ.doc_no != "")
                {
                    ...
                    string Request = "EXEC_REF_DOC" + "!@" + AppSessionState.client + "!@" + AppSessionState.OBJ_COMPANY.comp_code + "!@" + AppSessionState.OBJ_LOCATION.location_id + "!@" + (MasterEntity.doc_cat ?? REQ_PARA_OBJ.doc_cat) + "!@" + (MasterEntity.doc_type ?? REQ_PARA_OBJ.doc_type) + "!@" + (REQ_PARA_OBJ.ref_doc ?? MasterEntity.ref_doc) + "!@" + (MasterEntity.source_doc_cat ?? REQ_PARA_OBJ.ref_doc_cat) + "!@" + (MasterEntity.source_doc_type ?? REQ_PARA_OBJ.ref_doc_type) + "!@" + AppSessionState.EmpId + "!@" + AppSessionState.UserID + "!@" + ts_code_vm;
                    MCTemp = repository_MC.GetDataWithReturnDomainObject<MC_MM_T001>(MCTemp, Request, "MM_T001_BL", "MM", "ExecuteReferenceDocument", 0, "");

                    if (MCTemp.MASTER_ENTITY_LIST.Count > 0)
                    {
                        MasterEntity = MCTemp.MASTER_ENTITY_LIST[0];
                        ItemsEntity = MCTemp.ITEMS_ENTITY_LIST;
                        BatchEntity = MCTemp.BATCH_ENTITY_LIST;
                    }
        ...
        private void InsertReferenceDocumentType(object InputValue)
        {
            try
            {
                ...
                if (POPUPEntityObject != null)
                {
                    ...
                    REQ_PARA_OBJ.ref_doc_cat = POPUPEntityObject.doc_cat;
                    REQ_PARA_OBJ.ref_doc_type = POPUPEntityObject.doc_cat;
                }
            }
            catch (Exception ex)
            {
        ...
        private void InsertMovementType(object InputValue)
        {
            ...
            {
                string Request = "";
                MM_M0004 POPUPEntityObject = null;
                #region Command Parameter Read Section
                try
                {
                    if (InputValue.GetType() == typeof(string) && InputValue != null)
                    ...
                    else if (InputValue != null && ((IEnumerable)InputValue).Cast<MM_M0004>().ToList().Count > 0)
                    {
                        POPUPEntityObject = ((IEnumerable)InputValue).Cast<MM_M0004>().ToList()[0];
                    }
                }
                catch (Exception ex) { }
        ...
        private void InsertItem(object InputValue)
        {
            try
            {
                ...
                if (POPUPEntityObject != null && MM_T001_A_OBJ != null)
                {
                    ...
                    {
                        Request = "Load_Batch_Data" + "!@" + AppSessionState.client + "!@" + AppSessionState.OBJ_COMPANY.comp_code + "!@" + AppSessionState.OBJ_LOCATION.location_id + "!@" + this.doc_cat_vm + "!@" + this.doc_cat_vm + "!@" + MM_T001_A_OBJ.ItemCode + "!@" + MM_T001_A_OBJ.unit_code + "!@" + MM_T001_A_OBJ.store_code;
                        MCTemp = repository_MC.GetDataWithReturnDomainObject<MC_MM_T001>(MC, Request, "MM_T001_BL", "MM", "LoadAll", 0, "");
        ...
        private void LoadDocumentByNumber(object ParameterObject, string ParameterReference)
        {
            try
            {
                ...
                NewRecord = false;
                MoveFlag = false;
                MCTemp = repository_MC.GetDataWithReturnDomainObject<MC_MM_T001>(MCTemp, Request, "MM_T001_BL", "MM", "LoadDocumentByDocumentNumber", 0, "");
        ...
        private void Load_BackFlip_Data()
        {
            try
            {
                ...
                string Request = "Load_BackFlip_Data" + "!@" + AppSessionState.client + "!@" + AppSessionState.OBJ_COMPANY.comp_code + "!@" + (REQ_PARA_OBJ.location_id ?? AppSessionState.OBJ_LOCATION.location_id) + "!@" + REQ_PARA_OBJ.doc_cat + "!@" + REQ_PARA_OBJ.doc_type + "!@" + REQ_PARA_OBJ.emp_id + "!@" + REQ_PARA_OBJ.party_code + "!@" + REQ_PARA_OBJ.t_status + "!@" + REQ_PARA_OBJ.active + "!@" + Convert.ToDateTime(REQ_PARA_OBJ.from_date).ToString("MM/dd/yyyy") + "!@" + Convert.ToDateTime(REQ_PARA_OBJ.to_date).ToString("MM/dd/yyyy");
                MCTemp = repository_MC.GetDataWithReturnDomainObject<MultipleContext_SEL_T003>(MCTemp, Request, "MM_T001_BL", "MM", "Load_BackFlip_Data", 0, "");
                ...
                if (TabIndexValue == 1)
                {
                    if (MM_T001_A_OBJ != null)
                    {
                        ...
                        if (ServerTripRequired == true && !string.IsNullOrWhiteSpace(MM_T001_A_OBJ.store_code) && !string.IsNullOrWhiteSpace(MM_T001_A_OBJ.ItemCode) && MM_T001_A_OBJ.debcr_ind == "D")
                        {
                            ...
                            MCTemp = repository_MC.GetDataWithReturnDomainObject<MultipleContext_LOG_T001_A>(MCTemp, Request, "MM_T001_BL", "MM", "LoadAll", 0, Request);
        ...
        protected override void OnSaveAction(InquiryActionResult<MM_T001> result)
        {
            try
            {
                if (Validations() == true)
                {
                    ...
                    if (NewRecord == true)
                    {
                        //MCTemp = repository_MC.SaveWithReturnDomainObject<MC_MM_T001>(MCTemp, "MM_T001_BL", "MM");
                        MasterEntity = repository.SaveWithReturnDomainObject<MM_T001>(MasterEntity, "MM_T001_BL", "MM");
                    }
                    else if (NewRecord == false)
                    {
                        //MCTemp = repository_MC.UpdateWithReturnDomainObject<MC_MM_T001>(MCTemp, "MM_T001_BL", "MM");
                        MasterEntity = repository.UpdateWithReturnDomainObject<MM_T001>(MasterEntity, "MM_T001_BL", "MM");
                    }
        ...
        private void SetBusinessEntitiesAfterLoad(string ParameterOption1, string ParameterOption2)
        {
            try
            {
                ...
                {
                    MC.ITEMS_ENTITY_LIST = (ObservableCollection<MM_T001_A>)new ObjectSerializationService().XMLToObject(MasterEntity.XmlDataDocument_MM_T001_A, MC.ITEMS_ENTITY_LIST);
                    ItemsEntity.Clear();
                ...
                {
                    MC.BATCH_ENTITY_LIST = (ObservableCollection<MM_T001_B>)new ObjectSerializationService().XMLToObject(MasterEntity.XmlDataDocument_MM_T001_B, MC.BATCH_ENTITY_LIST);
                    BatchEntity.Clear();
                    BatchEntity = MC.BATCH_ENTITY_LIST;
                }
                else
                {
                    BatchEntity = new ObservableCollection<MM_T001_B>();
        ...
        protected override void OnFlipAction(InquiryActionResult<MM_T001> result)
        {
            ...
            MCTemp = repository_MC.GetDataWithReturnDomainObject<MC_MM_T001>(MC, Request, "MM_T001_STD", "SCM", "RefreshData", 0, "");
