...
{
    class MM_T001_GRN_VM2 : WorkspaceViewModel<MM_T001>
    {
        #region AutoSuggest TextBox Region
        public static readonly DependencyProperty AutoSuggestTextViewModelProperty = DependencyProperty.Register("AutoSuggestTextViewModel", typeof(AutoSuggestTextViewModel<dynamic>), typeof(MM_T001_GRN_VM2));
        ...
        List<string> _strListstatus;
        public List<string> StringListstatus
        {
        ...
        #region . Constructor .
        public MM_T001_GRN_VM2(string ts_code)
                : base()
        {
        ...
        public MM_T001_GRN_VM2(string ts_code,string doc_no)
                : base()
        {
            this.ts_code_vm = ts_code;
        ...
        private void LoadInitialData()
        {
            try
            {
                MasterEntity.doc_cat = "GR";
                MasterEntity.doc_type = "GR";
                string Request = "LoadInitialData" + "!@" + AppSessionState.comp_code + "!@" + AppSessionState.location_Id + "!@" + MasterEntity.doc_type + "!@" + MasterEntity.doc_cat + "!@" + "101,118,119,115" + "!@" + AppSessionState.EmpId + "!@" + AppSessionState.client;
                ...
                var movtpfilter = (from o in MC.MovementTypeList where o.mov_tp == "101" || o.mov_tp == "117" || o.mov_tp == "115" select o).ToList();
                ...
                StringListMovType = movtpfilter.Select(x => x.mov_tp).ToList();
                ...
                //StringListMake = Typelist.Select(x => x.parametervalue).ToList();
                ...
                StatusCollection = CollectionViewSource.GetDefaultView(MC.t_statusList);
                StatusCollection.Filter = new Predicate<object>(Filter_Status);
                StringListstatus = MC.t_statusList.Select(x => x.t_status).ToList();
            ...
            try
            {
                ...
                if (ParameterObject != null)
                {
                    if (((IEnumerable)ParameterObject).Cast<MM_T001_FLIP>().ToList().Count > 0)
                    {
                        ...
                        if (ItemDetailsEntity.Count > 0 && dgSelectedIndexItem != -1)
                        {
                            TotalQty = 0;
                            TotalQty = Convert.ToDecimal(ItemDetailsEntity.Where(x => x.active == true).Sum(x => x.qty));
                        }
        ...
        private void InsertStatus(object InputValue)
        {
            try
            {
                ...
                try
                {
                    ...
                    else if (InputValue != null && ((IEnumerable)InputValue).Cast<SYS_M025>().Count() > 0)
                    {
        ...
        void ModelUpdated_Item(object sender, EventArgs e)
        {
            try
            {
                ...
                if (sender.ToString() == "qty" && dgSelectedIndexItem != -1 && dgSelectedIndexItem < ItemDetailsEntity.Count)
                {
                    ...
                    ItemDetailsEntity[dgSelectedIndexItem].challan_qty = ItemDetailsEntity[dgSelectedIndexItem].qty;
                    TotalQty = 0;
                    TotalQty = Convert.ToDecimal(ItemDetailsEntity.Where(x => x.active == true).Sum(x => x.qty));
                }
            }
        ...
        private void FilterPoAllocationDataGrid()
        {
            try
            {
                if (POAllocationEntity != null && POAllocationEntity.Count > 0 && dgSelectedIndexItem >= 0 && ItemDetailsEntity != null && ItemDetailsEntity.Count > 0 && dgSelectedIndexPOAllocation != -1)
                {
        ...
        private bool Validations()
        {
            ...
            foreach (var o in ItemDetailsEntity)
            {
                ...
                if (o.ItemCode != null && o.ItemCode != "")
                {
                    ...
                    if (o.para1 == null || o.para1 == "")
                    {
                        ...
                        showMessageService.ButtonSetup = DialogButton.Ok;
                        showMessageService.Caption = "Message";
                        showMessageService.Text = String.Format("Please Enter Make for the item {0} and Parameter {1} At Index {2}", o.ItemCode, o.sku_desc, ItemDetailsEntity.IndexOf(o));
                        showMessageService.ShowMessage();
                        return false;
                    }
                    if (o.para2 == null || o.para2 == "")
                    {
                        ...
                        showMessageService.ButtonSetup = DialogButton.Ok;
                        showMessageService.Caption = "Message";
                        showMessageService.Text = String.Format("Please Enter Type for the item {0} and Parameter {1} At Index {2}", o.ItemCode, o.sku_desc, ItemDetailsEntity.IndexOf(o));
                        showMessageService.ShowMessage();
                        return false;
                    }
                    if (o.mat_con == null || o.mat_con == "")
                    {
                        ...
                        showMessageService.ButtonSetup = DialogButton.Ok;
                        showMessageService.Caption = "Message";
                        showMessageService.Text = String.Format("Please Enter Material Condition for the item {0} and Parameter {1} At Index {2}", o.ItemCode, o.sku_desc, ItemDetailsEntity.IndexOf(o));
                        showMessageService.ShowMessage();
                        return false;
                    }
                }
            ...
            #region . Duplicate Entry Checking .
            bool anyDuplicates = ItemDetailsEntity.Select(x => new { x.ItemCode, x.sku, x.machine_id, x.batch_no }).Distinct().Count()
        ...
        private void InsertMake(object InputValue, bool NewRow, bool AllowDuplicate, bool AllowModify)
        {
            try
            {
                ...
                if (POPUPEntityObject != null) // Only enter in the code block if ENtity Not null.
                {
                    var InputValueIfExists = ItemDetailsEntity.Where(X => X.para1 == POPUPEntityObject.parametervalue).FirstOrDefault(); // Prefer Primary Key for this instruction.
                    int IndexOfExistValue = ItemDetailsEntity.IndexOf(ItemDetailsEntity.Where(X => X.para1 == POPUPEntityObject.parametervalue).FirstOrDefault()); // Prefer Primary/Unique Key of Row for this instruction.

                    if (dgSelectedIndexItem >= 0 && ItemDetailsEntity.Count > dgSelectedIndexItem) //Update Row: If Old or Blank row get added, then Edit Rule will applicable in this Loop.
                    {
                        if (ItemDetailsEntity[dgSelectedIndexItem].id == 0 && ((IndexOfExistValue == -1) || (IndexOfExistValue != -1 && AllowDuplicate == true))) // It will allowe to add unique value or Duplicate value if AllowDuplicate Status == True, id=0 indicates it will add or edit for New record only.
                        {
                            ItemDetailsEntity[dgSelectedIndexItem].para1 = POPUPEntityObject.parametervalue;
                        }
                        else if (ItemDetailsEntity[dgSelectedIndexItem].para1 != POPUPEntityObject.parametervalue)
                        {
                            ItemDetailsEntity[dgSelectedIndexItem].para1 = "";
                        }
                    }
                    #region Clear Empty Row
                    MM_T001_A newObj = new MM_T001_A();
        ...
        #region . Status .
        private string _filterString_Status; //Sattus
        public string filterString_Status
        {
            ...
            set
            {
                ...
                FilterCollection_Status();
            }
        }
        private void FilterCollection_Status()
        {
            if (StatusCollection != null)
            {
                StatusCollection.Refresh();
            }
        }
        public bool Filter_Status(object obj)
        {
            ...
            if (data != null)
            {
                ...
                {
                    return (data.t_status != null && data.t_status.ToLower().Contains(_filterString_Status.ToLower()) ||
                        data.t_display != null && data.t_display.ToLower().Contains(_filterString_Status.ToLower())
                        );
                }
                return true;
            }
