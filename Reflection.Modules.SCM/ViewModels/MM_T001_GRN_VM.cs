...
{
    public class MM_T001_GRN_VM : WorkspaceViewModel<MM_T001>
    {
        #region AutoSuggest TextBox Region
        public static readonly DependencyProperty AutoSuggestTextViewModelProperty = DependencyProperty.Register("AutoSuggestTextViewModel", typeof(AutoSuggestTextViewModel<dynamic>), typeof(MM_T001_GRN_VM));
        ...
        #region . Constructor .
        public MM_T001_GRN_VM(string ts_code) : base()
        {
            ...
            BatchDetailsEntity.CollectionChanged += new System.Collections.Specialized.NotifyCollectionChangedEventHandler(CollectionChangedNotifyForBatch);
            //POAllocationEntity.CollectionChanged += new System.Collections.Specialized.NotifyCollectionChangedEventHandler(CollectionChangedNotifyForPoAllocation); //deleted as per requirement : CRI
        ...
        public MM_T001_GRN_VM(string ts_code, string doc_no) : base()
        {
            parameter = false; MoveFlag = true;
            this.ts_code_vm = ts_code;
        ...
        private void DefaultValues()
        {
            ...
            MasterEntity.source_doc_type = "PO";
        ...
        private void LoadInitialData()
        {
            try
            {
                MasterEntity.doc_cat = "GR";
                MasterEntity.doc_type = "GR";
                string Request = "LoadInitialData" + "!@" + AppSessionState.comp_code + "!@" + AppSessionState.location_Id + "!@" + MasterEntity.doc_type + "!@" + MasterEntity.doc_cat + "!@" + "101,118,119,115,117,127,128" + "!@" + AppSessionState.EmpId + "!@" + AppSessionState.client;
                ...
                var movtpfilter = (from o in MC.MovementTypeList where o.mov_tp != "106" && o.mov_tp != "116" select o).ToList();
                MovementTypeCollection = CollectionViewSource.GetDefaultView(movtpfilter);
                ...
                ItemCollection = CollectionViewSource.GetDefaultView(MC.ItemPopupList);
                ItemCollection.Filter = new Predicate<object>(Filter_Item);
                StringListItem = MC.ItemPopupList.Select(x => x.ItemCode).ToList();
                ...
                //Filters AutoSuggest
                SuggestedValue = new ValueConverter(x => x == null ? "" : ((SYS_M025)x).t_display);
                TheFilter = (o, prefix) => (((SYS_M025)o).t_display ?? "").ToString().ToLower().Contains(prefix.ToLower()) || (((SYS_M025)o).t_status ?? "").ToString().ToLower().Contains(prefix.ToLower());
                ASFltrt_status = new AutoSuggestTextViewModel<dynamic>(MC.t_statusList, TheFilter, SuggestedValue, "t_display", true);
                ...
                ASFltrSoldToParty = new AutoSuggestTextViewModel<dynamic>(MC.PartyList, TheFilter, SuggestedValue, "PartyNm", true);
                ...
                TheFilter = (o, prefix) => ((SYS_M007_P)o).doc_type_user.ToString().ToLower().Contains(prefix.ToLower());
        ...
        private void LoadDocumentByRefDocNo(object InputValue)    // this method is little complicated and lengthy need to optimize : pending optimization
        {
            try
            {
                if (MasterEntity.source_doc_no != null || MasterEntity.source_doc_no != "")
                {
                    ...
                    {
                        string doc_notemp = MasterEntity.doc_no;
                        string movtypetemp = MasterEntity.mov_tp;
                        string movtypenmtemp = MasterEntity.mov_name;
                        ...
                        {
                            if (movtypetemp == null || movtypetemp == "")
                            {
                                MasterEntity.mov_tp = "101";
                                MasterEntity.mov_name = "Goods Receipt for Purchase Order";
                                MasterEntity.client = AppSessionState.client;
                            }
                            if (MasterEntity.PartyId != null && MasterEntity.PartyId != "" && MasterEntity.PartyId != MCTemp.GRNMasterList[0].PartyId && (MasterEntity.doc_no == null || MasterEntity.doc_no == ""))
                            {
                                IShowMessageViewService showMessageService = this.GetViewService<IShowMessageViewService>();
                                showMessageService.ButtonSetup = DialogButton.OkCancel;
                                showMessageService.Caption = "Message";
                                showMessageService.Text = String.Format("Vendor/Supplier Does Not Match With Vendor/Supplier of PO you are Loading.\n This Will Delete Existing Vendor/Supplier With Existing Items, Batches As Well As PO Allocation\n Click Ok to Continue or Click Cancel to Cancel PO Loading ", this.Title);
                                showMessageService.ShowMessage();
                                ...
                                if (showMessageService.ShowMessage() == DialogResult.Ok)
                                {
                                    ...
                                    FilterString_RefDocNo = MasterEntity.PartyId;

                                    DefaultValues();

                                    MasterEntity.doc_no = doc_notemp;
                                    MasterEntity.mov_tp = movtypetemp;
                                    MasterEntity.mov_name = movtypenmtemp;
                                    MasterEntity.tr_mode = transmodetemp;
                                    MasterEntity.client = AppSessionState.client;

                                    foreach (var o in MCTemp.ItemDetailsList)
                                    {
                                        var InputValueIfExists = ItemDetailsEntity.Where(X => X.ItemCode == o.ItemCode && X.sku == o.sku && X.po_no == o.source_doc_no).FirstOrDefault(); // Prefer Primary Key for this instruction.
                                        int IndexOfExistValue = ItemDetailsEntity.IndexOf(ItemDetailsEntity.Where(X => X.ItemCode == o.ItemCode && X.sku == o.sku && X.po_no == o.source_doc_no).FirstOrDefault()); // Prefer Primary/Unique Key of Row for this instruction.
                                        ...
                                        if (IndexOfExistValue1 == -1)
                                        {
                                            POAllocationEntity.Add(new MM_T001_C()
                                            {
                                                ...
                                                allocated_qty = Convert.ToDecimal(allocatedqty),
                                                rec_qty = o.qty,
                                                active = true,
                            ...
                            else if (MasterEntity.PartyId != null && MasterEntity.PartyId != "" && MasterEntity.PartyId == MCTemp.GRNMasterList[0].PartyId)
                            {
                                MasterEntity = MCTemp.GRNMasterList[0];
                                FilterString_RefDocNo = MasterEntity.PartyId;
                                DefaultValues();
                            ...
                            else if ((MasterEntity.PartyId == null || MasterEntity.PartyId == "") && (MasterEntity.doc_no == null || MasterEntity.doc_no == null))
                            {
                                MasterEntity = MCTemp.GRNMasterList[0];
                                FilterString_RefDocNo = MasterEntity.PartyId;
                                DefaultValues();
                            ...
                            else
                            {
                                IShowMessageViewService showMessageService = this.GetViewService<IShowMessageViewService>();
                                showMessageService.ButtonSetup = DialogButton.OkCancel;
                                showMessageService.Caption = "Loading Failes...!!";
                                showMessageService.Text = String.Format("You are Not Allowed to Load PO of Different Vendor After GRN No is Generated.", this.Title);
                                showMessageService.ShowMessage();
                            }
                        }
                    }
        ...
        private void LoadDocumentByDocumentNumber(object ParameterObject, string ParameterReference)
        {
            try
            {
                ...
                {
                    Request = "GRNDetails" + "!@" + ParameterObject.ToString();
                }
                else
                {
                    if (((IEnumerable)ParameterObject).Cast<MM_T001_FLIP>().ToList().Count > 0)
                    {
                        ...
                        Request = "GRNDetails" + "!@" + ParameterEntityObject.doc_no;
                    }
                }
                NewRecord = false;
                parameter = false;
                MoveFlag = false;
                MCTemp = repository_MC.GetDataWithReturnDomainObject<MC_MM_T001>(MCTemp, Request, "GoodsReceiptNote", "SCM", "GRNDetails", 0, "");
        ...
        private void LoadHistory()
        {
            try
            {
                ...
                var msg = new NotificationMessage("MM_T001_GRN_VM");
        ...
        private void LoadRecords()
        {
            ...
            else
            {
                ...
                string Request = "LoadRecordsofSelectedDate" + "!@" + AppSessionState.comp_code + "!@" + AppSessionState.location_Id + "!@" + MasterEntity.doc_type + "!@" + MasterEntity.doc_cat + "!@" + "101,118,119,115" + "!@" + AppSessionState.EmpId + "!@" + Convert.ToDateTime(MasterEntity.bf_FromDate).ToString("MM/dd/yyyy") + "!@" + Convert.ToDateTime(MasterEntity.bf_ToDate).ToString("MM/dd/yyyy");
        ...
        private void InsertParty(object InputValue)
        {
            try
            {
                ...
                if (POPUPEntityObject != null) //Application:New Record Only.Condition: Only enter in the code block if EntityObject is Not null and not assigning same value again if event fire twice.
                {
                    ...
                    MasterEntity.vendor = POPUPEntityObject.PartyNm;
                    FilterString_RefDocNo = MasterEntity.PartyId;
                }
            }
            catch (Exception ex)
        ...
        private void InsertMovementType(object InputValue)
        {
            try
            {
                ...
                if (POPUPEntityObject != null) //Application:New Record Only.Condition: Only enter in the code block if EntityObject is Not null and not assigning same value again if event fire twice.
                {
                    ...
                    {
                        var refdoctype = (from o in MC.DocTypeList where o.doc_cat == "PO" select o).ToList();
                        RefDocTypeCollection = CollectionViewSource.GetDefaultView(refdoctype.ToList());
                        RefDocNoCollection.Refresh();

                        DistinctRefDocNo = MC.SourceDocNoList.Where(x => x.doc_cat == "PO").OrderByDescending(PUR_T002_A_P => PUR_T002_A_P.po_no).GroupBy(PUR_T002_A_P => PUR_T002_A_P.po_no).Select(g => g.First()).ToList();
                        RefDocNoCollection = CollectionViewSource.GetDefaultView(DistinctRefDocNo.ToList());
                        RefDocNoCollection.Filter = new Predicate<object>(Filter_RefDocNo);
                        RefDocNoCollection.Refresh();
                    }
                    else if (MasterEntity.mov_tp == "117")
                    {
                        var refdoctype = (from o in MC.DocTypeList where o.doc_cat == "DN" select o).ToList();
                        RefDocTypeCollection = CollectionViewSource.GetDefaultView(refdoctype.ToList());
                        RefDocTypeCollection.Refresh();

                        DistinctRefDocNo = MC.SourceDocNoList.Where(x => x.doc_cat == "DN").OrderByDescending(PUR_T002_A_P => PUR_T002_A_P.po_no).GroupBy(PUR_T002_A_P => PUR_T002_A_P.po_no).Select(g => g.First()).ToList();
                        RefDocNoCollection = CollectionViewSource.GetDefaultView(DistinctRefDocNo.ToList());
                        RefDocNoCollection.Filter = new Predicate<object>(Filter_RefDocNo);
                        RefDocNoCollection.Refresh();
                    }
                    else if (MasterEntity.mov_tp == "119")
                    {
                        var refdoctype = (from o in MC.DocTypeList where o.doc_cat == "RO" select o).ToList();
                        RefDocTypeCollection = CollectionViewSource.GetDefaultView(refdoctype.ToList());
                        RefDocTypeCollection.Refresh();
                        MasterEntity.source_doc_type = "RO";

                        DistinctRefDocNo = MC.SourceDocNoList.Where(x => x.doc_cat == "RO").OrderByDescending(PUR_T002_A_P => PUR_T002_A_P.po_no).GroupBy(PUR_T002_A_P => PUR_T002_A_P.po_no).Select(g => g.First()).ToList();
                        RefDocNoCollection = CollectionViewSource.GetDefaultView(DistinctRefDocNo.ToList());
                        RefDocNoCollection.Filter = new Predicate<object>(Filter_RefDocNo);
                        RefDocNoCollection.Refresh();
                    }
        ...
        private void InsertRedDocType(object InputValue)                  //ref doc type
        {
            try
            {
                ...
                if (POPUPEntityObject != null) // Only enter in the code block if ENtity Not null.
                {
                    ...
                    else if (dgSelectedIndexItem >= 0 && ItemDetailsEntity.Count > dgSelectedIndexItem) //Update Row: If Old or Blank row get added, then Edit Rule will applicable in this Loop.
                    {
                        if (ItemDetailsEntity[dgSelectedIndexItem].id == 0 && ((IndexOfExistValue == -1) || (IndexOfExistValue != -1 && AllowDuplicate == true))) // It will allowe to add unique value or Duplicate value if AllowDuplicate Status == True
                        {
                            ...
                            ItemDetailsEntity[dgSelectedIndexItem].t_status = "001";
                            ItemDetailsEntity[dgSelectedIndexItem].t_display = (from o1 in MC.t_statusList where o1.t_status == MasterEntity.t_status select o1.t_display).FirstOrDefault();
        ...
        private void GetSkuDescription()
        {
            try
            {
                if (ItemDetailsEntity[dgSelectedIndexItem].sku_desc == null || ItemDetailsEntity[dgSelectedIndexItem].sku_desc == "")
                {
                    for (int i = 0; i < SelectedParaValueCollection.Count; i++)
                    {
                        if ((ItemDetailsEntity[dgSelectedIndexItem].sku_desc == "" || ItemDetailsEntity[dgSelectedIndexItem].sku_desc == null) && (String.IsNullOrEmpty(SelectedParaValueCollection[i].parametervalue) || SelectedParaValueCollection[i].parametervalue.Trim() != "NA"))
                        {
        ...
        private bool Validations()
        {
            try
            {
                ...
                if (MasterEntity.PartyId == null || MasterEntity.PartyId == "")
                {
                    ...
                    showMessageService.ButtonSetup = DialogButton.Ok;
                    showMessageService.Caption = "Validation";
                    showMessageService.Text = String.Format("Please select Vendor Or Supplier");
                    showMessageService.ShowMessage();
                    return false;
                }
                ...
                foreach (var o in ItemDetailsEntity)
                {
                    ...
                    {    // Item Duplication Validation
                        ...
                        {
                            if (o.ItemCode == p.ItemCode && o.sku == p.sku && o.po_no == p.po_no && p.active == true)
                            {
                                flag++;
                            }
                        }
        ...
        private void RemoveRefDoc()
        {
            try
            {
                foreach (MM_T001_A item in ItemDetailsEntity)
                {
                    if (item.ref_doc_no != null)
                    {
                        MC.SourceDocNoList.RemoveAll(X => X.po_no == item.source_doc_no);
                    }
                }
                if (MC.SourceDocNoList.Count > 0)
                {
                    DistinctRefDocNo = MC.SourceDocNoList.GroupBy(PUR_T002_A_P => PUR_T002_A_P.po_no).Select(g => g.First()).ToList();
                    RefDocNoCollection = CollectionViewSource.GetDefaultView(DistinctRefDocNo.ToList());
                    RefDocNoCollection.Filter = new Predicate<object>(Filter_RefDocNo);
        ...
        protected override void OnCreateAction(InquiryActionResult<MM_T001> result)
        {
            try
            {
                NewRecord = true; parameter = false; MoveFlag = true;
                MasterEntity = new MM_T001();
                ItemDetailsEntity = new ObservableCollection<MM_T001_A>();
                BatchDetailsEntity = new ObservableCollection<MM_T001_B>();
                POAllocationEntity = new ObservableCollection<MM_T001_C>();
