...
{
    public class LOG_T001_A_VM_Service : WorkspaceViewModel<LOG_T001_A>
    {
        ...
        #region . Constructor .
        public LOG_T001_A_VM_Service(string ts_code) : base()
        {
            parameter = false;
            ...
            SelectedLOG_T001_A.shipment_mode = "By Road";
            SelectedLOG_T001_A.doc_type = "DN";
            SelectedLOG_T001_A.doc_cat = "DN";
            SelectedLOG_T001_A.ladding_date = DateTime.Now;
            SelectedLOG_T001_A.delivery_type = "RM";
            SelectedLOG_T001_A.del_desc = "Repair and Maintainance";
            SelectedLOG_T001_A.ref_doc_type = "GR";
            SelectedLOG_T001_A.active = true;
            SelectedLOG_T001_A.client = AppSessionState.client;
        ...
        public LOG_T001_A_VM_Service(string ts_code,string doc_no) : base()
        {
            parameter = false;
            this.ts_code_vm = ts_code;
        ...
        private void LoadInitialData()
        {
            try
            {
                ...
                string Request = "LoadInitialData" + "!@" + AppSessionState.comp_code + "!@" + AppSessionState.location_Id.ToString() + "!@" + SelectedLOG_T001_A.doc_type + "!@" + SelectedLOG_T001_A.doc_cat
                    + "!@" + "RM" + "!@" + "RM" + "!@" + AppSessionState.EmpId + "!@" + AppSessionState.client;
                ...
                var refdoctemp = (from o in MC.DocCategoryList where o.doc_type == "DN" || o.doc_type == "DC" || o.doc_type == "DE" || o.doc_type == "FD" select o).ToList();
                ...
                if (Convert.ToString(items) == SelectedLOG_T001_A.order_no)
                {
                    ...
                    else
                    {
                        ...
                        if (MCTemp.OrderDataList.Count > 0)
                        {
                            ...
                            else if (SelectedLOG_T001_A.ref_doc_cat == "GR")
                            {
                                SelectedLOG_T001_A.delivery_type = "RM";
                            }
                            else
                ...
                if (ItemDetails != null)
                {
                    ...
                    else if (dgSelectedIndex >= 0 && Del_Note_Dtails.Count > dgSelectedIndex && ItemDetails.Select == true) //Update Row: If Old or Blank row get added, then Edit Rule will applicable in this Loop.
                    {
                        if (Del_Note_Dtails[dgSelectedIndex].ItemCode == null && ((IndexOfExistValue == -1) || (IndexOfExistValue != -1 && AllowDuplicate == true))) // It will allowe to add unique value or Duplicate value if AllowDuplicate Status == True
                        {
                            ...
                            Del_Note_Dtails[dgSelectedIndex].order_item_code = ItemDetails.ItemCode;
                        }
        ...
        private void FilterBatchDataGrid()
        {
            try
            {
                if (ItemBatch_list != null && ItemBatch_list.Count > 0 && dgSelectedIndex >= 0 && Del_Note_Dtails != null && Del_Note_Dtails.Count > 0)
                {
                    ...
                    {
                        DataGridView.Filter = adv => ((LOG_T001_C)adv).ItemCode.Equals(Del_Note_Dtails[dgSelectedIndex].ItemCode) && ((LOG_T001_C)adv).sku.Equals(Del_Note_Dtails[dgSelectedIndex].sku);
                    }
        ...
        private bool Validation()
        {
            ...
            if ((SelectedLOG_T001_A.tr_party == null || SelectedLOG_T001_A.tr_party == "") && (SelectedLOG_T001_A.shipment_mode == null || SelectedLOG_T001_A.shipment_mode == ""))
            {
                IShowMessageViewService showMessageService = this.GetViewService<IShowMessageViewService>();
                showMessageService.ButtonSetup = DialogButton.Ok;
            ...
            else if (SelectedLOG_T001_A.shipment_mode == null || SelectedLOG_T001_A.shipment_mode == "")
            {
                IShowMessageViewService showMessageService = this.GetViewService<IShowMessageViewService>();
                showMessageService.ButtonSetup = DialogButton.Ok;
                showMessageService.Caption = "Warning";
            ...
            foreach (var o in Del_Note_Dtails)
            {
                ...
                if (o.id == 0 && o.active == true)
                {
                    ...
                    if (IndexOfExistValue == -1)
                    {
                        ...
                        string unit = MC.ItemList.Where(X => X.ItemCode == o.ItemCode).Select(x => x.unit_code).FirstOrDefault();
                        ...
                        else
                        {
                            ...
                            string baseunit = MC.ConFactorList.Where(X => X.unit_code == o.unit_code).Select(x => x.base_unit_code).FirstOrDefault();
        ...
        private void RemoveRefDoc()
        {
            try
            {
                ...
                {
                    MC.OrderList.RemoveAll(X => SelectedLOG_T001_A.order_no.Contains(X.order_no));
                }
                var msg = new NotificationMessage("LOG_T001_A_VM_Service");
        ...
        protected override void OnDocumentAction()
        {
            try
            {
                if (!string.IsNullOrEmpty(SelectedLOG_T001_A.delivery_no))
                {
                    //this.IsDocumentViewerShow = !IsDocumentViewerShow;
                    Messenger.Default.Send<DocumentViewerPayload>(new DocumentViewerPayload() { DocumentNumber = SelectedLOG_T001_A.delivery_no.ToString().Replace("/", "--"), DocumentList = MCTemp.Attachment, client = AppSessionState.client, comp_code = (SelectedLOG_T001_A.comp_code ?? AppSessionState.comp_code) });
                }
            }
            catch (Exception ex) { }
        }
