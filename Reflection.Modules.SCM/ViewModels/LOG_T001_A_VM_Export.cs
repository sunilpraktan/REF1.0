...
{
    class LOG_T001_A_VM_Export : WorkspaceViewModel<LOG_T001_A>
    {
        ...
        MultipleContext_LOG_T001_A MCTempItem = new MultipleContext_LOG_T001_A();
        ...
        private ICollectionView _TransportModeCollection;
        public ICollectionView TransportModeCollection
        {
            get { return _TransportModeCollection; }
            set { _TransportModeCollection = value; RaisePropertyChanged("TransportModeCollection"); }
        }
        ...
        #region . Constructor .
        public LOG_T001_A_VM_Export(string ts_code)
            : base()
        {
            parameter = false;
            ...
            MCTempItem = new MultipleContext_LOG_T001_A();
        ...
        public LOG_T001_A_VM_Export(string ts_code,string doc_no)
            : base()
        {
            parameter = false;
            this.ts_code_vm = ts_code;
        ...
        private void LoadInitialData()
        {
            try
            {
                ...
                SelectedLOG_T001_A.doc_type = "DE";
                SelectedLOG_T001_A.doc_cat = "DE";
                string Request = "LoadInitialData" + "!@" + AppSessionState.comp_code + "!@" + AppSessionState.location_Id.ToString() + "!@" + SelectedLOG_T001_A.doc_type + "!@" + SelectedLOG_T001_A.doc_cat
                    + "!@" + "" + "!@" + "OE" + "!@" + AppSessionState.EmpId + "!@" + AppSessionState.client;
              ...
              items =>
              {
                ...
                SelectedLOG_T001_A.shipment_mode = "By Air";
                SelectedLOG_T001_A.delivery_type = "OE";
                SelectedLOG_T001_A.ref_doc_cat = "PI";
                SelectedLOG_T001_A.ref_doc_type = "PI";
                SelectedLOG_T001_A.ladding_date = DateTime.Now;
                SelectedLOG_T001_A.pack_rem = "PACKING@10000 Pcs.In one pouch@2 pvc pouches in a box@5 inner cartons in one export carton.@@DIMENSION@ 345X280X170 MM";
                SelectedLOG_T001_A.pack_rem = SelectedLOG_T001_A.pack_rem.Replace("@", System.Environment.NewLine);
                ...
                var refdoctemp = (from o in MC.DocCategoryList where o.doc_type == "DE" || o.doc_type == "DC" select o).ToList();
                ...
                var pilist = (from o in MC.OrderList where o.doc_cat == "PI" select o).ToList();

                S_OrderCollection = CollectionViewSource.GetDefaultView(pilist.ToList());
                S_OrderCollection.Filter = new Predicate<object>(S_OrderFilter);
                StringListOrderNo = pilist.Select(x => x.order_no).ToList();
                ...
                IncotermsCollection = CollectionViewSource.GetDefaultView(MC.IncoTermsList);       // added by madhuri
                ...
                TransportModeCollection = CollectionViewSource.GetDefaultView(MC.TransportMode);
                TransportModeCollection.Filter = new Predicate<object>(Filter_Tr_mode);
                //StringListTransportMode = MC.TransportMode.Select(x => x.tr_mode).ToList();
                ...
                if (Convert.ToString(items) == SelectedLOG_T001_A.order_no)
                {
                    ...
                    else
                    {
                        ...
                        if (MCTemp.OrderDataList.Count > 0)
                        {
                            ...
                            {
                                if (MCTempItem.ItemList == null)
                                {
                                    MCTempItem.ItemList = MCTemp.ItemList;
                                    ItemsCollection = CollectionViewSource.GetDefaultView(MCTempItem.ItemList);
                                    ItemsCollection.Filter = new Predicate<object>(ItemsFilter);
                                }
                                else if (MCTempItem.ItemList.Count >= 0)
                                {
                                    foreach (ADM_M022_P item in MCTemp.ItemList)
                                    {
                                        var InputValueIfExists = MCTempItem.ItemList.Where(X => X.ref_item_row_id == item.ref_item_row_id).FirstOrDefault(); // Prefer Primary Key for this instruction.
                                                                                                                                                             //int IndexOfExistValue = MCTempItem.ItemList.IndexOf(MCTempItem.ItemList.Where(X => X.ref_item_row_id == item.ref_item_row_id).FirstOrDefault()); // Prefer Primary Key for this instruction.
                                        if (InputValueIfExists == null)
                                        {
                                            MCTempItem.ItemList.Add(item);
                                        }
                                    }
                                    ItemsCollection = CollectionViewSource.GetDefaultView(MCTempItem.ItemList);
                                    ItemsCollection.Filter = new Predicate<object>(ItemsFilter);
            ...
            try
            {
                ...
                if (POPUPEntityObject != null) // Only enter in the code block if ENtity Not null.
                {
                    ...
                    if (dgSelectedIndex >= 0 && Del_Note_Dtails.Count > dgSelectedIndex) //Update Row: If Old or Blank row get added, then Edit Rule will applicable in this Loop.
                    {
                        ...
                        {
                            #region con factor try
                            //if ( Del_Note_Dtails[dgSelectedIndex].unit_code != null && Del_Note_Dtails[dgSelectedIndex].unit_code != "" &&
                            //    Del_Note_Dtails[dgSelectedIndex].qty != null && Del_Note_Dtails[dgSelectedIndex].qty != 0)
                            //{
                            //    string baseunit = (from o in MC.ConFactorList where o.unit_code == Del_Note_Dtails[dgSelectedIndex].unit_code select o.base_unit_code).ToString();
                            //    decimal cfactor = Convert.ToDecimal(from o in MC.ConFactorList where o.unit_code == Del_Note_Dtails[dgSelectedIndex].unit_code && o.base_unit_code == baseunit select o.c_factor);

                            //    if (POPUPEntityObject.unit_code == baseunit)
                            //    {
                            //        Del_Note_Dtails[dgSelectedIndex].qty *=  cfactor;
                            //    }
                            //    else
                            //    {
                            //        Del_Note_Dtails[dgSelectedIndex].qty *= cfactor;

                            //        decimal cfactordest = Convert.ToDecimal(from o in MC.ConFactorList where o.unit_code == POPUPEntityObject.unit_code && o.base_unit_code == baseunit select o.c_factor);
                            ...
                            //if ( Del_Note_Dtails[dgSelectedIndex].unit_code != null && Del_Note_Dtails[dgSelectedIndex].unit_code != "" &&
                            //    Del_Note_Dtails[dgSelectedIndex].net_price != null && Del_Note_Dtails[dgSelectedIndex].net_price != 0)
                            //{
                            ...
                            //    if (POPUPEntityObject.unit_code == baseunit)
                            //    {
                            //        Del_Note_Dtails[dgSelectedIndex].net_price *= cfactor;
                            //    }
                            //    else
                            //    {
                            //        Del_Note_Dtails[dgSelectedIndex].net_price *= cfactor;
                            ...
                            if (Del_Note_Dtails[dgSelectedIndex].unit_code == "MILLION" && POPUPEntityObject.unit_code == "THOUSAND")
                            {
                                if (Del_Note_Dtails[dgSelectedIndex].qty != null && Del_Note_Dtails[dgSelectedIndex].qty != 0)
                                {
                                    Del_Note_Dtails[dgSelectedIndex].qty *= 1000;
                                }

                                if (Del_Note_Dtails[dgSelectedIndex].net_price != null && Del_Note_Dtails[dgSelectedIndex].net_price != 0)
                                {
                                    Del_Note_Dtails[dgSelectedIndex].net_price /= 1000;
                                }
                            }
        ...
        private void GetSelectedBatchForItem(object InputValue, bool NewRow, bool AllowDuplicate, bool AllowModify)
        {
            try
            {
                ...
                if (BatchDetails != null)
                {
                    ...
                    var FilteredBatch = (from o in ItemBatch_list where o.ItemCode == Del_Note_Dtails[dgSelectedIndex].ItemCode &&
        ...
        private void CalculateWeightC()
        {
            try
            {
                ...
                SelectedLOG_T001_A.KindOfPkgs = "";
                SelectedLOG_T001_A.KindOfPkgs = "CARTON BOX@" + "1/" + SelectedLOG_T001_A.no_of_packages + " TO "
                    + SelectedLOG_T001_A.no_of_packages + "/" + SelectedLOG_T001_A.no_of_packages;

                SelectedLOG_T001_A.KindOfPkgs = SelectedLOG_T001_A.KindOfPkgs.Replace("@", System.Environment.NewLine);

                SelectedLOG_T001_A.net_weight = ItemBatch_list.Where(x=>x.active == true).Sum(x => x.net_wt);
                SelectedLOG_T001_A.wt_goods = ItemBatch_list.Where(x => x.active == true).Sum(x => x.gross_wt);
            }
            catch (Exception ex)
            {
                IShowMessageViewService showMessageService = this.GetViewService<IShowMessageViewService>();
        ...
        private bool Validation()
        {
            ...
            if (SelectedLOG_T001_A.no_of_packages == null || SelectedLOG_T001_A.no_of_packages == 0)
            {
                IShowMessageViewService showMessageService = this.GetViewService<IShowMessageViewService>();
                showMessageService.ButtonSetup = DialogButton.Ok;
                showMessageService.Caption = "Validation";
                showMessageService.Text = String.Format("You must enter at least 1 Box in Box Details ");
                showMessageService.ShowMessage();
                return false;
            }

            if (SelectedLOG_T001_A.KindOfPkgs == null || SelectedLOG_T001_A.KindOfPkgs == "" ||
                SelectedLOG_T001_A.tr_party == null || SelectedLOG_T001_A.tr_party == "" ||
                SelectedLOG_T001_A.tr_mode == null || SelectedLOG_T001_A.tr_mode == "" ||
                SelectedLOG_T001_A.pack_rem == null || SelectedLOG_T001_A.pack_rem == "" )
            {
                IShowMessageViewService showMessageService = this.GetViewService<IShowMessageViewService>();
                showMessageService.ButtonSetup = DialogButton.Ok;
                showMessageService.Caption = "Validation : Loading and Packing Tab";
                showMessageService.Text = String.Format("Compulsory fields in this tab: Kind of Packages, Transporter, Shipment Mode, Packing Remark ");
                showMessageService.ShowMessage();
                return false;
            }                 

            if (SelectedLOG_T001_A.country_code == null || SelectedLOG_T001_A.country_code == "" ||
                SelectedLOG_T001_A.port_desc == null || SelectedLOG_T001_A.port_desc == "" ||
                SelectedLOG_T001_A.final_dest == null || SelectedLOG_T001_A.final_dest == "" ||
                SelectedLOG_T001_A.incoterms == null || SelectedLOG_T001_A.incoterms == "" ||
                SelectedLOG_T001_A.cf_agent_code == null || SelectedLOG_T001_A.cf_agent_code == "" ||
                SelectedLOG_T001_A.lic_cod == null || SelectedLOG_T001_A.lic_cod == "" ||
                SelectedLOG_T001_A.origion_country == null || SelectedLOG_T001_A.origion_country == "" ||
                SelectedLOG_T001_A.ship_mark == null || SelectedLOG_T001_A.ship_mark == "" ||
                SelectedLOG_T001_A.port_load == null || SelectedLOG_T001_A.port_load == "" )
            {
                IShowMessageViewService showMessageService = this.GetViewService<IShowMessageViewService>();
                showMessageService.ButtonSetup = DialogButton.Ok;
                showMessageService.Caption = "Validation : Shipping Tab";
                showMessageService.Text = String.Format("Compulsory fields in this tab: Destination Country, Port of Discharge, Final Destination, Incoterms, \n C & F Agent, EPCG licence, Advance licence, Country of Origin, Shipping Mark, Port of Loading ");
                showMessageService.ShowMessage();
                return false;
            }
            ...
            foreach (var o in Del_Note_Dtails)
            {
                ...
                if (o.id == 0 && o.active == true)
                {
                    ...
                    {
                        //if (o.ItemCode == p.ItemCode && o.sku == p.sku && o.order_no == p.order_no && p.active == true)
        ...
        private void RemoveRefDoc()
        {
            try
            {
                ...
                var msg = new NotificationMessage("LOG_T001_A_VM_Export");
        ...
        #region Tr Mode
        private string _filterString_Tr_mode;
        public string FilterString_Tr_mode
        {
            get { return _filterString_Tr_mode; }
            set
            {
                _filterString_Tr_mode = value;
                RaisePropertyChanged("FilterString_Tr_mode");
                FilterCollection_Tr_mode();
            }
        }
        private void FilterCollection_Tr_mode()
        {
            if (_TransportModeCollection != null)
            {
                _TransportModeCollection.Refresh();
            }
        }
        public bool Filter_Tr_mode(object obj)
        {
            var data = obj as SYS_M026;
            if (data != null)
            {
                if (!string.IsNullOrEmpty(_filterString_Tr_mode))
                {
                    return (data.tr_name != null && data.tr_name.ToString().ToLower().Contains(_filterString_Tr_mode.ToLower()) ||
                            data.tr_mode != null && data.tr_mode.ToString().ToLower().Contains(_filterString_Tr_mode.ToLower())
                        );
                }
                return true;
            }
