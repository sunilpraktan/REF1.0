...
{
    public class MM_T001_STD_VM : WorkspaceViewModel<MM_T001>
    {
        #region AutoSuggest TextBox Region
        public static readonly DependencyProperty AutoSuggestTextViewModelProperty = DependencyProperty.Register("AutoSuggestTextViewModel", typeof(AutoSuggestTextViewModel<dynamic>), typeof(MM_T001_STD_VM));
        ...
        #region . Constructor .
        public MM_T001_STD_VM(string doc_cat, string ts_code) : base()
        {
            MoveFlag = true;
        ...
        public MM_T001_STD_VM(string doc_cat, string ts_code, string doc_no) : base()
        {
            MoveFlag = true;
            this.ts_code_vm = ts_code;
        ...
        private void LoadInitialData(string doc_cat_info, string ReferenceDoc)
        {
            ...
            {//"101,118,119,115,117,127,128" 
                ...
                MC = repository_MC.GetDataWithReturnDomainObject<MC_MM_T001>(MC, Request, "MM_T001_STD", "SCM", "LoadAll", 0, "");
        ...
        private void LoadDocumentByDocumentNumber(object ParameterObject, string ParameterReference)
        {
            try
            {
                ...
                NewRecord = false;
                MoveFlag = false;
                MCTemp = repository_MC.GetDataWithReturnDomainObject<MC_MM_T001>(MCTemp, Request, "MM_T001_STD", "SCM", "LoadDocumentByDocumentNumber", 0, "");
        ...
        private void Load_BackFlip_Data()
        {
            try
            {
                ...
                string Request = "Load_BackFlip_Data" + "!@" + AppSessionState.client + "!@" + AppSessionState.comp_code + "!@" + (RequestPara.location_id ?? AppSessionState.location_Id) + "!@" + (string.IsNullOrWhiteSpace(RequestPara.doc_cat) ? this.doc_cat_vm : RequestPara.doc_cat) + "!@" + (string.IsNullOrWhiteSpace(RequestPara.doc_type) ? MasterEntity.doc_type : RequestPara.doc_type) + "!@" + AppSessionState.EmpId + "!@" + RequestPara.PartyId + "!@" + RequestPara.t_status + "!@" + RequestPara.active + "!@" + Convert.ToDateTime(RequestPara.FromDate).ToString("MM/dd/yyyy") + "!@" + Convert.ToDateTime(RequestPara.ToDate).ToString("MM/dd/yyyy");
                MCTemp = repository_MC.GetDataWithReturnDomainObject<MultipleContext_SEL_T003>(MCTemp, Request, "MM_T001_STD", "SCM", "Load_BackFlip_Data", 0, "");
        ...
        private void ExecuteReferenceDocument(object InputValue)   // this method is little complicated and lengthy need to optimize : pending optimization
        {
            try
            {
                ...
                {
                    string Request = "ExecuteReferenceDocument" + "!@" + AppSessionState.client + "!@" + AppSessionState.comp_code + "!@" + AppSessionState.location_Id + "!@" + doc_cat_vm + "!@" + doc_cat_vm + "!@" + AppSessionState.EmpId + "!@" + AppSessionState.UserID + "!@" + RequestPara.doc_no + "!@" + RequestPara.doc_cat + "!@" + RequestPara.ref_doc;
                    MCTemp = repository_MC.GetDataWithReturnDomainObject<MC_MM_T001>(MCTemp, Request, "MM_T001_STD", "SCM", "ExecuteReferenceDocument", 0, "");
        ...
        private void AddSelectedRef(object InputValue)
        {
            try
            {
                ...
                if (POPUPEntityObject != null)
                {
                    ...
                    if (POPUPEntityObject.doc_cat == "TO" || POPUPEntityObject.doc_cat == "PO" || POPUPEntityObject.doc_cat == "OP" || POPUPEntityObject.doc_cat == "NP" || POPUPEntityObject.doc_cat == "DT" || POPUPEntityObject.doc_cat == "SO")
                    {
                        ...
                        foreach (var item in refdocTOM)
                        {
                            if (item.Select == true)
                            {
                                ...
                                RequestPara.ref_doc = RequestPara.ref_doc + "," + item.party_ref_no;
                            }
                        }
                ...
                if (e.Action == NotifyCollectionChangedAction.Add && ItemDetailsEntity.Count > 0) // Schedule can only enable to add if items exists in Items Entity.
                {
                    foreach (MM_T001_B item in e.NewItems)
                    {
                        ...
                        if (dgSelectedIndexItem <= ItemDetailsEntity.Count)
                        {
                            ...
                            item.qty = ItemDetailsEntity[dgSelectedIndexItem].qty;
                            item.rec_qty = ItemDetailsEntity[dgSelectedIndexItem].qty;
        ...
        protected override void OnSaveAction(InquiryActionResult<MM_T001> result)
        {
            try
            {
                if (Validations() == true)
                {
                    ...
                    if (NewRecord == true)
                    {
                        MasterEntity = repository.SaveWithReturnDomainObject<MM_T001>(MasterEntity, "MM_T001_STD", "SCM");
                    }
                    else if (NewRecord == false)
                    {
                        MasterEntity = repository.UpdateWithReturnDomainObject<MM_T001>(MasterEntity, "MM_T001_STD", "SCM");
                    }
        ...
        protected override void OnPrintAction(InquiryActionResult<MM_T001> result)
        {
            ...
            else
            {
                ...
                MCTemp = repository_MC.GetDataWithReturnDomainObject<MC_MM_T001>(MCTemp, Request, "MM_T001_STD", "SCM", "LoadAll", 0, "");
        ...
        private void OpenDocumentViewer(object InputValue)
        {
            ...
            try
            {
                ...
                string Request = "GetAllFiles" + "!@" + AppSessionState.client + "!@" + (EntityObjectParameter.comp_code ?? AppSessionState.comp_code) + "!@" + (EntityObjectParameter.location_Id ?? AppSessionState.location_Id) + "!@" + (doc_cat_vm ?? "") + "!@" + doc_cat_vm + "!@" + EntityObjectParameter.doc_no + "!@" + EntityObjectParameter.id.ToString();
