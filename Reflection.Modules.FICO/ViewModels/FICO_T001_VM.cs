...
{
    public class FICO_T001_VM : WorkspaceViewModel<ACC_T006>
    {
        #region AutoSuggest TextBox Declaration Region

        // public static readonly DependencyProperty AutoSuggestTextViewModelProperty = DependencyProperty.Register("AutoSuggestTextViewModel", typeof(AutoSuggestTextViewModel<dynamic>), typeof(FICO_T001_VM));
        // public AutoSuggestTextViewModel<dynamic> AutoSuggestTextViewModel { get { return (AutoSuggestTextViewModel<dynamic>)GetValue(AutoSuggestTextViewModelProperty); } set { SetValue(AutoSuggestTextViewModelProperty, value); } }

        private ICollectionView _ReferenceCollection;
        public ICollectionView ReferenceCollection
        {
            get { return _ReferenceCollection; }
            set { _ReferenceCollection = value; RaisePropertyChanged("ReferenceCollection"); }
        }
        ...
        #region Constructor
        public FICO_T001_VM(string ts_code) : base()
        {
            try
            {
        ...
        public FICO_T001_VM(string ts_code,string doc_cat) : base()
        {
            try
            {
        ...
        public FICO_T001_VM(string ts_code, string doc_cat, string doc_no) : base()
        {
            try
            {
                this.ts_code_vm = ts_code;
        ...
        private void ExecuteReference(object InputValue)
        {
            try
            {
                ...
                if (InputValue != null)
                {
                    POPUPEntityObject = (STD_LIST_BE)InputValue; //((IEnumerable)InputValue).Cast<STD_LIST_BE>().ToList()[0];
                }
                if (POPUPEntityObject != null) // Only enter in the code block if EntityObject is Not null and not assigning same value again if event fire twice.
                {
                    string RequestParameterData = "EXECUTE_REFERENCE" + "!@" + AppSessionState.client + "!@" + POPUPEntityObject.comp_code + "!@" + (POPUPEntityObject.location_id ?? "") + "!@" + this.doc_cat_vm + "!@" + this.doc_cat_vm + "!@" + POPUPEntityObject.ref_doc_no + "!@" + POPUPEntityObject.ref_doc_cat + "!@" + POPUPEntityObject.ref_doc_type + "!@" + AppSessionState.UserID;
                    MCTemp = repository_MC.GetDataWithReturnDomainObject<MultipleContext_ACC_T006>(MC, RequestParameterData, "ACC_T006_BL", "FICO", "", 0, "");
        ...
        //{
        //    if(MasterEntity.dc_ind == "D")
        //    {
        //        MasterEntity.loc_curr_amt = MasterEntity.doc_curr_amt * MasterEntity.exc_rate;
        //        MasterEntity.debit = MasterEntity.loc_curr_amt;
        //        if(DetailEntity.Count == 0)
        //        {
        //            MasterEntity.credit = 0;
        //        }
        //    }
        //    else if(MasterEntity.dc_ind == "C")
        //    {
        //        MasterEntity.loc_curr_amt = MasterEntity.doc_curr_amt * MasterEntity.exc_rate;
        //        MasterEntity.credit = MasterEntity.loc_curr_amt;
        //        if (DetailEntity.Count == 0)
        //        {
        //            MasterEntity.debit = 0;
        //        }
        ...
        private void LoadInitialData()
        {
            try
            {
                ...
                string Request = "LOAD_INI" + "!@" + AppSessionState.client + "!@" + AppSessionState.OBJ_COMPANY.comp_code + "!@" + (MasterEntity.location_Id ?? AppSessionState.OBJ_LOCATION.location_id) + "!@" + MasterEntity.doc_cat + "!@" + MasterEntity.doc_type + "!@" + MasterEntity.userid + "!@" + ts_code_vm;
                MC = repository_MC.GetDataWithReturnDomainObject<MultipleContext_ACC_T006>(MC, Request, "ACC_T006_BL", "FICO", "LoadAll", 0, "");

                ReferenceCollection = CollectionViewSource.GetDefaultView(MC.REF_DOC_LIST);
                ReferenceCollection.Filter = new Predicate<object>(FLTR_REF_DOC);
        ...
        private string _FLTR_STR_REF;
        public string FLTR_STR_REF
        {
            get { return _FLTR_STR_REF; }
            set
            {
                _FLTR_STR_REF = value;
                RaisePropertyChanged("FLTR_STR_REF");
                FLTR_COL_REF();
            }
        }

        private void FLTR_COL_REF()
        {
            if (ReferenceCollection != null)
            {
                ReferenceCollection.Refresh();
            }
        }
        public bool FLTR_REF_DOC(object obj)
        {
            ...
            if (data != null)
            {
                if (!string.IsNullOrEmpty(FLTR_STR_REF))
                {
                    return (data.ref_doc_cat != null && data.ref_doc_cat.ToLower().Contains(FLTR_STR_REF.ToLower()) ||
                           data.ref_doc_date != null && data.ref_doc_date.ToString().ToLower().Contains(FLTR_STR_REF.ToLower()) ||
                           data.emp_name != null && data.emp_name.ToLower().Contains(FLTR_STR_REF.ToLower()) ||
                           data.emp_name != null && data.ref_doc_no.ToLower().Contains(FLTR_STR_REF.ToLower()) ||
                           data.doc_cat != null && data.doc_cat.ToLower().Contains(FLTR_STR_REF.ToLower()) ||
                           data.emp_id != null && data.emp_id.ToLower().Contains(FLTR_STR_REF.ToLower()) ||
                           data.note != null && data.note.ToLower().Contains(FLTR_STR_REF.ToLower())
                       );
                }
                return true;
            }
