...
{
    public class FICO_M0020_VM : WorkspaceViewModel<FICO_M0004>
    {
        ...
        //public static readonly DependencyProperty AutoSuggestTextViewModelProperty = DependencyProperty.Register("AutoSuggestTextViewModel", typeof(AutoSuggestTextViewModel<dynamic>), typeof(FICO_M0020_VM));
        ...
        #region Constructor
        public FICO_M0020_VM(string ts_code) : base()
        {
        ...
        private void LoadInitialData()
        {
            try
            {
                string Request = "LOAD_INI_HBANK" + "!@" + AppSessionState.client + "!@" + AppSessionState.OBJ_COMPANY.comp_code;
                MC = repository_MC.GetDataWithReturnDomainObject<MC_FICO_M0004>(MC, Request, "FICO_M0004_A_BL", "FICO", "LOAD_INI", 0, "");
        ...
        private bool Validation()
        {
            foreach (var o in ItemsEntity)
            {
                ...
                if (o.selected == true)
                {
                    ...
                    if (flag > 1)
                    {
                        sms.ButtonSetup = DialogButton.Ok; sms.Caption = "Message"; sms.Text = String.Format("Duplicate Record not allowed for the Bank {0} and House Bank {1}", o.bank_name, o.hb_code); sms.ShowMessage();
                        return false;
                    }
                    if (string.IsNullOrWhiteSpace(o.bank_code))
                    {
                        sms.ButtonSetup = DialogButton.Ok; sms.Caption = "Message"; sms.Text = String.Format("Select House Bank for the Bank {0} and HB Code {1}", o.bank_name, o.hb_code);
            ...
            try
            {
                ...
                try
                {
                    if (InputValue.GetType() == typeof(string) && InputValue != null)
                    {
                        ...
                        {
                            try
                            { POPUPEntityObject = MC.BANK_LIST.Where(x => x.bank_code.Equals(Request, StringComparison.OrdinalIgnoreCase) == true).ToList()[0]; }
                            catch (Exception ex) { }
                        }
                    }
        ...
        protected override void OnSaveAction(InquiryActionResult<FICO_M0004> result)
        {
            try
            {
                ...
                if (Validation() == true)
                {
                    string strReturn = repository.Save<List<FICO_M0004>>(RequestList, "FICO_M0004_A_BL", "FICO");
