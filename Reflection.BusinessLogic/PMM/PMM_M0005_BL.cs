...
{
    public class PMM_M0005_BL : ReflectionBusinessLogic
    {
        ...
        public string Insert(string Request)
        {
            try
            {
                ...
                {
                    var reader = conn.QueryMultiple("PMM_M0005_INS_UPD", new { @Request = Request }, commandType: CommandType.StoredProcedure);
        ...
        public string Update(string Request)
        {
            try
            {
                using (IDbConnection conn = new SqlConnection(ReflectionConnectionString))
                {
                    var reader = conn.QueryMultiple("PMM_M0001_INS_UPD", new { @Request = Request }, commandType: CommandType.StoredProcedure);
        ...
        public string GetData(string RequestValue, string QueryOption, int intValue, string strValue)
        {
            ...
            try
            {
                using (IDbConnection conn = new SqlConnection(ReflectionConnectionString))
                {
                    var reader = conn.QueryMultiple("PMM_M0005_GET", new { @Request = RequestValue }, commandType: CommandType.StoredProcedure);
