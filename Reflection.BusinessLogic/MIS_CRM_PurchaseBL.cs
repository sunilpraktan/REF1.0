...
{
    public class MIS_CRM_PurchaseBL : ReflectionBusinessLogic
    {
        private static string connectionString;
        public MIS_CRM_PurchaseBL(string BusinessEntity)
        {
            connectionString = base.ReflectionConnectionString;
        }
        public MIS_CRM_PurchaseBL()
        {
            connectionString = base.ReflectionConnectionString;
        }
        ...
        public string GetData(string strType, string QueryOption, int intValue, string strValue)
        {
            ...
            {             
                if (RequestOption == "LoadInitialData" || RequestOption == "Report")
                {
                    ...
                    {
                        var reader = conn.QueryMultiple("MIS_CRM_Purchase", new { @request = strType }, commandTimeout: 600, commandType: CommandType.StoredProcedure);
