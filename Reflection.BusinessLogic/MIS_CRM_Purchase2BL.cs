...
{
    public class MIS_CRM_Purchase2BL : ReflectionBusinessLogic
    {
        private static string connectionString;
        public MIS_CRM_Purchase2BL(string BusinessEntity)
        {
            connectionString = base.ReflectionConnectionString;
        }
        public MIS_CRM_Purchase2BL()
        {
            connectionString = base.ReflectionConnectionString;
        }
        ...
        public string GetData(string strType, string QueryOption, int intValue, string strValue)
        {
            ...
            try
            {
                if (RequestOption == "LoadInitialData" || RequestOption == "Report")
                {
                    ...
                    {
                        var reader = conn.QueryMultiple("MIS_CRM_Purchase2", new { @request = strType }, commandTimeout: 600, commandType: CommandType.StoredProcedure);
                        ...
                        {
                            if (strType.Split('!')[1].ToString() == "R003")
                            {
