...
namespace Reflection.BusinessLogic
{
    ...
    public class MM_T001_MR_BL : ReflectionBusinessLogic
    {
        ...
        public MM_T001_MR_BL(string BusinessEntity)
        {
            connectionString = base.ReflectionConnectionString;
        }
        public MM_T001_MR_BL()
        {
            connectionString = base.ReflectionConnectionString;
        }
        ...
        public string Insert(string Request)
        {
            ...
            try
            {
                ...
                {
                    var reader = conn.QueryMultiple("MM_T001_MR_Insert", new { @Request = Request }, commandTimeout: 600, commandType: CommandType.StoredProcedure);
        ...
        public string Update(string Request)
        {
            ...
            try
            {
                using (IDbConnection conn = new SqlConnection(connectionString))
                {
                    var reader = conn.QueryMultiple("MM_T001_MR_Update", new { @Request = Request }, commandTimeout: 600, commandType: CommandType.StoredProcedure);
        ...
        public string Delete(string Request)
        {
            try
            {
                using (IDbConnection conn = new SqlConnection(connectionString))
                {
                    int intOut = conn.Execute("MM_T001_MR_Delete", new { @doc_no = Request }, commandType: CommandType.StoredProcedure);
        ...
        public string GetData(string RequestValue, string QueryOption, int intValue, string strValue)
        {
            ...
            try
            {
                using (IDbConnection conn = new SqlConnection(connectionString))
                {
                    var reader = conn.QueryMultiple("MM_T001_MR_LoadAll", new { @Request = RequestValue }, commandTimeout: 600, commandType: CommandType.StoredProcedure);

                    if (RequestOption == "LoadInitialData")
                    {
                        var documentDataFlipGrid = reader.Read<MM_T001Flip>().ToList();
