...
{
    public class ADM_M0123_BL : ReflectionBusinessLogic
    {
        ...
        public string Insert(string Request, string RequestOption)
        {
            try
            {
                ...
                {
                    var reader = conn.QueryMultiple("ADM_M0123_INS", new { @Request = Request }, commandTimeout: 0, commandType: CommandType.StoredProcedure);
        ...
        public string Update(string Request, string RequestOption)
        {
            try
            {
                using (IDbConnection conn = new SqlConnection(ReflectionConnectionString))
                {
                    var reader = conn.QueryMultiple("ADM_M0123_UPD", new { @Request = Request }, commandTimeout: 0, commandType: CommandType.StoredProcedure);
        ...
        public string GetData(string RequestValue, string QueryOption, int intValue, string strValue)
        {
            try
            {
                ...
                using (IDbConnection conn = new SqlConnection(ReflectionConnectionString))
                {
                    var reader = conn.QueryMultiple("ADM_M0123_GET", new { @Request = RequestValue }, commandTimeout: 0, commandType: CommandType.StoredProcedure);

                    if (RequestOption == "LOAD_INI")
                    {
                        MC.OBJECT_TYPE_LIST = reader.Read<STD_LIST_BE>().ToList();
                        MC.STATUS_LIST = reader.Read<ADM_M0013>().ToList();
                        MC.CLASS_TYPE_LIST = reader.Read<Classification>().ToList();
                        MC.CLASS_LIST = reader.Read<Classification>().ToList();

                    }
                    else if (RequestOption == "LOAD_OBJECTS")
                    {
                        MC.OBJECT_LIST = reader.Read<STD_LIST_BE>().ToList();
                    }
