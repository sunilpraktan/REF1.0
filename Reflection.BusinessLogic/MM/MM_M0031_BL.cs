...
{
    public class MM_M0031_BL : ReflectionBusinessLogic
    {
        ...
        public string Insert(string Request)
        {
            try
            {
                ...
                {
                    var reader = conn.QueryMultiple("MM_M0031_INS_UPD", new { @Request = Request }, commandTimeout: 300, commandType: CommandType.StoredProcedure);
        ...
        public string Update(string Request)
        {
            try
            {
                using (IDbConnection conn = new SqlConnection(ReflectionConnectionString))
                {
                    var reader = conn.QueryMultiple("MM_M0031_INS_UPD", new { @Request = Request }, commandType: CommandType.StoredProcedure);
                    //MasterEntity = reader.Read<STD_BE_A>().ToList()[0];
                    MC.ItemsEntity = reader.Read<STD_BE_A>().ToList();
                }
        ...
        public string GetData(string RequestValue, string strType, int intValue, string strValue)
        {
            ...
            try
            {
                using (IDbConnection conn = new SqlConnection(ReflectionConnectionString))
                {
                    var reader = conn.QueryMultiple("MM_M0031_GET", new { @Request = RequestValue }, commandType: CommandType.StoredProcedure, commandTimeout: 600);
