...
{
    public class QMS_T014_BL : ReflectionBusinessLogic
    {
        private static string connectionString;
        QMS_T003 MasterEntity = new QMS_T003();
        MC_QMS_T003 MC = new MC_QMS_T003();

        public QMS_T014_BL(string BusinessEntity)
        {
        ...
        public string Insert(string Request)
        {
            try
            {
                ...
                MasterEntity = MC.INSP_LOT_LIST[0];
                RequestValue = ObjectSerializationService.ObjectToXML(MasterEntity);

                using (IDbConnection conn = new SqlConnection(connectionString))
                {
                    var reader = conn.QueryMultiple("QMS_T003_U_INS", new { @Request = RequestValue }, commandTimeout: 0, commandType: CommandType.StoredProcedure);
                    MC.USAGE_DECISION = reader.Read<QMS_T003_U>().ToList();
                }
