...
{
    public class QMS_R01_BL : ReflectionBusinessLogic
    {
        public QMS_R01_BL()
        {}
        public string GetData(string Request, string QueryOption, int intValue, string strValue)
        {
            ...
            try
            {
                if (RequestOption == "LOAD_INI" || RequestOption == "REPORT")
                {
                    ...
                    {
                        var reader = conn.QueryMultiple("QMS_R01_SP", new { @request = Request }, commandTimeout: 0, commandType: CommandType.StoredProcedure);
