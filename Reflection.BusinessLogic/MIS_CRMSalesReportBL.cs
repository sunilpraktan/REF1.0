...
{
    public class MIS_CRMSalesReportBL : ReflectionBusinessLogic
    {
        ...
        public MIS_CRMSalesReportBL(string BusinessEntity)
        {
            connectionString = base.ReflectionConnectionString;
        }
        public MIS_CRMSalesReportBL()
        {
            connectionString = base.ReflectionConnectionString;
        }
            ...
            try
            {
                if (strValue == "MIS_CRM_Sales2")
                {
                    if (RequestOption == "LoadInitialData" || RequestOption == "Report")
                    {
                        ...
                        {
                            var reader = conn.QueryMultiple("MIS_CRM_SalesPeriodic2", new { @request = strType }, commandType: CommandType.StoredProcedure);
                ...
                else
                {
                    if (RequestOption == "LoadInitialData" || RequestOption == "Report")
                    {
                        using (IDbConnection conn = new SqlConnection(connectionString))
                        {
                            var reader = conn.QueryMultiple("MIS_CRM_SalesPeriodic", new { @request = strType }, commandTimeout: 0, commandType: CommandType.StoredProcedure);
                            ...
                            if (RequestOption == "LoadInitialData")
                            {
                                ...
                                var Trade_TypesList = reader.Read<SYS_M037>().ToList();
                                MC.Trade_Types = Trade_TypesList.ToList();
