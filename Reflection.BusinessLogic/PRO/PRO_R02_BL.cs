...
{
    public class PRO_R02_BL : ReflectionBusinessLogic
    {
        public PRO_R02_BL()
        { }
        public string GetData(string Request, string QueryOption, int intValue, string strValue)
        {
            ...
            try
            {
                ...
                if (RequestOption == "LOAD_INI" || RequestOption == "REPORT")
                {
                    ...
                    {
                        var reader = conn.QueryMultiple("PRO_R02", new { @request = Request }, commandTimeout: 0, commandType: CommandType.StoredProcedure);
