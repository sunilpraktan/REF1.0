...
{
    public class ENG_M0005_BL : ReflectionBusinessLogic
    {
        ENG_M0005 MasterEntity = new ENG_M0005();
        ...
        public string Insert(string Request)
        {
            try
            {
                ...
                {
                    var reader = conn.QueryMultiple("ENG_M0005_INS", new { @Request = Request }, commandTimeout: 0, commandType: CommandType.StoredProcedure);

                    MC.MEASUREMENT_POINT_LIST = reader.Read<ENG_M0005>().ToList();
                    MasterEntity = MC.MEASUREMENT_POINT_LIST[0];
                }
        ...
        public string Update(string Request)
        {
            try
            {
                using (IDbConnection conn = new SqlConnection(ReflectionConnectionString))
                {
                    var reader = conn.QueryMultiple("ENG_M0005_UPD", new { @Request = Request }, commandTimeout: 0, commandType: CommandType.StoredProcedure);
        ...
        public string GetData(string RequestValue, string QueryOption, int intValue, string strValue)
        {
            ...
            try
            {
                using (IDbConnection conn = new SqlConnection(ReflectionConnectionString))
                {
                    var reader = conn.QueryMultiple("ENG_M0005_GET", new { @Request = RequestValue }, commandTimeout: 0, commandType: CommandType.StoredProcedure);
