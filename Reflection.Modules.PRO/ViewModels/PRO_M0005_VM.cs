...
{
    public class PRO_M0005_VM : WorkspaceViewModel<ADM_M028>
    {
        #region AutoSuggest TextBox Declaration Region
        public static readonly DependencyProperty AutoSuggestTextViewModelProperty = DependencyProperty.Register("AutoSuggestTextViewModel", typeof(AutoSuggestTextViewModel<dynamic>), typeof(PRO_M0005_VM));
        ...
        private void LoadInitialData()
        {
            try
            {
                ...
                string Request = "LoadInitialData" + "!@" + AppSessionState.client + "!@" + AppSessionState.OBJ_COMPANY.comp_code + "!@" + MasterEntity.location_Id + "!@0!@1";
                ...
                StringListDepartments = MC.Departments.Select(x => x.dept_name?.ToString()).ToList();
        ...
        private void LoadBackFlipData(object InputValue)
        {
            try
            {
                ...
                string Request = "LOAD_BACKFLIP" + "!@" + AppSessionState.client + "!@" + AppSessionState.OBJ_COMPANY.comp_code + "!@" + REQ_PARA.location_id + "!@" + REQ_PARA.from_date + "!@" + REQ_PARA.active + "!@!@1";
                MCTemp = repository_MCTemp.GetDataWithReturnDomainObject<MultipleContext_SEL_T001>(MCTemp, Request, "ADM_M0028_BL", "ADM", "LoadAll", 0, "");
        ...
        private void LoadDocumentByDocumentNumber(object ParameterObject, string ParameterReference)
        {
            ...
            try
            {
                if (((IEnumerable)ParameterObject).Cast<STD_LIST_BE>().ToList().Count > 0)
                {
                    ...
                    var msg = new NotificationMessage("PRO_M0005_VM");
        ...
        private void InsertCountry(object InputValue, bool NewRow, bool AllowDuplicate, bool AllowModify)
        {
            ...
            try
            {
                if (InputValue.GetType() == typeof(string) && InputValue != null)
                {
                    ...
                    {
                        try
                        { POPUPEntityObject = MC.Country.Where(x => x.CntryName.Equals(Request, StringComparison.OrdinalIgnoreCase) == true).ToList()[0]; }
                        catch (Exception ex)
                        {
                        }
                    }
        ...
        private bool Validation()
        {
           if (string.IsNullOrWhiteSpace(MasterEntity.comp_code))
            ...
            else if (MasterEntity.PartyNm == null || MasterEntity.PartyNm.ToString().Trim().Length == 0)
            {
                ...
                showMessageService.ButtonSetup = DialogButton.Ok;
                showMessageService.Caption = "Required";
                showMessageService.Text = String.Format("Please input Party name...", this.Title);
                showMessageService.ShowMessage();
                return false;
            }
            else if (string.IsNullOrWhiteSpace(MasterEntity.PartyType))
            {
                ...
                showMessageService.ButtonSetup = DialogButton.Ok;
                showMessageService.Caption = "Required";
                showMessageService.Text = String.Format("Please input Supplier Type...", this.Title);
                showMessageService.ShowMessage();
                return false;
            }
        ...
        #region Constructor
        public PRO_M0005_VM(string ts_code, string doc_cat) : base()
        {
        ...
        private void DefaultValues()
        {
            ...
            MasterEntity.Customer = false;
            MasterEntity.Supplier = true;
            MasterEntity.PartyType = "001";
            MasterEntity.comp_code = AppSessionState.OBJ_COMPANY.comp_code;
            MasterEntity.location_Id = AppSessionState.OBJ_LOCATION.location_id;
