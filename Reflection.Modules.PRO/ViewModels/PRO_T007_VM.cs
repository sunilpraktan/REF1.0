...
{
    public class PRO_T007_VM : WorkspaceViewModel<PUR_T002_A>
    {
        #region AutoSuggest TextBox Region
        public static readonly DependencyProperty AutoSuggestTextViewModelProperty = DependencyProperty.Register("AutoSuggestTextViewModel", typeof(AutoSuggestTextViewModel<dynamic>), typeof(PRO_T007_VM));
            ...
            set
            {
                if (_dgSelectedIndexItem != value)
                {
                    ...
                    if (_dgSelectedIndexItem >= 0 && _dgSelectedIndexItem < dgItemsEntity.Count && ItemEntityObject != null)
                    {
                        ...
                        ASReqNo = new AutoSuggestTextViewModel<dynamic>(MC.PR_LIST, TheFilter, SuggestedValue, "po_req_no", "req_no", true);
        ...
        public ICollectionView PR_ITEM_COLL
        {
        ...
        #endregion
        #region Constructor
        private static PRO_T007_VM Expose_ViewModel;
        public static PRO_T007_VM SharedViewModel()
        {
            return Expose_ViewModel ?? (Expose_ViewModel = new PRO_T007_VM(null, null));
        }
        public PRO_T007_VM(string doc_cat, string ts_code) : base()
        {
        ...
        public PRO_T007_VM(string doc_cat, string ts_code, string doc_no) : base()
        {
            CursorControl.SetBusyState();
            this.doc_cat_vm = doc_cat;
            this.ts_code_vm = ts_code;
        ...
        private void LoadInitialData(string doc_cat_info, string ReferenceDoc)
        {
            ...
            try
            {
                ...
                //string Request = "LoadInitialData" + "!@" + AppSessionState.OBJ_COMPANY.comp_code + "!@" + AppSessionState.client.ToString() + "!@" + doc_cat_info + "!@" + doc_cat_info + "!@" + AppSessionState.po_code + "!@" + AppSessionState.pg_code + "!@" + AppSessionState.EmpId + "!@" + AppSessionState.OBJ_LOCATION.location_id;
                ...
                //var RequisitionData = (from o in MC.PR_LIST where o.@select == true select o).ToList();
                //SuggestedValue = new ValueConverter(x => x == null ? "" : ((STD_LIST_BE)x).req_no);
                //TheFilter = (o, prefix) => (((STD_LIST_BE)o).req_no ?? "").ToString().ToLower().Contains(prefix.ToLower()) || (((STD_LIST_BE)o).ItemCode ?? "").ToString().ToLower().Contains(ItemEntityObject.ItemCode.ToLower()) || (((STD_LIST_BE)o).sku ?? "").ToString().ToLower().Contains(ItemEntityObject.sku ?? "".ToLower());
                //ASReqNo = new AutoSuggestTextViewModel<dynamic>(MC.PR_LIST, TheFilter, SuggestedValue, "po_req_no", "req_no", true);
                //ASReqNo.AutoSuggestVM.IsEmptyValueAllowed = true;
                //ASReqNo.AutoSuggestVM.IsFreeTextAllowed = false;
        ...
        private void LoadBackFlipData(object Parameter)
        {
            ...
            {
                CursorControl.SetBusyState();
                string Request = "LoadBackFlipData" + "!@" + AppSessionState.client + "!@" + MasterEntity.comp_code + "!@" + (SearchEntityObject.location_Id ?? AppSessionState.OBJ_LOCATION.location_id) + "!@" + doc_cat_vm + "!@" + (SearchEntityObject.doc_type ?? "") + "!@!@" + AppSessionState.UserID + "!@" + (SearchEntityObject.EmpId ?? AppSessionState.EmpId) + "!@" + ts_code_vm + "!@" + AppSessionState.po_code + "!@" + (AppSessionState.pg_code ?? "") + "!@" + (SearchEntityObject.PartyId ?? "") + "!@!@" + SearchEntityObject.active + "!@" + SearchEntityObject.t_status + "!@" + Convert.ToDateTime(SearchEntityObject.from_date).ToString("MM/dd/yyyy") + "!@" + Convert.ToDateTime(SearchEntityObject.to_date).ToString("MM/dd/yyyy");
        ...
        private void DefaultValues(string doc_cat_info)
        {
            ...
            //}
            //else if (doc_cat_vm == "RT")
            //{
            //    MasterEntity.supplying_plant = AppSessionState.OBJ_LOCATION.location_id;
            ...
            //var tempt_display = (from o in MC.t_statusList where o.ind_default == "1" select o).ToList();
                    ...
                    if (dgItemsEntity != null && dgItemsEntity.Count > 0 && ItemRowIndex >= 0 && ItemRowIndex < dgItemsEntity.Count) //Condition satisfy only if Items collection is not empty.
                    {
                        if (dgItemsEntity[ItemRowIndex].qty >= 0 && dgItemsEntity[ItemRowIndex].unit_price >= 0 && dgItemsEntity[ItemRowIndex].active != false) // Must not null or empty.
                        {
                            ...
                            else
                            {
                                ...
                                dgItemsEntity[ItemRowIndex].discount_amt = decimal.Round((decimal)dgItemsEntity[ItemRowIndex].discount_amt, RoundUpDecimals);
                                dgItemsEntity[ItemRowIndex].sub_total = (dgItemsEntity[ItemRowIndex].gross_value - dgItemsEntity[ItemRowIndex].discount_amt);
                                dgItemsEntity[ItemRowIndex].sub_total = decimal.Round((decimal)(dgItemsEntity[ItemRowIndex].sub_total ?? 0), RoundUpDecimals);
                            }
                        }
        ...
        private bool Validation()
        {
            ...
            if ((MasterEntity.PartyId == null || MasterEntity.PartyId == "") && (this.doc_cat_vm != "TO" || doc_cat_vm != "RT"))
            {
                IShowMessageViewService showMessageService = this.GetViewService<IShowMessageViewService>();
                showMessageService.ButtonSetup = DialogButton.Ok;
                showMessageService.Caption = "Message";
            ...
            if ((MasterEntity.supplying_plant == null || MasterEntity.supplying_plant == "" || MasterEntity.rec_plant == null || MasterEntity.rec_plant == "") && (this.doc_cat_vm == "TO" || doc_cat_vm == "RT"))
            {
                IShowMessageViewService showMessageService = this.GetViewService<IShowMessageViewService>();
                showMessageService.ButtonSetup = DialogButton.Ok;
                showMessageService.Caption = "Message";
            ...
            if ((MasterEntity.bill_address_id == null || MasterEntity.bill_address_id == "") && (this.doc_cat_vm != "TO" || doc_cat_vm != "RT"))
            {
                IShowMessageViewService showMessageService = this.GetViewService<IShowMessageViewService>();
                showMessageService.ButtonSetup = DialogButton.Ok;
                showMessageService.Caption = "Message";
            ...
            if ((MasterEntity.del_address_id == null || MasterEntity.del_address_id == "") && (this.doc_cat_vm != "TO" || doc_cat_vm != "RT"))
            {
                IShowMessageViewService showMessageService = this.GetViewService<IShowMessageViewService>();
                showMessageService.ButtonSetup = DialogButton.Ok;
                showMessageService.Caption = "Message";
        ...
        private void InsertRecPlantOld(object InputValue)
        {
            try
            {
                ...
                if (POPUPEntityObject != null) //Application:New Record Only.Condition: Only enter in the code block if EntityObject is Not null and not assigning same value again if event fire twice.
                {
                    ...
                    if (doc_cat_vm == "TO" || doc_cat_vm == "RT")
                    {
            ...
            try
            {
                ...
                if (POPUPEntityObject != null) //&& MasterEntity.PartyId != POPUPEntityObject.PartyId) //Application:New Record Only.Condition: Only enter in the code block if EntityObject is Not null and not assigning same value again if event fire twice.
                {
                    ...
                    else if (NewRecord == true && (MasterEntity.PartyId != POPUPEntityObject.PartyId || MasterEntity.party_name != POPUPEntityObject.PartyNm))
                    {
                        ...
                        MC.TCondition = MCTemp.TCondition;
        ...
        private void WindowEvetCall(object InputValue)
        {
            try
            {
                ...
                else
                {
                    ...
                    //Task t = LoadInitialDataNew(doc_cat_vm, null);
                    //await LoadInitialDataNew(doc_cat_vm, null);
                    //await Task.Run(() => LoadInitialDataNew(doc_cat_vm, null));
                    DefaultValues(doc_cat_vm);
                }
                Messenger.Default.Send<NotificationMessage>(new NotificationMessage(ts_code_vm));
            }
            ...
            try
            {
                ...
                else
                {
                    if (((IEnumerable)ParameterObject).Cast<PUR_T002_AFlip>().ToList().Count > 0)
                    {
                        ...
                        Request = "LOAD_DOC_BY_DOC_NO" + "!@" + AppSessionState.client + "!@" + AppSessionState.OBJ_COMPANY.comp_code + "!@!@!@!@" + ParameterEntityObject.po_no + "!@" + AppSessionState.UserID + "!@" + AppSessionState.EmpId + "!@" + ts_code_vm + "!@" + AppSessionState.po_code + "!@" + (AppSessionState.pg_code ?? "") + "!@" + ParameterEntityObject.PartyId;
        ...
        private void CreateDocumentByReferenceDocumentNumber(object ParameterObject, string ParameterReference)
        {
            try
            {
                ...
                if (ParameterObject.GetType() == typeof(string) && ParameterObject != null) // This Block of code read parameter . First for string and Entity Object in else part.
                {
                    ...
                    {
                        if (ParameterObject.ToString() == "CreateDocumentFromRequisitionItemNumber" && MasterEntity.t_status == "01")
                        {
                            //var RequisitionData = (from o in MC.PR_LIST where o.@select == true select o).ToList();
                            //SuggestedValue = new ValueConverter(x => x == null ? "" : ((STD_LIST_BE)x).doc_no);
                            //TheFilter = (o, prefix) => (((STD_LIST_BE)o).doc_no ?? "").ToString().ToLower().Contains(prefix.ToLower());
                            ...
                            //MCTemp = repository_MC.GetDataWithReturnDomainObject<MultipleContext_PUR_T002_A>(MCTemp, Request, "PRO_T002_BL", "PRO", "CreateDocumentFromRequisitionItemNumber", 0, "");
                            //MasterEntity = MCTemp.DocumentMaster[0];
                            //dgItemsEntity.Clear();
                            //dgItemScheduleEntity.Clear();
                            if (dgItemsEntity.Count == 0)
                            ...
                            foreach (var item in MC.PR_LIST)
                            {
                                if (item.selected == true)
                                {
                                    var InputValueIfExists = dgItemsEntity.Where(x => x.ItemCode == item.item_code && x.sku == item.sku && x.del_address_id == MasterEntity.del_address_id && (x.unit_price ?? 0) == (item.cost ?? 0)).FirstOrDefault(); // Prefer Primary Key for this instruction.
                                    int intIndex = dgItemsEntity.IndexOf(dgItemsEntity.Where(x => x.ItemCode == item.item_code && x.sku == item.sku && x.del_address_id == MasterEntity.del_address_id && (x.unit_price ?? 0) == (item.cost ?? 0)).FirstOrDefault()); // Prefer Primary Key for this instruction.
                                    ...
                                    if (InputValueIfExists == null && CheckIfExistsAlready == null)
                                    {
                                        ...
                                        ItemObject.t_status = "01";
                                        //ItemObject.t_display = "Draft";
                                        item.t_display = (from o in MC.t_statusList where o.t_status == item.t_status select o.t_display).FirstOrDefault();
                                        ItemObject.comp_code = AppSessionState.OBJ_COMPANY.comp_code;
                                        ...
                                        ScheduleItemEntityObject.t_status = "01";
                ...
                if (MC.ItemListForPopup.Count > 0 && string.IsNullOrWhiteSpace(MasterEntity.rec_plant) == false) // This Block of code read parameter . First for string and Entity Object in else part.
                {
                    ...
                    foreach (var item in MC.ItemListForPopup)
                    {
                        if (item.Select == true)
                        {
                            ...
                            ItemObject.t_status = "01";
                            ItemObject.t_display = (from o in MC.t_statusList where o.t_status == "01" select o.t_display).FirstOrDefault();
                            ItemObject.comp_code = AppSessionState.OBJ_COMPANY.comp_code;
                            ItemObject.location_Id = AppSessionState.OBJ_LOCATION.location_id;
                            ItemObject.active = true;
                            ...
                            ScheduleItemEntityObject.t_status = "01";
                            ScheduleItemEntityObject.t_display = (from o in MC.t_statusList where o.t_status == "01" select o.t_display).FirstOrDefault();
                            ScheduleItemEntityObject.active = true;
                            ScheduleItemEntityObject.location_Id = AppSessionState.OBJ_LOCATION.location_id;
                            ScheduleItemEntityObject.comp_code = AppSessionState.OBJ_COMPANY.comp_code;
...
#pragma warning disable CS0108 // Member hides inherited member; missing new keyword
        ...
        public bool Filter_Terms(object obj)
        {
            ...
            if (data != null)
            {
                ...
                {
                    return (data.ts_code != null && data.tc_code.ToString().ToLower().Contains(_FilterString_Terms.ToLower())) ||
        ...
        public bool FilterRequisitionNo(object obj)
        {
            ...
            if (data != null)
            {
                ...
                {
                    return (data.doc_no != null && data.doc_no.ToString().ToLower().Contains(_FilterString_Requisition.ToLower()) ||
                           (data.doc_date != null && data.doc_date.ToString().ToLower().Contains(_FilterString_Requisition.ToLower())) ||
                           (data.item_code != null && data.item_code.ToString().ToLower().Contains(_FilterString_Requisition.ToLower())) ||
                           (data.item_name != null && data.item_name.ToString().ToLower().Contains(_FilterString_Requisition.ToLower())) ||
                           (data.sku_desc != null && data.sku_desc.ToString().ToLower().Contains(_FilterString_Requisition.ToLower())) ||
                           (data.plan_date != null && data.plan_date.ToString().ToLower().Contains(_FilterString_Requisition.ToLower())) ||
                           ...
                           data.emp_name != null && data.emp_name.ToString().ToLower().Contains(_FilterString_Requisition.ToLower()));
