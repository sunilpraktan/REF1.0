...
{
    public class PRO_T009_VM : WorkspaceViewModel<PUR_T002_A>
    {
        #region AutoSuggest TextBox Region
        public static readonly DependencyProperty AutoSuggestTextViewModelProperty = DependencyProperty.Register("AutoSuggestTextViewModel", typeof(AutoSuggestTextViewModel<dynamic>), typeof(PRO_T009_VM));
        ...
        #endregion
        #region Constructor
        private static PRO_T009_VM Expose_ViewModel;
        public static PRO_T009_VM SharedViewModel()
        {
            return Expose_ViewModel ?? (Expose_ViewModel = new PRO_T009_VM(null));
        }
        public PRO_T009_VM(string ts_code) : base()
        {
        ...
        public PRO_T009_VM(string ts_code, string doc_no) : base()
        {
            this.ts_code_vm = ts_code;
        ...
        private void LoadInitialData()
        {
            try
            {
                ...
                List<STD_LIST_BE> RequisitionNo = MC.PR_LIST;
                PR_ITEM_COLL = CollectionViewSource.GetDefaultView(RequisitionNo);
                PR_ITEM_COLL.Filter = new Predicate<object>(FilterRequisitionNo);
                strListRequisition = RequisitionNo.Select(x => x.doc_no).ToList();
        ...
        private void OpenDocumentViewer(object InputValue)
        {
            ...
            try
            {
                ...
                string Request = "GetAllFiles" + "!@" + AppSessionState.client + "!@" + (EntityObjectParameter.comp_code ?? AppSessionState.OBJ_COMPANY.comp_code) + "!@" + (EntityObjectParameter.location_Id ?? AppSessionState.OBJ_LOCATION.location_id) + "!@!@!@" + EntityObjectParameter.ItemCode;
                        ...
                        if (!String.IsNullOrEmpty(dgItemsEntity[RowIndex].tax_id.Trim()) && dgItemsEntity[RowIndex].active != false) //Condition satisfy only if Selected Item having Tax assigned.
                        {
                            ...
                            if (TaxListTemp.Count > 0)
                            {
                                foreach (ACC_M013_P SingleTax in TaxListTemp) // Foreach Loop for Computation of Induvidual Tax as per the proerties in Tax Master.
                                {
                                    ...
                                    TotalDocumentTaxes.Add(new ACC_T006_B() { id = 0, tax_amount = TaxAmount, account_id = 0, sequence = SingleTax.sequence, doc_no = MasterEntity.po_no, manual = "Auto", base_amount = BasePrice, amount = SingleTax.amount, tax_code_id = SingleTax.id, account_analytic_id = 0, base_code_id = SingleTax.id, tax_name = SingleTax.description, gl_code = "", ItemCode = dgItemsEntity[RowIndex].ItemCode, sku = dgItemsEntity[RowIndex].sku, item_line_id = dgItemsEntity[RowIndex].id, fin_year = "2015", active = true, location_Id = AppSessionState.OBJ_LOCATION.location_id, comp_code = AppSessionState.OBJ_COMPANY.comp_code });
                                }
        ...
        private void LoadDocumentByDocumentNumber(object ParameterObject, string ParameterReference) //For Flip
        {
            try
            {
                ...
                var msg = new NotificationMessage("PRO_T009_VM");
                ...
                if (POPUPEntityObject != null) // Only enter in the code block if ENtity Not null.
                {
                    ...
                    else if (dgSelectedIndexItem >= 0 && dgItemsEntity.Count > dgSelectedIndexItem) //Update Row: If Old or Blank row get added, then Edit Rule will applicable in this Loop.
                    {
                        if (dgItemsEntity[dgSelectedIndexItem].id == 0 && ((IndexOfExistValue == -1) || (IndexOfExistValue != -1 && AllowDuplicate == true))) // It will allowe to add unique value or Duplicate value if AllowDuplicate Status == True
                        {
                            ...
                            dgItemScheduleEntity[dgSelectedIndexItemSchedule].location_Id = AppSessionState.OBJ_LOCATION.location_id;
                            dgItemScheduleEntity[dgSelectedIndexItemSchedule].comp_code = AppSessionState.OBJ_COMPANY.comp_code;
