...
{
    public class PRO_T002_VM : WorkspaceViewModel<PUR_T002_A>
    {
        #region AutoSuggest TextBox Region
        public static readonly DependencyProperty AutoSuggestTextViewModelProperty = DependencyProperty.Register("AutoSuggestTextViewModel", typeof(AutoSuggestTextViewModel<dynamic>), typeof(PRO_T002_VM));
        ...
        public int dgSelectedIndexItem
        {
            ...
            set
            {
                if (_dgSelectedIndexItem != value)
                {
                    ...
                    {
                        TotalDocumentTaxesItem = new ObservableCollection<ACC_T006_B>(TotalDocumentTaxes.Where(tax => tax.ItemCode == dgItemsEntity[dgSelectedIndexItem].ItemCode && (tax.sku?.ToString() ?? "") == (dgItemsEntity[dgSelectedIndexItem].sku?.ToString() ?? "") && tax.item_row_id == dgItemsEntity[dgSelectedIndexItem].id));
                    }
        ...
        //}
        public PRO_T002_VM(string ts_code)
            : base()
        {
        ...
        public PRO_T002_VM(string ts_code, string doc_no)
            : base()
        {
            CursorControl.SetBusyState();
            this.ts_code_vm = ts_code;
        ...
        private void LoadInitialData()
        {
            ...
            try
            {
                MasterEntity.doc_cat = "IN";
                MasterEntity.doc_type = "IN";
                ...
                TheFilter = (o, prefix) => (((STD_DOC_TYPE)o).doc_type_user ?? "").ToString().ToLower().Contains(prefix.ToLower()) ||
                                           (((STD_DOC_TYPE)o).doc_desc ?? "").ToString().ToLower().Contains(prefix.ToLower());
                ASOrderType = new AutoSuggestTextViewModel<dynamic>(MC.DOC_TYPE_LIST, TheFilter, SuggestedValue, "doc_type_user", true);
                ...
                ASDefault2 = new AutoSuggestTextViewModel<dynamic>(MC.CONDITION_LIST, TheFilter, SuggestedValue, "ts_code", "ts_code", true);
                ASDefault2.AutoSuggestVM.IsEmptyValueAllowed = true;
                ASDefault2.AutoSuggestVM.IsFreeTextAllowed = true;

                SuggestedValue = new ValueConverter(x => x == null ? "" : ((ADM_M0051)x).con_type);
                TheFilter = (o, prefix) => (((ADM_M0051)o).con_type ?? "").ToString().ToLower().Contains(prefix.ToLower()) || (((ADM_M0051)o).long_text ?? "").ToString().ToLower().Contains(prefix.ToLower());
                ASTermsCond = new AutoSuggestTextViewModel<dynamic>(MC.CONDITION_LIST, TheFilter, SuggestedValue, "ts_code", "ts_code", true);
        ...
        private void TaxComputation(bool Compute)
        {
            try
            {
                if (Compute == true)
                {
                    ...
                    {
                        if (dgItemsEntity[dgSelectedIndexItem].qty > 0 && dgItemsEntity[dgSelectedIndexItem].unit_price > 0) // Must not null or empty.
                        {
        ...
        {
            if (e.PropertyName == "qty" || e.PropertyName == "unit_price" || e.PropertyName == "tax_id")
            {
        ...
        {
            //This will get called when the property of an object inside the collection changes
            if (sender.ToString() == "qty" || sender.ToString() == "unit_price" || sender.ToString() == "tax_id")
            {
                Computation(true, dgSelectedIndexItem);
            }
            this.ErrorExist = false; /*MasterEntity.HasErrors;*/
        ...
        private void LoadDocumentByReferenceDocumentNumber_(object ParameterObject, string ParameterReference)
        {
            try
            {
                string Request = "";
        ...
        private void LoadDocumentByDocumentNumber(object ParameterObject, string ParameterReference)
        {
            ...
            try
            {
                ...
                if (ParameterObject.GetType() == typeof(string) && ParameterObject != null) // This Block of code read parameter . First for string and Entity Object in else part.
                {
                    ...
                    else if (ParameterReference == "DocumentNo")
                    {
                        ParametersStringValue = ParameterObject.ToString().Trim();
                    }
                ...
                else if (ParameterObject != null)
                {
                    if (((IEnumerable)ParameterObject).Cast<PUR_T002_AFlip>().ToList().Count > 0)
                    {
                        ...
                        Request = "LoadDocumentWithDocumentNumber" + "!@" + ParameterEntityObject.po_no + "!@" + ParameterEntityObject.doc_type;
                        NewRecord = false;
                ...
                if (ParameterReference == "ReferenceDocument")
                {
                    ...
                    MasterEntity.doc_cat = "IN";
                    MasterEntity.doc_type = "IN";
                    MasterEntity.doc_type_user = "IN";
                    MasterEntity.display_doc_type = "IN";
                    MasterEntity.doc_desc = "Purchase Order";
                    MasterEntity.po_no = "";
                    MasterEntity.po_date = DateTime.Now;
                    MasterEntity.add_by = AppSessionState.UserID;
...
#pragma warning disable CS0108 // Member hides inherited member; missing new keyword
        ...
        protected override void OnPrintAction(InquiryActionResult<PUR_T002_A> result)
        {
            ...
            Request = "LoadDocumentWithDocumentNumber" + "!@" + MasterEntity.po_no;
            ...
            ReportManager.DisplayReport(objDataSource, objDataSourceName, "\\Procurment\\PurchaseInquiry.rdlc", getParametersList(), ReportDisplayName);
        }
